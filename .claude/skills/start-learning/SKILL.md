...
(mission, entry point, progress so far) and ask whether they want to resume
(`/learn`), re-run placement, or start over.
...
alongside this one): 5 areas, 10 questions, mapped to an entry phase. If the
learner says they already know where they want to start ("just start me at
phase 7"), respect that, skip the quiz, and record the entry point as
self-selected.
...
## Placement
- Date: <YYYY-MM-DD>
- Score: <total>/10 (<area breakdown, or "self-selected">)
