...
RU_COMPLETE_TRANSLATIONS = {
    ...
    "Your coding agent becomes your tutor. Two commands, no clone, no setup:": "Ваш coding agent становится преподавателем. Две команды — без клонирования и настройки:",
    ...
    "**Option A — learn in your terminal *(recommended)*.** Install the learning skills into any agent and let the course drive itself:": "**Вариант A — учиться в терминале *(рекомендуется)*.** Установите учебные skills в любого агента, и курс сам поведёт вас дальше:",
    ...
    "Cloning also auto-loads the learning skills in Claude Code, and gives every lesson's code to `/learn` for real execution instead of read-along.": "При клонировании учебные skills автоматически загружаются в Claude Code, а `/learn` получает код каждого урока для реального выполнения, а не только чтения.",
    ...
    "The learning skills (any agent: Claude, Cursor, Codex, OpenClaw, Hermes, …)": "Учебные skills (для любого агента: Claude, Cursor, Codex, OpenClaw, Hermes, …)",
    ...
    "Phase 0: Setup & Tooling `12 lessons`": "Фаза 0: настройка и инструменты `12 lessons`",
    ...
    "Install course skills into your agent": "Установите skills курса в своего агента",
    "Two skill sets, two installers:": "Два набора skills — два способа установки:",
    "**The learning skills** (`/start-learning`, `/learn`, `/course-guide`, `/claude-certification`, `/find-your-level`, `/check-understanding`) live under [`skills/`](skills/) and install into any agent with one command — no clone, no Python:": "**Учебные skills** (`/start-learning`, `/learn`, `/course-guide`, `/claude-certification`, `/find-your-level`, `/check-understanding`) находятся в [`skills/`](skills/) и устанавливаются в любого агента одной командой — без клонирования и Python:",
    "`skills` writes to whichever directory your agent picks up: `.claude/skills/`, `.cursor/skills/`, `.codex/skills/`, OpenClaw's skills folder, Hermes's bundle path, or any SKILL.md-aware tool. One command, every agent.": "`skills` записывает файлы в каталог, который использует ваш агент: `.claude/skills/`, `.cursor/skills/`, `.codex/skills/`, каталог skills OpenClaw, bundle-путь Hermes или любой инструмент с поддержкой SKILL.md. Одна команда для любого агента.",
    "**The lesson artifacts.** The repo ships 388 skills and 99 prompts under `phases/**/outputs/`; install them via `scripts/install_skills.py`. Requires cloning the repo. Supports tag filters, dry-runs, and per-agent layouts:": "**Артефакты уроков.** Репозиторий содержит 388 skills и 99 промптов в `phases/**/outputs/`; установите их через `scripts/install_skills.py`. Требуется клонирование репозитория. Поддерживаются фильтры по тегам, dry run и разные layouts для агентов:",
    "`<target>` is the skills directory for your agent (examples: `~/.claude/skills/`, `~/.cursor/skills/`, `~/.config/openclaw/skills/`, `.skills/`, or any path your agent reads).": "`<target>` — каталог skills вашего агента (например, `~/.claude/skills/`, `~/.cursor/skills/`, `~/.config/openclaw/skills/`, `.skills/` или любой читаемый агентом путь).",
