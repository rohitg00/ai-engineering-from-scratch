...
(function () {
  ...
  function motion(path, kt, dur, begin) {
    return svgEl('animateMotion', { path: path, keyTimes: kt, values: '0;1', dur: dur + 's', begin: (begin || 0) + 's', repeatCount: 'indefinite', calcMode: 'linear' });
  }
  ...
  function contractNet(host) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      pkt.appendChild(motion(fwd, '0;0.2;1', period));
      svg.appendChild(pkt);
      var back = 'M' + bx + ',' + bys[i] + ' L' + mx + ',' + my;
      var bid = svgEl('circle', { r: '4', fill: (i === win ? WARN : MUTE) });
      bid.appendChild(anim('opacity', '0;0;1;1;0', '0;0.3;0.34;0.55;0.6', period));
      bid.appendChild(motion(back, '0;0.34;0.6;1', period));
      svg.appendChild(bid);
    }
  ...
  function workStealing(host) {
    ...
    for (j = 0; j < 6; j++) {
      ...
      g.appendChild(motion(path, '0;0.35;1', period, begin));
      svg.appendChild(g);
    }
  ...
  function handoffRouting(host) {
    ...
    tok.appendChild(motion(mpath, '0;0.33;0.66;1', period));
    for (i = 0; i < 3; i++) {
  ...
  function agentCard(host) {
    ...
    disc.appendChild(motion('M' + cx + ',' + (cy - 20) + ' L' + rx + ',' + (ry + 14), '0;0.18;1', period));
  ...
  function checkpointReplay(host) {
    ...
    wa.appendChild(svgEl('animateMotion', { path: waPath, keyTimes: '0;0.4;1', values: '0;0.5;1', dur: period + 's', repeatCount: 'indefinite', calcMode: 'linear' }));
    ...
    wb.appendChild(svgEl('animateMotion', { path: wbPath, keyTimes: '0;0.55;1', values: '0;0.55;1', dur: period + 's', repeatCount: 'indefinite', calcMode: 'linear' }));
