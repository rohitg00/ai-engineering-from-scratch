...
(function () {
  ...
  function videoDenoise(host) {
    var svg = newSvg(220);
    var fw = 90, fh = 64, gap = 14, oy = 40, ox = 22, frames = 5;
  ...
  function a2aLifecycle(host) {
    ...
    states.forEach(function (st, i) {
      ...
      svg.appendChild(svgEl('rect', { x: x - 42, y: 120, width: 84, height: 26, rx: 13, fill: 'var(--bg-surface,#eee)', stroke: SOFT, 'stroke-width': '1' }, [
        anim('fill', 'var(--bg-surface,#eee);' + BP + ';' + BP + ';var(--bg-surface,#eee)', '8s', { keyTimes: '0;' + (0.15 + i * 0.2).toFixed(2) + ';' + (0.32 + i * 0.2).toFixed(2) + ';1' }),
        anim('fill-opacity', '1;0.85;0.85;1', '8s', { keyTimes: '0;' + (0.15 + i * 0.2).toFixed(2) + ';' + (0.32 + i * 0.2).toFixed(2) + ';1' })
      ]));
