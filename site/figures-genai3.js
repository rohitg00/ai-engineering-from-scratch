...
(function () {
  ...
  function anim(attr, vals, dur, extra) {
    var a = { attributeName: attr, values: vals, dur: dur, repeatCount: 'indefinite', calcMode: 'spline' };
  ...
  function matryoshka(host) {
    ...
    for (i = 0; i < dims; i++) {
      ...
      // cell stays opaque while cut >= keep ; we animate cut value 1 -> .125 -> 1
      var lo = '0.1', hiOp = '0.85';
      // opacity high when keep <= cut. Approximate with 3-phase keyframes per cell.
      var op;
      if (keep <= 0.125) op = hiOp + ';' + hiOp + ';' + hiOp + ';' + hiOp;
      else if (keep <= 0.25) op = hiOp + ';' + lo + ';' + lo + ';' + hiOp;
      else if (keep <= 0.5) op = hiOp + ';' + lo + ';' + lo + ';' + hiOp;
      else op = hiOp + ';' + lo + ';' + lo + ';' + hiOp;
      cell.appendChild(anim('opacity', op, '7s', { keyTimes: '0;0.4;0.6;1', keySplines: '.4 0 .2 1;0 0 1 1;.4 0 .2 1' }));
