...
(function () {
  ...
  function cdZero(host) {
    ...
    for (i = 0; i < n; i++) {
      ...
      var rs = svgEl('path', { d: 'M ' + (x + bw / 2) + ' ' + (optY - 14) + ' L ' + (x + bw / 2) + ' ' + (optY - 2), stroke: 'var(--ink-mute,#777)', 'stroke-width': '2', 'marker-end': '' });
  ...
  function cdPipeline(host) {
    var s = svg(240), N = 4, M = 4, PAD = 50, cellW = 70, cellH = 26, gap = 6;
    var rowY = 40;
    var i, j;
    // stage row labels
    for (i = 0; i < N; i++) {
