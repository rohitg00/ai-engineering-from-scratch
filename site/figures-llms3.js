...
(function () {
  ...
  function encoderDecoder(host) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      er.appendChild(anim('fill', 'var(--bg,#fafaf5);' + SOFT, '4s', { keyTimes: '0;0.2', fill: 'freeze', repeatCount: '1' }));
      svg.appendChild(er);
    }
  ...
  function rnnVsParallel(host) {
    ...
    for (i = 0; i < n; i++) {
      for (var j = i + 1; j < n; j++) {
        ...
        web.appendChild(anim('opacity', '0;0.5;0', '4s', { keyTimes: '0;0.12;0.45' }));
        svg.appendChild(web);
      }
    }
  ...
  function draftVerifyTokens(host) {
    ...
    sweep.appendChild(anim('opacity', '0;0.6;0.6;0', '5s', { keyTimes: '0;0.5;0.6;0.65' }));
  ...
  function selfCritiqueLoop(host) {
    ...
    stages.forEach(function (s, i) {
      ...
      node.appendChild(anim('fill', 'var(--bg,#fafaf5);' + BP + ';var(--bg,#fafaf5)', '6s', { keyTimes: '0;' + (0.04 + i * 0.333).toFixed(3) + ';' + (0.2 + i * 0.333).toFixed(3) }));
