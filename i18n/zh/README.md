...
<p align="center">
  <img src="../../assets/banner.svg" alt="《从零开始学 AI 工程》参考手册横幅" width="100%">
</p>
...
<p align="center">
  ...
  <br><sub>翻译后的首页已提交到仓库。英文版为准；课程页面由机器翻译并存放在 <code>translations</code> 分支。详见 <a href="../../docs/i18n.md">docs/i18n.md</a>。</sub>
</p>
...
## 出自 [Agent Memory——排名第一的持久记忆工具 ⭐](https://github.com/rohitg00/agentmemory) <a href="https://github.com/rohitg00/agentmemory/stargazers"><img src="https://img.shields.io/github/stars/rohitg00/agentmemory?style=flat-square&labelColor=fafaf5&color=3553ff" alt="GitHub 星标数"></a> 作者之手，可自然适配任何智能体或聊天助手。
...
<p align="center"><sub><b>114,584</b> 位读者 &nbsp;·&nbsp; 过去 30 天内 <b>181,995</b> 次页面浏览 &nbsp;·&nbsp; 截至 2026-08-29</sub></p>
<!-- STATS:END -->

## 从这里开始：选择你想构建的内容

开始之前，你不必浏览完 523 节课。只需选择一个目标。每个链接都会在 GitHub 或网站上打开同一套课程，两个版本使用完全相同的课程代码。

| 你的目标 | 在 GitHub 上学习 | 在网站上学习 |
|---|---|---|
| 我刚入门，想打下完整基础 | [阶段 0：环境配置与工具](../../phases/00-setup-and-tooling/) | [开发环境](https://aiengineeringfromscratch.com/lesson?path=phases/00-setup-and-tooling/01-dev-environment) |
| 我懂 Python，想学习数学和机器学习基础 | [阶段 1：数学基础](../../phases/01-math-foundations/) | [线性代数直觉](https://aiengineeringfromscratch.com/lesson?path=phases/01-math-foundations/01-linear-algebra-intuition) |
| 我想构建生产级 LLM 应用 | [阶段 11：LLM 工程](../../phases/11-llm-engineering/) | [提示词工程](https://aiengineeringfromscratch.com/lesson?path=phases/11-llm-engineering/01-prompt-engineering) |
...
| 我想在实现前先确定正确的构建方向 | [产品判断与交付路线](../../learning-paths/shaping-the-build.json) | [产品判断与交付](https://aiengineeringfromscratch.com/lesson?path=phases/14-agent-engineering/47-outcomes-before-output&learningPath=shaping-the-build) |
| 我想使用模型上下文协议（MCP）进行构建 | [模型上下文协议（MCP）路线](../../phases/13-tools-and-protocols/README.md#model-context-protocol-mcp-path) | [模型上下文协议（MCP）路线](https://aiengineeringfromscratch.com/lesson?path=phases/13-tools-and-protocols/06-mcp-fundamentals&learningPath=model-context-protocol) |
| 我想编写并发布 Agent Skills | [Agent Skills 专项路线](../../phases/13-tools-and-protocols/README.md#agent-skills-fast-path) | [Agent Skills 路线](https://aiengineeringfromscratch.com/lesson?path=phases/13-tools-and-protocols/22-skills-and-agent-sdks&learningPath=agent-skills) |
...
不确定自己适合从哪里开始？使用 [`start-learning` 定位导师](../../skills/start-learning/SKILL.md)，或查看[网站的先决条件指南](https://aiengineeringfromscratch.com/prereqs.html)。

你还可以在 [AI 工程学习路线](https://aiengineeringfromscratch.com/learning-paths.html)中比较四个核心领域和六条职业路线。

### 用同样的方法学习每一课

1. **阅读** `docs/en.md`，并用自己的话解释核心思想。
2. **输入并构建**关键代码，不要把代码块当成摆设。
3. 从仓库根目录（即包含 `README.md` 和 `phases/` 的目录）**运行**课程命令。
4. **保留证据**：命令、工作目录、退出码、有意义的输出，以及你修改或产出的成果。
5. 只有当你能解释输出，并且无需猜测就能做出一项小改动时，才**继续**学习。

除非课程明确要求切换目录，否则课程页面中的命令路径都相对于仓库根目录。如果一节课提供多种语言，请运行你正在学习的语言所对应的实现。
...
预检会区分当前必需的条件与之后才需要的工具。每项必需条件若检查失败，都会给出检测到的原因和修复命令。第二条命令运行一节零依赖课程，最后会展示矩阵乘向量正是神经网络层内部执行的运算。请保存这段终端输出，作为你的第一份证据。

## 30 秒添加 AI 导师

如果你已经安装 Node.js、`npx` 和支持技能的编程智能体，只需两条命令，就能让编程智能体变成你的导师。安装或读取导师无需克隆仓库；运行专项路线实验需要 `python3`，Agent Skills 宿主实验还需要选择一个宿主，并具备可写的用户级或项目级技能作用域。
...
调用语法由宿主决定，而非可移植的 `SKILL.md` 格式：

| 宿主 | 开始课程 | 开始模型上下文协议（MCP）路线 | 开始 Agent Skills 路线 | 运行阶段测验 |
|---|---|---|---|---|
...
| 其他兼容宿主 | `使用 start-learning 开始课程。` | `使用 learn-mcp 开始模型上下文协议（MCP）路线。` | `使用 learn-agent-skills 开始 Agent Skills 工程路线。` | `使用 check-understanding 测验我对阶段 13 的掌握程度。` |

十道题的定位测验会根据你已有的知识确定起始阶段，并把个性化学习计划保存到 `LEARNING.md`。之后，`learn` 技能每次会话讲授一节课，依次涵盖概念、数学、代码和测验。它会直接从本仓库获取课程；遇到难点时，`course-guide` 技能会把你带到讲解该内容的确切课程。在 Codex 中使用 `learn` 和 `course-guide` 调用这些技能；在 Claude Code 中使用 `/learn` 和 `/course-guide`；在其他兼容宿主中，则按名称要求使用相应技能。

只想学习模型上下文协议（MCP）？请使用你的宿主所对应的 MCP 调用方式。它会创建 `MCP-LEARNING.md`，并沿着一条包含 17 节课的路线，依次学习无状态请求、传输、双向通信、安全性、可靠性、注册表治理与一致性证据。确切顺序和检查点见[模型上下文协议（MCP）清单](../../learning-paths/model-context-protocol.json)。

只想学习 Agent Skills？请使用你的宿主所对应的 Agent Skills 调用方式。它会创建 `AGENT-SKILLS-LEARNING.md`，并沿着一条连贯的五课路线，依次学习契约、发现、调用、沙箱边界、发布评估和真实宿主可移植性。你也可以从网站上的 [Agent Skills 路线](https://aiengineeringfromscratch.com/lesson?path=phases/13-tools-and-protocols/22-skills-and-agent-sdks&learningPath=agent-skills)开始。

安装程序会列出它能配置的宿主，并询问安装位置。如果你还没有 Node.js、`npx`、`python3`、受支持的宿主或可写作用域，请使用网站，或手动阅读 `docs/en.md`。这种方式仍能学习概念，但在预检条件具备之前，真实宿主中的发现、调用、脚本和卸载证据仍无法完成。请前往 [aiengineeringfromscratch.com](https://aiengineeringfromscratch.com) 阅读课程。

## 运作方式

大多数 AI 资料都零散地讲授知识：这里一篇论文，那里一篇微调文章，别处又来一个炫目的智能体演示。这些碎片很少能连成体系。你做出了聊天机器人，却解释不了它的损失曲线；你给智能体接上了函数，却说不清调用它的模型内部，注意力机制究竟做了什么。

这套课程就是那条主线。20 个阶段、523 节课、四种语言：Python、TypeScript、Rust、Julia。一端是线性代数，另一端是自主智能体群。每种算法都先从基础数学开始亲手构建：反向传播、分词器、注意力机制、智能体循环。等到 PyTorch 登场时，你早已明白它在底层做什么。

每节课都遵循同一个循环：阅读问题、推导数学、编写代码、运行测试、保留成果。没有五分钟速成视频，没有复制粘贴式部署，也不会全程手把手带领。课程免费、开源，并且可以在你自己的笔记本电脑上运行。
...
二十个阶段层层递进。数学是地基，智能体和生产系统是屋顶。如果你已经掌握下层知识，可以直接往后学；但不要跳过基础之后，又疑惑为什么上层内容出了问题。
...
每节课都遵循六个环节。*亲手构建 / 实际使用* 的分法贯穿始终：先从零实现算法，再用生产级库运行同一种操作。因为你亲手写过精简版本，所以能真正理解框架在做什么。
...
**选项 A——在终端中学习（推荐）。** 完成上述 Node.js、`npx`、宿主和作用域预检后，把学习技能安装到兼容的智能体中，让课程自动引导学习：

```bash
npx skills add rohitg00/ai-engineering-from-scratch
```

请使用上方针对不同宿主的调用表。安装后的技能提供 `start-learning`、`learn`、`course-guide`，以及专项的 `learn-mcp` 和 `learn-agent-skills` 路线。无需克隆即可从本仓库获取课程正文；但若要运行从仓库复制的代码命令以及可执行的 MCP 或 Agent Skills 实验，则必须在本地克隆仓库。学习进度会保存在项目中的 `LEARNING.md`、`MCP-LEARNING.md` 或 `AGENT-SKILLS-LEARNING.md`，因此每次会话都能接续上次的进度。

**选项 B——阅读。** 在 [aiengineeringfromscratch.com](https://aiengineeringfromscratch.com) 打开任何已完成的课程，或展开[目录](#contents)中的任一阶段。无需配置，也无需克隆。
...
克隆仓库还会在 Claude Code 中自动加载学习技能，并把每节课的代码交给 `learn` 导师实际运行，而不只是陪你阅读。

### 先决条件

- 你会编写代码（任何语言均可，懂 Python 更好）。
- 你想理解 AI **究竟如何运作**，而不只是调用 API。

### 准备 Claude 认证

[Claude 认证学院](../../certifications/claude/README.md)是一套免费、开源的备考课程，覆盖全部四条官方 Claude 认证路线：Associate Foundations、Developer Foundations、Architect Foundations 和 Architect Professional。每条路线都包含与考试蓝图对应的课程、可运行实验、诊断测验、综合项目，以及一套完整的原创模拟考试。

你可以在 Claude Code、Codex、ChatGPT、Cursor 或其他智能体中使用 [AI 原生 GitHub 入门指南](../../certifications/claude/GETTING_STARTED.md)。在 Codex 中运行 `claude-certification`，在 Claude Code 中运行 `/claude-certification`，或要求其他宿主使用 `claude-certification`。它会选择一条认证路线，在 `CLAUDE-CERTIFICATION.md` 中创建可持续保存的学习路径，逐步授课、运行真实实验，并根据学习成果提供反馈。同一套课程也可在[认证网站](https://aiengineeringfromscratch.com/certifications.html)上学习。

本学院是根据公开考试目标编写的独立学习资料，与 Anthropic 无隶属关系，不会复现正式考试题目，也不保证学习者通过考试。

### 学习技能

| 技能 | 功能 |
|---|---|
| [`start-learning`](../../skills/start-learning/SKILL.md) | 一次性入门：明确学习动机、完成定位测验，并将个性化计划保存到 `LEARNING.md`。 |
| [`learn`](../../skills/learn/SKILL.md) | 导师循环：先进行热身回忆，再以互动方式讲授下一课，最后完成测验；同时记录进度和复习队列。 |
| [`course-guide`](../../skills/course-guide/SKILL.md) | 主题导航器。无论是“我该去哪里学习注意力机制？”还是“我的损失值是 NaN”，都能定位到确切课程并提供链接。 |
| [`learn-mcp`](../../skills/learn-mcp/SKILL.md) | 模型上下文协议（MCP）专项导师。创建 `MCP-LEARNING.md`，按 17 节课的清单授课，并记录传输格式、安全性、可靠性和一致性证据。 |
| [`learn-agent-skills`](../../skills/learn-agent-skills/SKILL.md) | Agent Skills 专项导师。创建 `AGENT-SKILLS-LEARNING.md`，讲授第 22、24、25、26 和 27 课，并记录真实宿主中的实践证据。 |
| [`claude-certification`](../../skills/claude-certification/SKILL.md) | 认证导师。选择 CCAO-F、CCDV-F、CCAR-F 或 CCAR-P；讲授每节课、运行实验、审阅成果、组织诊断和模拟考试，并保存进度。 |
| [`find-your-level`](../../skills/find-your-level/SKILL.md) | 十题定位测验。根据你的知识水平确定起始阶段，并生成包含课时估算的个性化路线。 |
| [`check-understanding <phase>`](../../skills/check-understanding/SKILL.md) | 每阶段八道题的测验，提供反馈和需要复习的具体课程。请使用上方调用表中的 Codex、Claude Code 或自然语言形式。 |
...
`phases/` 下包含 20 个阶段的核心课程可编译为一套六卷本书。CI 会用相同的核心课程源文件构建 EPUB 和 PDF，并将它们附加到每个 [GitHub 版本](https://github.com/rohitg00/ai-engineering-from-scratch/releases)中；下方链接始终指向最新版本。卷号表示丛书中的顺序，而不是版本号：每份文件都会标注带日期的版次，旧版仍可从对应的版本页面下载。

认证课程有意不转换为图书。其中的 AI 导师状态、可运行实验、交互式图示、诊断测验和限时模拟考试，仍以 GitHub 和网站为主要载体。
...
| 5 | 智能体篇 · 多模态、协议、自主系统与智能体群 | 12-16 | [EPUB](https://github.com/rohitg00/ai-engineering-from-scratch/releases/latest/download/aiefs-vol5-agents.epub) · [PDF](https://github.com/rohitg00/ai-engineering-from-scratch/releases/latest/download/aiefs-vol5-agents.pdf) |
...
图书是一份快照，而这个仓库则是持续更新的版本。每章末尾都会链接回课程的动画图示、测验和可运行代码。可使用 `python3 scripts/build_book.py` 在本地构建（需要 pandoc）；流水线详情见 [book/README.md](../../book/README.md)。
...
其他课程往往以*“恭喜，你学会了 X”*收尾；这里的每节课都会产出一个**可复用工具**，可安装或粘贴到你的日常工作流中。
...
<tr>
<td valign="top">粘贴到任何 AI 助手中，针对特定任务获得专家级帮助。</td>
<td valign="top">放入 Claude、Cursor、Codex、OpenClaw、Hermes，或任何能够读取 <code>SKILL.md</code> 的智能体。</td>
<td valign="top">部署为自主工作者——你会在阶段 14 亲手编写它的循环。</td>
<td valign="top">接入任何兼容 MCP 的客户端。你会在阶段 13 从头到尾完成构建。</td>
</tr>
</table>
...
阶段 14 第 1 课：智能体循环。约 120 行纯 Python 代码，零依赖。
...
<details id="phase-1">
<summary><b>阶段 1 — 数学基础</b> &nbsp;<code>22 节课</code>&nbsp; <em>通过代码直观理解每个 AI 算法背后的数学。</em></summary>
<br/>
...
<details id="phase-2">
<summary><b>阶段 2 — 机器学习基础</b> &nbsp;<code>18 节课</code>&nbsp; <em>经典机器学习 — 仍是大多数生产级 AI 系统的支柱。</em></summary>
<br/>
...
<details id="phase-3">
<summary><b>阶段 3 — 深度学习核心</b> &nbsp;<code>13 节课</code>&nbsp; <em>从第一性原理构建神经网络。亲手实现之前，不使用框架。</em></summary>
<br/>
...
<details id="phase-4">
<summary><b>阶段 4 — 计算机视觉</b> &nbsp;<code>28 节课</code>&nbsp; <em>从像素走向理解 — 涵盖图像、视频、3D、VLM 和世界模型。</em></summary>
<br/>
...
<details id="phase-5">
<summary><b>阶段 5 — NLP：从基础到进阶</b> &nbsp;<code>29 节课</code>&nbsp; <em>语言是通往智能的接口。</em></summary>
<br/>
...
<details id="phase-6">
<summary><b>阶段 6 — 语音与音频</b> &nbsp;<code>17 节课</code>&nbsp; <em>聆听、理解、表达。</em></summary>
<br/>
...
<details id="phase-7">
<summary><b>阶段 7 — 深入解析 Transformer</b> &nbsp;<code>16 节课</code>&nbsp; <em>改变一切的架构。</em></summary>
<br/>
...
<details id="phase-8">
<summary><b>阶段 8 — 生成式 AI</b> &nbsp;<code>15 节课</code>&nbsp; <em>创造图像、视频、音频、3D 等内容。</em></summary>
<br/>
...
<details id="phase-10">
<summary><b>阶段 10 — 从零构建 LLM</b> &nbsp;<code>24 节课</code>&nbsp; <em>构建、训练并理解大语言模型。</em></summary>
<br/>
...
<details id="phase-11">
<summary><b>阶段 11 — LLM 工程</b> &nbsp;<code>17 节课</code>&nbsp; <em>让 LLM 在生产环境中发挥作用。</em></summary>
<br/>
...
<details id="phase-12">
<summary><b>阶段 12 — 多模态 AI</b> &nbsp;<code>25 节课</code>&nbsp; <em>跨模态地看、听、读与推理 — 从 ViT 图像块到计算机操作智能体。</em></summary>
<br/>
...
<details id="phase-13">
<summary><b>阶段 13 — 工具与协议</b> &nbsp;<code>31 节课</code>&nbsp; <em>连接 AI 与现实世界的接口。</em></summary>
<br/>
...
<details id="phase-14">
<summary><b>阶段 14 — 智能体工程</b> &nbsp;<code>54 节课</code>&nbsp; <em>从第一性原理构建智能体，可靠地使用编程智能体，并在实现前塑造工作。</em></summary>
<br/>
...
<details id="phase-15">
<summary><b>阶段 15 — 自主系统</b> &nbsp;<code>22 节课</code>&nbsp; <em>长程智能体、自我改进与 2026 年安全技术栈。</em></summary>
<br/>
...
<details id="phase-16">
<summary><b>阶段 16 — 多智能体与群体系统</b> &nbsp;<code>25 节课</code>&nbsp; <em>协作、涌现与集体智能。</em></summary>
<br/>
...
<details id="phase-17">
<summary><b>阶段 17 — 基础设施与生产</b> &nbsp;<code>28 节课</code>&nbsp; <em>将 AI 交付到现实世界。</em></summary>
<br/>
...
<details id="phase-18">
<summary><b>阶段 18 — 伦理、安全与对齐</b> &nbsp;<code>30 节课</code>&nbsp; <em>构建造福人类的 AI。这不是可选项。</em></summary>
<br/>
...
<details id="phase-19">
<summary><b>阶段 19 — 综合项目</b> &nbsp;<code>85 节课</code>&nbsp; <em>17 个端到端产品 + 9 条深度构建路线。每个项目 20–40 小时；每条路线 4–12 节课。</em></summary>
<br/>
...
可将它们接入 Claude、Cursor、Codex、OpenClaw、Hermes，或任何能够读取 SKILL.md / AGENTS.md 目录的智能体。这些是真正实用的工具，不是课后作业。
...
`skills` 会写入安装时选择的宿主和作用域，例如 `.claude/skills/`、`.cursor/skills/`、`.codex/skills/` 或其他受支持的技能目录。请确认所选宿主能够发现该目标目录。

**课程工件。** 仓库在 `phases/**/outputs/` 下提供了 396 个技能和 99 个提示词；可通过 `scripts/install_skills.py` 安装。此方式需要先克隆仓库，并支持按标签筛选、试运行以及针对不同智能体的目录布局：
...
`<target>` 是你的智能体所使用的技能目录（例如 `~/.claude/skills/`、`~/.cursor/skills/`、`~/.config/openclaw/skills/`、`.skills/`，或智能体能够读取的任何路径）。

默认情况下，脚本会拒绝覆盖现有目标；列出所有冲突路径后，以状态码 1 退出。使用 `--dry-run` 可预览冲突，使用 `--force` 可执行覆盖。每次非试运行操作都会在目标目录写入 `manifest.json`，其中包含按类型和阶段分组的完整清单。请选择你的智能体能够读取的布局：
...
阶段 14 的综合项目提供了一套可复用的智能体工作台包（AGENTS.md、模式定义以及初始化 / 验证 / 交接脚本）。使用以下命令即可将其脚手架安装到任意仓库：
...
你将获得七个已接通的工作台界面、一个初始 `task_board.json`，以及采用 `schema_version: 1` 的全新 `agent_state.json`。接下来，编辑任务和 `AGENTS.md`，运行 `scripts/init_agent.py`，再将契约交给你的智能体。该工作台包的源文件位于 `phases/14-agent-engineering/42-agent-workbench-capstone/outputs/agent-workbench-pack/`。

### 以 JSON 浏览完整课程

`scripts/build_catalog.py` 会遍历磁盘上的每个阶段、每节课和每个工件，并在仓库根目录写入 `catalog.json`。一个文件囊括课程的全部真实信息。
...
目录清单由文件系统生成，而不是从 README 中提取，因此其中的数量始终与磁盘上的实际内容一致。你可以用它构建网站、支持下游工具，或验证 README 中的数量是否发生偏移。其模式定义记录在脚本顶部。

GitHub Action（`.github/workflows/curriculum.yml`）会在每个 PR 中重新构建 `catalog.json`；如果提交的文件已经过期，构建将失败。编辑任何课程后，请运行 `python3 scripts/build_catalog.py` 并提交结果，否则 CI 会拒绝该 PR。同一工作流还会以仅警告模式运行 `audit_lessons.py`（因此已有偏差不会阻碍贡献者）。

### 对每节课的 Python 代码进行冒烟检查

`scripts/lesson_run.py` 会对每节课 `code/` 目录下的所有 `.py` 文件进行字节码编译。默认模式只检查语法——不会执行代码，无需 API 密钥，也无需重量级机器学习依赖。它能够捕获贡献者最常引入的回归问题（缩进错误、损坏的 f-string 和意外改动）。
...
`--execute` 会运行每节课的 `code/main.py`（或第一个 `.py` 文件），超时时间为 10 秒。如果课程入口文件以 `# requires: pkg1, pkg2` 注释开头，并列出了非标准库依赖，则该课程会被跳过，原因为 `needs <deps>`。此脚本需主动选择运行，未接入 CI。

仅使用标准库，要求 Python 3.10+。设置 `LINK_CHECK_SKIP=domain1,domain2` 可覆盖默认跳过列表（`twitter.com`、`x.com`、`linkedin.com`、`instagram.com`、`medium.com`——这些域名会严格拦截自动化 HEAD/GET 请求）。

## 从哪里开始

| 背景 | 起点 | 预计用时 |
|---|---|---|
| 编程和 AI 新手 | 阶段 0——环境搭建 | 约 306 小时 |
| 会 Python，刚接触机器学习 | 阶段 1——数学基础 | 约 270 小时 |
| 会机器学习，刚接触深度学习 | 阶段 3——深度学习核心 | 约 200 小时 |
| 会深度学习，想学习大语言模型和智能体 | 阶段 10——从零构建大语言模型 | 约 100 小时 |
| 资深工程师，只想学习智能体工程 | 阶段 14——智能体工程 | 约 60 小时 |
| 只想构建生产级 MCP 系统 | [模型上下文协议（MCP）学习路径](../../learning-paths/model-context-protocol.json) | 约 23 小时 15 分钟 |
| 只想构建生产级智能体技能 | [智能体技能工程学习路径](../../learning-paths/agent-skills.json) | 约 9.5 小时 |
...
<table>
<tr>
<th align="left" width="50%"><sub>图 003 · A</sub><br/><b>行业信号</b></th>
<th align="left" width="50%"><sub>图 003 · B</sub><br/><b>涵盖的奠基论文</b></th>
</tr>
<tr>
<td valign="top">

> *“如今最热门的编程语言是英语。”*<br/> — **Andrej Karpathy**（[推文](https://x.com/karpathy/status/1617979122625712128)）
>
> *“软件工程正在我们眼前被重塑。”*<br/> — **Boris Cherny**，Claude Code 创作者
>
> *“模型会不断进步，而真正具有复利效应的能力，是**知道该构建什么**。”*<br/> — 2026 年行业共识

</td>
<td valign="top">

- *Attention Is All You Need（注意力就是你所需要的一切）* — Vaswani 等，2017 → [阶段 7](#phase-7)
- *Language Models are Few-Shot Learners（语言模型是少样本学习器）*（GPT-3）→ [阶段 10](#phase-10)
- *Denoising Diffusion Probabilistic Models（去噪扩散概率模型）* → [阶段 8](#phase-8)
- *InstructGPT / RLHF（基于人类反馈的强化学习）* → [阶段 10](#phase-10)
- *Direct Preference Optimization（直接偏好优化）* → [阶段 10](#phase-10)
- *Chain-of-Thought Prompting（思维链提示）* → [阶段 11](#phase-11)
- *ReAct：大语言模型中的推理与行动* → [阶段 14](#phase-14)
- *Model Context Protocol（模型上下文协议）* — Anthropic → [阶段 13](#phase-13)
...
| 为团队或学校复刻项目 | [FORKING.md](../../FORKING.md) |
...
任何规则失败时，退出状态码均为非零值。规则（L001–L010）会验证目录结构、`docs/en.md` 是否存在及是否含有 H1、`code/` 是否非空、`quiz.json` 模式（拒绝曾导致问题 #102 的旧版 `q/choices/answer` 键），以及课程文档中的相对链接。
...
本课程免费并采用 MIT 许可证，共有 523 节课。课程维护资金来自现金赞助和获批的硬件实验室合作伙伴。

**影响力：** 每月 114,584 名访客 · 181,995 次页面浏览（经 Vercel Web Analytics 于 2026-08-29 验证）· 50,728 颗 GitHub 星标（于 2026-08-29 验证）。

### 当前赞助商

| 赞助商 | 赞助内容 |
|---|---|
| [CodeRabbit](https://coderabbit.link/rohit-ghumare) · [iii](https://iii.dev?utm_source=ai-engineering-from-scratch&utm_medium=readme&utm_campaign=sponsor) | 当前赞助商，支持课程维护和开源开发。 |
| [![Vercel 开源计划](https://vercel.com/oss/program-badge-2026.svg)](https://vercel.com/open-source-program) | 由 Vercel 开源计划提供开源基础设施支持。 |

| 级别 | 美元/月 | 赞助权益 |
|------|------|---|
| 支持者 | $25 | 名称列入 BACKERS.md |
| 青铜 | $250 | README 赞助区中的纯文字条目 + 发布日推文 |
| 白银 | $750 | README 中的小型徽标 + 在 API 课程中列为受支持的服务商之一 |
| 黄金 | $2,000 | README 中的中型徽标 + 赞助商页面 + 每季度在 X / LinkedIn 联合推广 |
| 铂金 | $5,000 | 首屏主视觉徽标 + 一节专属集成课程，最多 1 家合作伙伴 |
| 钻石 / 冠名合作伙伴 | $10,000 | 独家冠名位置 + 主视觉徽标 + 季度报告及联合推广，最多 1 家合作伙伴 |

硬件制造商可通过单独的[硬件实验室合作伙伴政策](../../SPONSORS.md#hardware-lab-partner)支持本课程。设备属于实物支持，不会自动获得现金赞助级别。

完整价目表、硬性规则、定价基准和影响力数据请参阅 [SPONSORS.md](../../SPONSORS.md)。可通过 [GitHub Sponsors](https://github.com/sponsors/rohitg00) 进行赞助。

```text
░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒
```

如果这份手册对你有帮助，请为仓库点亮星标。你的支持能让项目持续发展。
