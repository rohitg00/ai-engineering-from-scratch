...
Zamansal fark (TD) öğrenimi farkı böler. Tek bir `(s, a, r, s')` geçişinden, tek adımlı bir hedef `r + γ V(s')` oluşturun ve `V(s)`'yi ona doğru itin. Modeli yok. Tam bölüm yok. RHS'de yaklaşık bir `V` kullanılmasından kaynaklanan sapma, ancak MC'den ve birinci adımdaki çevrimiçi güncellemelerden önemli ölçüde daha düşük sapma.
...
Parantez içindeki miktar `δ = r + γ V(s') - V(s)` TD hatasıdır. MC'deki `G_t - V(s_t)`'nin çevrimiçi analogudur. Yakınsama, `α`'nin Robbins-Monro'yu (`Σ α = ∞`, `Σ α² < ∞`) karşılamasını ve tüm durumların sonsuz sıklıkta ziyaret edilmesini gerektirir.

**Q-öğrenme.** Kontrol için politika dışı bir tank avcısı yöntemi:

`Q(s, a) ← Q(s, a) + α [r + γ max_{a'} Q(s', a') - Q(s, a)]`

`max`, agent'nin gerçekte hangi eylemi gerçekleştirdiğine bakılmaksızın, `s'`'den itibaren *açgözlü* politikanın takip edileceğini varsayar. Bu ayrıştırma, Q-öğrenmenin `Q*` öğrenmesini sağlarken, agent ε-açgözlülük yoluyla keşfediyor. Mnih ve ark. (2015) bunu Atari'de derin Q-öğrenmeye dönüştürdü (Ders 05).

**SARSA.** Politikaya bağlı bir tank avcısı yöntemi:

`Q(s, a) ← Q(s, a) + α [r + γ Q(s', a') - Q(s, a)]`

Ad, `(s, a, r, s', a')` tanımlama grubudur. SARSA, açgözlü `argmax`'yi değil, agent'nin *aslında* gerçekleştirdiği `a'` eylemini kullanıyor. ε-açgözlü `π`'nin çalıştığı her şey için `Q^π`'ye yakınsar, `ε → 0` limitinde `Q*` olur.

**Uçurumda yürüme farkı.** Klasik uçurumda yürüme görevinde (uçurumdan düşme = ödül -100), Q-öğrenme uçurumun kenarı boyunca en uygun yolu öğrenir ancak bazen keşif sırasında cezayı alır. SARSA, uçurumdan bir adım uzakta daha güvenli bir yol öğreniyor çünkü keşif gürültüsünü Q değerine dahil ediyor. Eğitimle her ikisi de `ε → 0`'de ideal seviyeye ulaşır. Pratikte bu önemlidir: deployment'de keşif gerçekten yapılırken, SARSA'nın davranışı daha muhafazakardır.

**Beklenen SARSA.** `Q(s', a')`'yi `π` altındaki beklenen değerle değiştirin:

`Q(s, a) ← Q(s, a) + α [r + γ Σ_{a'} π(a'|s') Q(s', a') - Q(s, a)]`

SARSA'dan daha düşük sapma (`a'` örneği yok), aynı politika hedefi. Genellikle modern ders kitaplarında varsayılandır.

**n-adım TD ve TD(λ).** Önyükleme öncesinde `n` adımlarını bekleyerek TD(0) ve MC arasında enterpolasyon yapın. `n=1` TD'dir, `n=∞` MC'dir. `(1-λ)λ^{n-1}` geometrik ağırlıklarıyla tüm `n` üzerinden TD(λ) ortalamaları. Çoğu derin RL, 3 ile 20 arasında `n` kullanır.
...
100 bölüm başına ortalama getiriyi izleyin. Q-öğrenme, basit deterministik GridWorld'de daha hızlı birleşir; SARSA uçurum yürüyüşü konusunda daha muhafazakar. `code/main.py`'deki 4×4 GridWorld'de, `α=0.1, ε=0.1` ile ~2.000 bölümden sonra her ikisi de neredeyse ideale yakın.

### 4. Adım: DP gerçeğiyle karşılaştırın

`Q*`'yi elde etmek için değer yinelemesini (Ders 02) çalıştırın. `max_{s,a} |Q_learned(s,a) - Q*(s,a)|`'yi kontrol edin. Sağlıklı bir tablolu TD agent, 10.000 bölümün ardından 4×4 GridWorld'deki `~0.5`'ye iniyor.

## Tuzaklar

- **İlk Q değerleri önemlidir.** İyimser başlangıç (negatif ödüllü bir görev için `Q = 0`) keşfetmeyi teşvik eder. Kötümser başlangıç, açgözlü bir politikayı sonsuza kadar tuzağa düşürebilir.
- **α programı.** Sabit `α`, durağan olmayan problemler için uygundur. `α_n = 1/n`'nin bozulması teoride yakınsama sağlar ancak pratikte çok yavaştır — `α`'yi `[0.05, 0.3]`'ye sabitleyin ve öğrenme eğrisini izleyin.
- **ε programı.** Yüksekten başlayın (`ε=1.0`), `ε=0.05`'ye bozun. "GLIE" (sonsuz keşifle sınırda açgözlü) yakınsama koşuludur.
- **Q-öğrenmede maksimum eğilim.** `Q` gürültülü olduğunda `max` operatörü yukarı yönde eğilim gösterir. Fazla tahmine yol açar — Hasselt'in Çift Q-öğrenmesi (Ders 05'te DDQN tarafından kullanılmıştır) bunu iki Q tablosuyla düzeltir.
...
2. **Orta.** Bir uçurumda yürüme ortamı oluşturun (4×12, son sıra -100 ödülü olan uçurumdur ve başlangıç ​​için sıfırlayın). Q-öğrenme ve SARSA nihai politikalarını karşılaştırın. Her birinin izlediği yolların ekran görüntüsünü alın. Hangisi uçuruma daha yakın?
3. **Zor.** Çift Q-öğrenmeyi uygulayın. Gürültülü bir ödül GridWorld'de (adım başına ödüle Gauss gürültüsü σ=5 eklenmiştir), Q-öğrenmenin `V*(0,0)`'yi anlamlı bir miktarda fazla tahmin ettiğini, Çift Q-öğrenmenin ise bunu yapmadığını gösterin.
...
| Q-öğrenme | "Politika dışı RL 101" | Sonraki durum eylemlerine ilişkin `max` ile tank avcısı güncellemesi; Davranış politikasından bağımsız olarak `Q*`'yi öğrenir. |
| SARSA | "Politikaya uygun Q-öğrenme" | Gerçek bir sonraki eylemi kullanarak tank avcısı güncellemesi; mevcut ε-açgözlü π için `Q^π`'yi öğrenir. |
...
| Maksimumlaştırma önyargısı | "Q-öğrenme fazla tahmin ediyor" | Gürültülü tahminlerin üzerindeki `max` yukarı yönlü eğilimde; Double Q-öğrenme ile düzeltildi. |

## Daha Fazla Okuma

- [Watkins ve Dayan (1992). Q-öğrenme](https://link.springer.com/article/10.1007/BF00992698) — orijinal kağıt ve yakınsama kanıtı.
