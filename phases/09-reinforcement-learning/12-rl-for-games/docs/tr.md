...
Oyunlar RL'nin istediği her şeye sahiptir. Temiz ödül (kazanç/mağlubiyet). Sonsuz bölümler (kendi kendine oynatma sıfırlanır). Mükemmel simülasyon (oyun *simülatördür*). Ayrık veya küçük sürekli eylem alanları. Rakip sağlamlığı zorlayan çoklu-agent yapısı.
...
- Monte Carlo Ağaç Arama (MCTS): Her harekette olası devamların bulunduğu ağacı genişletin. Önceki + önyükleme olarak `(p, v)` kullanın. Düğümleri UCB'ye (PUCT) göre seçin: `a* = argmax Q(s, a) + c · p(a|s) · √N(s) / (1 + N(s, a))`.
- Kendi kendine oynama: agent-vs-agent oyunları oyna. `t` hamlesinde, MCTS ziyaret dağıtımı `π_t` politika eğitim hedefi haline gelir.
- Kayıp: `L = (v - z)² - π · log p + c · ||θ||²`. `z` oyunun sonucudur (+1 / 0 / -1).
...
- `f(s_latent)`: politika önceki + değerini tahmin edin.
...
- Politika: LLM. Eylemler: token'lar. Durum: prompt + şimdiye kadarki yanıt.
- Eleştiri yok (PPO tarzı V_φ). Bunun yerine, her prompt için politikadan `G` tamamlamayı örnekleyin. Her biri için ödülü hesaplayın. REINFORCE tarzı güncelleme sinyali olarak **grup-göreceli avantajı** `A_i = (r_i - mean_r) / std_r` kullanın.
- KL cezasının sürüklenmeyi önleme politikasına referans olması (RLHF gibi).
- Tam kayıp:

`L_GRPO(θ) = -E_{q, {o_i}} [ (1/G) Σ_i A_i · log π_θ(o_i | q) ] + β · KL(π_θ || π_ref)`

Ödül modeli yok, eleştiri yok, MCTS yok. Gruba bağlı temel, üçünün de yerini alır. İşlemin çok küçük bir kısmında benchmark'ların akıl yürütmesinde PPO-RLHF kalitesiyle eşleşir veya onu aşar.

**R1 tarifinin tamamı.** DeepSeek-R1 (DeepSeek 2025) tek bir makalede iki modeldir:

- **R1-Zero.** DeepSeek-V3 temel modelinden başlayın. SFT yok. GRPO'yu doğrudan iki ödül bileşeniyle uygulayın: *doğruluk ödülü* (kural tabanlı - son yanıt doğru sayıya ayrıştırıldı mı / kod birim testleri geçti mi) ve *biçim ödülü* (tamamlama, düşünce zincirini `<think>…</think>` etiketlerine sardı mı). Binlerce adımdan sonra, ortalama yanıt uzunluğu ~100'den ~10.000 token'lara çıkar ve matematik benchmark puanları o1'e yakın önizleme seviyelerine yükselir. Model, akıl yürütmeyi sıfırdan öğrenir. Dezavantajı ise düşünce zincirlerinin çoğunlukla okunamaması, karışık diller olması ve stilistik ciladan yoksun olmasıdır.
...
3. **Reddetme örneklemesi + 2. tur SFT.** RL kontrol noktasından ~600 bin akıl yürütme yörüngesi örneği alın, yalnızca doğru nihai yanıtlara ve okunabilir CoT'ye sahip olanları tutun ve ~200 bin akıl yürütme olmayan SFT örnekleriyle (yazma, QA, öz biliş) birleştirin. Tabana tekrar ince ayar yapın.
...
**Akıl yürütme için neden PPO yerine GRPO.** DeepSeekMath makalesindeki (Şubat 2024) üç neden: (1) eğitilecek değer ağının olmaması, belleğin yarıya indirilmesi; (2) grup temel çizgisi, muhakeme görevlerinin ürettiği seyrek yörünge sonu ödülünü doğal olarak ele alır; (3) per-prompt normalleştirmesi, PPO'nun tek eleştirmeninin yapamadığı, son derece farklı zorluktaki problemler arasında avantajları karşılaştırılabilir hale getirir.
...
- *LLM gerekçesi*: henüz üretimde MCTS yok; Tam kullanıma sunmada GRPO, inference işlem için N'nin en iyisi. Süreç ödül modelleri (PRM'ler), adım düzeyinde aramanın yeniden ekleneceğine işaret ediyor.

## Build It — Kendin İnşa Et

`code/main.py`'daki kod, birden fazla örnek grubuna sahip bir haydut olan **GRPO'yu minyatür** uygular. Algoritma, LLM'deki ile aynıdır; yalnızca politika ve ortam daha basittir. 2025 yeniliği olan *kaybı* ve *gruba göre avantajı* öğretir.
...
### Adım 2: politika: softmax bölü K yanıtı token'lar başına prompt
...
prompt ile şartlandırılmış bir LLM'nin son katman çıktısına eşdeğerdir.

### 3. Adım: grup örneklemesi ve gruba göre avantaj
...
- **Grup boyutu çok küçük.** Grup temel çizgisinin varyansı `1/√G` gibi gider. `G = 4`'nin altında avantaj sinyali gürültülüdür; standart seçim `G = 8` ile `64` arasındadır.
...
- **Tamamen kendi kendine oynama döngüleri.** AlphaZero tarzı eğitim, genel toplamlı oyunlarda hakimiyet döngülerine takılıp kalabilir. Farklı rakip havuzları nedeniyle hafifletildi (lig oyunu, Ders 10).
...
| LLM matematik/kod muhakemesi | GRPO (DeepSeek-R1, Qwen-RL, açık kopyalar) |
| LLM hizalama | DPO / RLHF-PPO (GRPO değil; doğrulayıcı, doğrulanamaz tercihtir) |
...
1. **Kolay.** `code/main.py`'da GRPO haydutunu uygulayın. Her biri 2 prompt'lar × 4 yanıt token'lar üzerinde eğitim alın. `G=8` ile 1.000'den az güncellemede birleşin.
...
3. **Zor.** 2 uzunluktaki "akıl yürütme zincirine" uzatın: agent iki token yayar ve doğrulayıcı çifti ödüllendirir. GRPO'nun iki adımlı dizilerde kredi atamasını nasıl gerçekleştirdiğini ölçün. (İpucu: *tam dizi* başına grup avantajını hesaplayın, her iki token konumuna da dağıtın.)

## Anahtar Terimler

| Terim | İnsanlar ne diyor | Aslında ne anlama geliyor |
|------|-----------------|-----------------------|
| MCTS | "Öğrenilen ağ ile ağaç arama" | Monte Carlo Ağacı Arama; Öğrenilen `(p, v)` öncelikleri ile UCB1/PUCT seçimi. |
...
| Kendi kendine oynama | "Agent vs geçmiş benlik" | Sıfır toplamlı standart; simetrik eğitim sinyali. |
| Lig maçı | "Nüfusa dayalı kendi kendine oynama" | Geçmiş + şimdiki + sömürücüler rakip olarak örneklendi. |
...
- [Gümüş ve ark. (2017). İnsan bilgisi olmadan Go oyununda ustalaşmak (AlphaGo Zero)](https://www.nature.com/articles/nature24270).
- [Gümüş ve ark. (2018). Satranç, shogi ve Go'yu kendi kendine oynama (AlphaZero)](https://www.science.org/doi/10.1126/science.aar6404) konusunda uzmanlaşan genel bir pekiştirmeli öğrenme algoritması.
- [Schrittwieser ve ark. (2020). Öğrenilmiş bir modelle (MuZero)](https://www.nature.com/articles/s41586-020-03051-4) planlama yaparak Atari, Go, satranç ve shogi'de ustalaşmak.
...
- [DeepSeek-AI (2025). DeepSeek-R1: LLM'lerde Pekiştirmeli Öğrenme Yoluyla Muhakeme Yeteneğinin Teşvik Edilmesi](https://arxiv.org/abs/2501.12948) — tam dört aşamalı R1 tarifi artı R1-Sıfır ablasyon.
...
- [Hugging Face TRL — GRPOTrainer](https://huggingface.co/docs/trl/main/en/grpo_trainer) — GRPO'yu özel ödül işlevleriyle uygulamak için üretim referansı.
...
- [Sutton ve Barto (2018). Ch. 17 — Pekiştirmeli Öğrenmenin Sınırları](http://incompleteideas.net/book/RLbook2020.pdf) — R1'in LLM ölçeğinde örneklediği kendi kendine oyun, arama ve "tasarlanmış ödül" için ders kitabı çerçevesi.
