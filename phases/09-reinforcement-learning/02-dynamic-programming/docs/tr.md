...
> Dinamik programlama hileli RL'dir. Geçiş ve ödül işlevlerini zaten biliyorsunuz; Bellman denklemini `V` veya `π` hareketi durana kadar yinelersiniz. Her örnekleme tabanlı yöntemin yaklaşmaya çalıştığı şey benchmark'dir.
...
Yakınsama garantilidir çünkü (a) her iyileştirme adımı `π`'yi aynı tutar veya bazı durumlar için `V^π`'yi kesin olarak artırır, (b) deterministik politikaların alanı sınırlıdır. Büyük durum uzaylarında bile genellikle ~5–20 dış yinelemede yakınsar.

**Değer yinelemesi.** Değerlendirme ve iyileştirmeyi tek bir taramaya daraltır. Bellman *optimallik* denklemini uygulayın:

`V(s) ← max_a Σ_{s',r} P(s',r|s,a) [r + γ V(s')]`

`max_s |V_{new}(s) - V(s)| < ε`'ye kadar tekrarlayın. Açgözlü eylemi gerçekleştirerek sonunda politikayı çıkarın. Yineleme başına kesinlikle daha hızlıdır (iç değerlendirme döngüsü yoktur), ancak genellikle yakınsama için daha fazla yineleme gerekir.
...
**`γ < 1` neden önemlidir?** Bellman operatörü, `||T V - T V'||_∞ ≤ γ ||V - V'||_∞` üst normunda bir `γ` daralmasıdır. Daralma benzersiz bir sabit nokta ve geometrik yakınsama anlamına gelir. `γ < 1`'yi bırakırsanız garantiyi kaybedersiniz; sınırlı bir ufka veya emici bir terminal durumuna ihtiyacınız vardır.
...
Ders 01'deki aynı 4×4 GridWorld'ü kullanın. Stokastik bir değişken ekliyoruz: `0.1` olasılığıyla agent rastgele bir dik yöne kayar.
...
`transitions(s, a)`, `(s', r, p)`'nin bir listesini döndürür. Bu modelin tamamı.

### Adım 2: politika değerlendirmesi

Bir `π(s) = {action: prob}` politikası verildiğinde, `V` hareketi durana kadar Bellman denklemini yineleyin:
...
`π`'yi açgözlü politika w.r.t ile değiştirin. `V`. `π` değişmediyse geri dönün; optimum noktadayız.
...
4×4: 4–6 dış yinelemede tipik yakınsama. `V*(0,0) ≈ -6`'yi ve adım sayısını kesin olarak azaltan bir politikayı çıktılar.
...
- **Sup-norm ve L2 yakınsaması.** Ortalama değil, `max |V_new - V|` kullanın. Teorik garanti üst normdadır.
...
- **Politika bağları.** İki eylemin Q değeri eşitse, `argmax` her yinelemede bağları farklı şekilde bozabilir ve "politika kararlı" denetiminin salınmasına neden olabilir. Sabit bir eşitlik bozma kullanın (sabit sırayla ilk eylem).
- **Durum uzayı patlaması.** DP, tarama başına `O(|S| · |A|)`'dir. ~10⁷ duruma kadar çalışır. Bunun ötesinde, fonksiyon yaklaşımına ihtiyacınız vardır (Aşama 9 · 05'ten itibaren).
...
| Çevrimdışı RL (CQL, IQL) | Muhafazakar Q-yineleme — DP, OOD eylemlerine ceza verdi |

Ne zaman birisi "en uygun değer fonksiyonu" dese, "DP sabit noktası"nı kastediyor demektir. Bir kağıtta `V*` veya `Q*`'yi gördüğünüzde bu döngüyü gözünüzde canlandırın.
...
1. **Kolay.** `γ ∈ {0.9, 0.99}` ile 4×4 GridWorld'de değer yinelemesini çalıştırın. `max |ΔV| < 1e-6`'ye kadar kaç tarama var? `V*`'yi 4×4 ızgara olarak yazdırın.
...
3. **Zor.** Değiştirilmiş politika yinelemesi oluşturun: değerlendirme adımında, yakınsama yerine yalnızca `k` taramalarını çalıştırın. `k ∈ {1, 2, 5, 10, 50}` için `V*(0,0)` hatası ile `k` grafiğini çizin. Eğri size değerlendirme/iyileştirme dengesi hakkında ne söylüyor?

## Anahtar Terimler

| Dönem | İnsanlar ne diyor | Aslında ne anlama geliyor |
|------|-----------------|-----------------------|
| Politika yinelemesi | "DP algoritması" | Politikanın değişmesi durana kadar alternatif değerlendirme (`V^π`) ve iyileştirme (açgözlü `π` w.r.t. `V^π`). |
| Değer yinelemesi | "Daha Hızlı DP" | Tek taramada uygulanan Bellman optimallik yedeklemesi; `V*`'ye geometrik olarak yakınsar. |
| Bellman operatörü | "Yineleme" | `(T V)(s) = max_a Σ P (r + γ V(s'))`; üst normda bir `γ` daralması. |
| Kasılma | "DP neden yakınlaşıyor?" | `\|\|T x - T y\|\| ≤ γ \|\|x - y\|\|`'ye sahip herhangi bir `T` operatörünün benzersiz bir sabit noktası vardır. |
...
- [Bertsekas (2019). Takviyeli Öğrenme ve Optimal Kontrol](http://www.athenasc.com/rlbook.html) — kasılma haritalama argümanlarının titizlikle ele alınması.
...
- [Bertsekas ve Tsitsiklis (1996). Nöro-Dinamik Programlama](http://www.athenasc.com/ndpbook.html) — sonraki her derste kullanılan DP'den yaklaşık DP / derin RL'ye köprü.
