...
> Markov Karar Süreci beş şeyden oluşur: durumlar, eylemler, geçişler, ödüller, indirim. RL'deki her şey (Q-öğrenme, PPO, DPO, GRPO) bu şekil üzerinde optimize edilir. Bir kez öğrenin, takviyeli öğrenimin geri kalanını ücretsiz okuyun.
...
Bir satranç botu yazıyorsunuz. Veya bir envanter planlayıcısı. Veya bir ticaret agent. Veya bir akıl yürütme modelini eğiten PPO döngüsü. Dört farklı alan, şaşırtıcı bir gerçek: dördü de aynı matematiksel nesneye çöküyor.

Denetimli öğrenme size `(x, y)` çiftleri verir ve bir işleve uymanızı ister. Takviyeli öğrenme size hiçbir etiket vermez; yalnızca bir durum akışı, gerçekleştirdiğiniz eylemler ve skaler bir ödül verir. Bu hamle oyunu kazandırdı mı? Stok yenileme kararı tasarruf sağladı mı? Ticaret kâr etti mi? LLM'nin az önce ürettiği token jüriden daha yüksek bir ödül almasına yol açtı mı?
...
- **Devletler** `S`. agent'nin karar vermesi gereken her şey. GridWorld'de hücre. Satrançta tahta. Bir LLM'de context window artı herhangi bir bellek.
- **Eylemler** `A`. Seçimler. Yukarı/aşağı/sola/sağa hareket ettirin. Bir hamle oyna. Bir token yayınlayın.
- **Geçişler** `P(s' | s, a)`. `s` durumu ve `a` eylemi verildiğinde, bir sonraki duruma göre dağılım. Satrançta deterministik, envanterde stokastik, LLM kod çözmede neredeyse deterministik.
- **Ödüller** `R(s, a, s')`. Skaler sinyal. Galibiyet = +1, mağlubiyet = -1. Gelir eksi maliyet. GRPO'daki log-olabilirlik oranı terimi.
- **İndirim** `γ ∈ [0, 1)`. Gelecekteki ödülün şimdiki zamana göre ne kadar önemli olduğu. `γ = 0.99` ~100 adımlık bir ufuk satın alır; `γ = 0.9` ~10 adet satın alır.
...
**Politikalar ve dönüşler.** `π(a | s)` politikası, durumları eylem dağılımlarıyla eşleştirir. Geri dönüş `G_t = r_t + γ r_{t+1} + γ² r_{t+2} + …` gelecekteki ödüllerin indirimli toplamıdır. `V^π(s) = E[G_t | s_t = s]` değeri, `π` politikası kapsamında `s`'den başlayan beklenen getiridir. Q değeri `Q^π(s, a) = E[G_t | s_t = s, a_t = a]`, belirli bir eylemle başlayan beklenen getiridir. Her RL algoritması bu ikisinden birini tahmin eder ve ardından `π`'yi buna göre geliştirir.
...
Rastgele politikayı 1000 kez çalıştırın. Bu 4×4 board için ortalama getiri -60 ila -80 civarındadır. Optimum getiri -6'dır (sağdan aşağıya düz çizgi). Bu açığı kapatmak 9. Aşamadaki her şeydir.
...
Etkili ufuk kabaca `1 / (1 - γ)`'dir. `γ = 0.9` → 10 adım. `γ = 0.99` → 100 adım. `γ = 0.999` → 1000 adım.

Çok düşükse agent miyop gibi davranır. Çok yüksekse kredi tahsisi gürültülü hale gelir, çünkü ilk adımların çoğu uzak gelecekteki ödülün sorumluluğunu paylaşır. Bölümler kısa ve sınırlı olduğundan LLM RLHF genellikle `γ = 1` kullanır. Kontrol görevleri `0.95–0.99`'yi kullanır. Uzun ufuklu strateji oyunları `0.999` kullanır.
...
- **Ödüllerin az olması.** Yalnızca kazanılan ödüller, geniş devlet alanlarında öğrenmeyi neredeyse imkansız hale getirir. Taklit yoluyla ödülleri (ara sinyal) veya önyüklemeyi şekillendirin (Aşama 9 · 09).
- **Ödül hackleme.** Proxy ödülünü optimize etmek çoğu zaman patolojik davranışlara neden olur. OpenAI'nin tekne yarışı agent, yarışı bitirmek yerine sürekli olarak güçlendiriciler toplayarak daireler çiziyordu. Ödülü her zaman temsili sonuca göre değil, hedef sonuca göre tanımlayın.
- **Yanlış spesifikasyon indirimi** Sonsuz ufuktaki bir görevde `γ = 1`, her değeri sonsuz kılar. Daima sonlu bir ufukla veya `γ < 1` ile sınırlayın.
- **Ödül ölçeği.** {+100, -100} ile {+1, -1} arasındaki ödüller aynı optimal politikaları sağlar ancak çok farklı gradient büyüklükleri sağlar. PPO/DQN'ye bağlanmadan önce `[-1, 1]`-ish'e normalleştirin.

## Kullan onu

2026 yığını, koda dokunmadan önce her RL ardışık düzenini bir MDP'ye indirir:

| Durum | Devlet | Eylem | Ödül | γ |
|-----------|-------|--------|--------|---|
...
| Yüksek Lisans için RLHF | Bağlam token'ler | Sonraki token | Sonunda ödül modeli puanı | 1.0 (bölüm ~200 token) |
...
1. **Kolay.** `code/main.py`'de 4×4 GridWorld'ü ve rastgele politika dağıtımını uygulayın. 10.000 bölüm çalıştırın. Ortalamayı ve getiri std'sini rapor edin. Optimum getiriyle (-6) karşılaştırın.
2. **Orta.** Tek tip rastgele ilkesi için `policy_evaluation`'yi `γ ∈ {0.5, 0.9, 0.99}` ile çalıştırın. `V`'yi her biri için 4×4 ızgara olarak yazdırın. Daha büyük `γ` ile terminal yakınındaki durum değerlerinin neden daha hızlı büyüdüğünü açıklayın.
...
| Devlet | "agent'nin gördüğü şey" | Seçilen politika sınıfı kapsamında gelecekteki dinamikler için yeterli istatistik. |
| Politika | "Agent'nin davranışı" | Koşullu dağıtım `π(a \| s)` veya deterministik harita `s → a`. |
| Geri Dön | "Toplam ödül" | Geçerli adımdan indirimli toplam `Σ γ^t r_t`. |
| Değer | "Devlet ne kadar iyidir" | `s`'den başlayarak `π` kapsamında beklenen getiri. |
| Q değeri | "Bir eylem ne kadar iyidir" | İlk eylem `a` ile `s`'den başlayarak `π` kapsamında beklenen getiri. |
...
- [Sutton ve Barto (2018). Takviyeli Öğrenme: Giriş, 2. baskı](http://incompleteideas.net/book/RLbook2020.pdf) — ders kitabı. Ch. 3, MDP'leri ve Bellman denklemlerini kapsar; Ch. 1, sonraki her dersin altında yatan ödül hipotezini motive eder.
...
- [OpenAI Dönüyor — Bölüm 1: Temel Kavramlar](https://spinningup.openai.com/en/latest/spinningup/rl_intro.html) — derin bir RL açısından kısa MDP primeri.
