...
- [mlabonne LLM Scientist -- Supervised Fine-Tuning](https://github.com/mlabonne/llm-course#4-supervised-fine-tuning) -- the roadmap section for SFT: training techniques (full FT, LoRA, QLoRA), TRL/Unsloth/Axolotl, distributed training with DeepSpeed/FSDP, and monitoring; pair this with the lesson for the frameworks view
- [mlabonne -- Fine-tune Llama 3.1 with Unsloth (blog + notebook)](https://huggingface.co/blog/mlabonne/sft-llama3) -- end-to-end SFT walkthrough with Unsloth on a free Colab, the reference implementation mlabonne recommends
- [Rasbt LLMs-from-scratch -- Chapter 7: Fine-tuning to Follow Instructions](https://github.com/rasbt/LLMs-from-scratch/tree/main/ch07) -- Instruction tuning implemented from scratch in PyTorch (no TRL, no PEFT); the best way to understand what SFT does under the hood before reaching for QLoRA
- [Niels Transformers-Tutorials -- Mistral SFT](https://github.com/NielsRogge/Transformers-Tutorials/tree/master/Mistral) -- `Supervised_fine_tuning_(SFT)_of_an_LLM_using_Hugging_Face_tooling.ipynb`: TRL `SFTTrainer` with PEFT LoRA on Mistral 7B, the minimal HF recipe
- [Rasbt LLMs-from-scratch -- DPO from scratch](https://github.com/rasbt/LLMs-from-scratch/tree/main/ch07/04_preference-tuning-with-dpo) -- the preference-tuning stage that comes after SFT, implemented without TRL for pedagogical clarity
