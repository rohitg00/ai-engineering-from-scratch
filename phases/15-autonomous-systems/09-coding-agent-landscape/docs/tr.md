...
Tamamlayıcı sorun, benchmark doygunluğunun regresyonları gizlemesidir. SWE-bench Verified doygunluğa yakındır ve kolay görev kuyruğu (≤2 satır gerektiren 500 görevden 161'i) en yüksek puanları yukarı çeker. Gerçek dünyadaki kalite, aynı liderlerin hala %23-59 arasında yer aldığı SWE-bench Pro (10'dan fazla satır değişikliği) gibi dağıtımlarda daha iyi ölçülür.

## Konsept

### SWE-bank, bir paragraf

SWE-bench (Jimenez ve diğerleri), temel gerçek yamalarla gerçek GitHub sorunlarını ele alır ve bir agent'dan test paketinin başarılı olmasını sağlayacak bir yama üretmesini ister. SWE-bench Verified (OpenAI, 2024), belirsiz ve hatalı görevlerin kaldırıldığı, insanlar tarafından seçilen 500 görevlik bir alt kümedir. SWE-bench Pro daha zorlu bir haleftir; mevcut sınır agent'larin %23-59 seviyesinde olduğu 10'dan fazla değişiklik satırı gerektiren görevler.

### 2022 → 2026 eğrisi aslında neyi gösteriyor?

- **2022**: ham SWE-bench'te ~%4 araştırma modelleri.
- **2024**: GPT-4 + Devin tarzı iskele ~%14; ~%12'de SWE-agent.
- **2025**: Aider içindeki Claude 3.5/3.7 Sonnet ve SWE-agent %40–55 aralığına doğru ilerliyor.
...
| Claude Kodu | Tescilli | İzin modları + rutinler | Ders 10 agent loop konusunu ayrıntılı olarak ele alıyor |
...
1. **Geri alma**: Okunacak doğru dosyaları bulmak sessiz bir darboğazdır. SWE-agent'ın ACI'sı, OpenHands'ın dosya dizini ve Aider'ın repo haritasının tümü buna saldırır.
...
OpenHands yazarları ve Epoch AI, SWE-bench Verified'ın kolay bir kuyruğu olduğunu belirtiyor: 500 görevden 161'i yalnızca 1-2 satırlık değişikliğe ihtiyaç duyuyor. Yüksek puanlar kısmen bu kuyruktan kaynaklanıyor. SWE-bench Pro, 10'dan fazla hat değişikliğiyle sınırlandırır ve sınır sistemleri için bile %23-59 aralığında puanlar döndürür. Prodüksiyon dağıtımınız neredeyse kesinlikle Doğrulanmış'tan çok Pro'ya daha yakın.

Bir agent seçmenin anlamı: kendi hata biriktirme listenizin Pro benzeri bir alt kümesini çalıştırın. Önemli olan puan, gönderdiğiniz şeyi temsil eden görevlerdeki puandır.

## Use It — Hazır Araçla Uygula

`code/main.py` , sabit bir mini görev dağılımındaki iki oyuncak agent iskelesini karşılaştırır:
...
`outputs/skill-scaffold-audit.md` , önerilen bir kodlama-agent iskelesini benimsemeden önce denetlemenize yardımcı olur: alma kalitesi, doğrulayıcı varlığı, korumalı alan yalıtımı ve benchmark-dağıtım uyumu.

## Egzersizler

1. `code/main.py`'yı çalıştırın. Her iskele aynı görev setinde kaç tur yapar? Her birinin eylem başına patlama yarıçapı nedir?
...
|---|---|---|
| SWE-bank | "Kodlama benchmark" | Temel gerçek yamalar ve test paketleriyle ilgili gerçek GitHub sorunları |
...
| ACI (Agent-Bilgisayar Arayüzü) | "SWE-agent'ın formatı" | İnsan kabukları için değil, LLM ergonomisi için tasarlanmış komut seti |
...
- [Epoch AI — SWE sıralaması skor tablosu](https://epoch.ai/benchmarks) — canlı takip edilen skorlar.
