# Uzun Süreli Arka Plan Agent'lar: Dayanıklı Yürütme

> Üretim uzun ufuklu agent'lar `while True`'da çalışmaz. Her LLM çağrısı, kontrol noktası, yeniden deneme ve tekrar oynatma içeren bir etkinliğe dönüşür. Temporal'ın OpenAI AgentSDK entegrasyonu Mart 2026'da GA'ya geçti. Claude Kod Rutinleri (Antropik), kalıcı bir yerel süreç olmadan planlanmış Claude Kodu çağrılarını çalıştırır. Oturumlar insan girişi üzerine duraklatılır, dağıtımlardan sağ çıkar ve `thread_id` tarafından anahtarlanan en son kontrol noktasından devam ettirilir. Yeni ergonominin arkasında, yeni bir girdiyle eski bir model (iş akışı orkestrasyonu) yatıyor: LLM, iyileşme sırasında deterministik olarak yeniden oynatılması gereken, deterministik olmayan faaliyetler olarak çağrılıyor.
...
Dört saat boyunca çalışan bir agent düşünün. Üç aracı çağırır, promptkullanıcıyı iki kez arar ve kırk LLM çağrısı yapar. Yarı yolda, çalıştığı ana bilgisayar yeniden başlatılıyor. Ne oluyor?

- Saf bir `while True` döngüsünde: her şey kaybolur. Koşu sıfırdan yeniden başlar. Üç araç çağrısı (gerçek yan etkilerle birlikte) yeniden yürütülür. Kullanıcı zaten onaylamış olduğu şeyler için tekrar promptedilir. Kırk LLM çağrısı yeniden faturalandırılır.
...
Bu tam olarak etkinlik profilidir. Her LLM çağrısını bir etkinlik olarak sarmalamak, üstel geri çekilme ile yeniden deneme, yeniden başlatmalar arasında kontrol noktası belirleme ve hata ayıklama için tekrar oynatılabilir bir izleme olanağı sağlar.

### `thread_id` tarafından anahtarlanan kontrol noktaları

LangGraph, Microsoft Agent Framework, Cloudflare Dayanıklı Nesneler ve Claude Kod Rutinlerinin tümü aynı API şeklinde bir araya geldi: bir `thread_id` (veya eşdeğeri) oturumu tanımlar; her durum geçişi bir arka uca devam eder (varsayılan PostgreSQL, geliştirme için SQLite, önbellek için Redis); özgeçmiş en son kontrol noktasını okur.
...
- Doğruluğun bir context window içinde uçtan uca gerektirdiği görevler (bazı muhakeme görevleri; bazıları tek seferlik oluşturma).
...
`code/main.py` , stdlib Python'da minimum düzeyde dayanıklı yürütme motoru uygular. Şunları destekler:
...
Sürücü, üç etkinlikli bir iş akışını simüle eder, yarı yolda çöker ve (a) her şeyi yeniden yürüten saf bir yeniden denemeye karşı (b) yalnızca eksik etkinliği çalıştıran bir tekrar gösterir.

## Ship It — Kullanıma Sun

`outputs/skill-durable-execution-review.md` , önerilen uzun süreli agent deployment'yi doğru ve dayanıklı yürütme şekli açısından inceler: etkinlikler, determinizm, kontrol noktası arka ucu, insan girişi durumu ve devam ettirildiğinde HITL politikası.

## Egzersizler

1. `code/main.py`'yı çalıştırın. Saf yeniden deneme ve yeniden yürütme arasındaki etkinlik yürütme sayısındaki farkı gözlemleyin. Kilitlenme noktasını değiştirin ve tekrar sayısındaki değişiklikleri buna göre gösterin.
...
4. LangChain'in "Derin üretim agent'larin arkasındaki çalışma zamanı" yazısını okuyun. Çalışma zamanının devam ettiği her durumu listeleyin ve her birinin kapsadığı hata modunu adlandırın.
...
|---|---|---|
| İş Akışı | "Agent'ın betiği" | Deterministik düzenleme kodu; olay günlüğünden tekrar oynatılabilir |
...
- [Microsoft — Agent Framework: döngüdeki insan ve kontrol noktası oluşturma](https://learn.microsoft.com/en-us/agent-framework/workflows/human-in-the-loop) — requestInfoEvent şekli.
...
- [OpenAI Agent'nin SDK'sı + Geçici entegrasyon (Trigger.dev duyurusu)](https://trigger.dev) — Yüksek Lisans çağrıları için etkinlik şekli.
- [Antropik — Uygulamada agent özerkliğin ölçülmesi](https://www.anthropic.com/research/measuring-agent-autonomy) — 35 dakikalık bozulma referansı.
