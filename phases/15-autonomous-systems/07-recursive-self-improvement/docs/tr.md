...
2024 yılına kadar olan RSI tartışması çoğunlukla felsefiydi. 2025-2026 değişimi somuttur. AlphaEvolve (Ders 3) gelişmiş algoritmalar. Darwin Gödel Makinesi (Ders 4) agent iskeleyi geliştirdi. Anthropic'in AAR'ı (Ders 6) hizalama araştırmasını geliştirdi. Her sistem bir döngüdeki bir adımdır ve döngünün kapanma koşulu açık bir araştırma sorusudur.

## Konsept

### Özyinelemeli kişisel gelişimin tam olarak anlamı nedir

Bir kişisel gelişim döngüsü: Verilen `S_n` sistemi, hedefte daha iyi puan alan `S_{n+1}` sistemini üretir. `S_{n+1}`, `S_{n+2}`'yi üreten düzenlemeyi önerdiğinde süreç yinelemelidir. Yetenek RSI: hedef görev performansıdır. Hizalama RSI: hedef hizalama kalitesidir.
...
Miles Brundage ve Jared Kaplan, RSI'yi "en büyük risk" olarak nitelendirdi. Çerçevelemeleri: yetenek, hizalamayı aşar çünkü yetenek net ölçülebilir hedeflere (benchmark'ler) sahipken uyumun belirsiz hedefleri vardır (değerler, ilkeler, niyet). Optimizasyon döngüleri keskin hedeflerde bulanık olanlardan daha iyidir.

### Yetenek ve hizalanma, bir yarış olarak

Paralel olarak birleşen iki süreci hayal edin. `r_c` oranındaki bileşiklerin yeteneği; `r_a` oranında hizalama. Yanlış hizalama boşluğu `M(t) = C(t) - A(t)`, `r_c > r_a` olduğunda büyür. Orandaki küçük farklılıklar zamanla büyük boşluklara neden olur.

Pratik soru: `r_a >= r_c`'yi bir RSI hattında yapabilir miyiz? Aday yaklaşımları:
...
RSI çalıştayı (özyinelemeli-workshop.github.io) somut örneklere odaklandı: değerlendirici tasarımı, koruma tasarımı, sınırlı iyileştirme kanıtları, döngüler arasındaki yetenek artışlarının izlenmesi. "RSI tehlikeli midir?" "RSI tarzı döngüler için korumaları nasıl tasarlarız" sorusu, RSI'nin en azından kısmen gönderildiğini gösteriyor.

Atölye özeti (openreview.net/pdf?id=OsPQ6zTQXV) dört güncel mühendislik açık problemini tanımlar:

1. Değerlendiricinin genellemesi (değerlendirme yine de `S_{n+10}`'de neyin önemli olduğunu ölçecek mi?).
...
`code/main.py` iki süreçli bir yarışı simüle eder: yetenek iyileştirme ve hizalama iyileştirme. Her döngü ayarlanabilir gürültü oranlarını uygular. Senaryo, büyüyen yanlış hizalama açığını ve varsayımsal bir güvenlik eşiğini tetikleyebilecek döngülerin payını izliyor.

## Gönderin

`outputs/skill-rsi-cycle-pause-spec.md`, bir RSI hattının duraklaması ve bir sonraki döngüden önce insan incelemesini beklemesi gereken koşulları belirtir.

## Egzersizler

1. `code/main.py --threshold 2.0`'yi çalıştırın. Yetenek oranı 1,15 ve hizalama oranı 1,08 (Senaryo A) ile, yanlış hizalama boşluğu `C - A` 2,0'ı geçene kadar kaç döngü var?
...
| Yetenek RSI | "Görev performansı bileşikleri" | Hedef benchmark puanı, genelleme veya ufuktur |
...
- [Yinelemeli Atölye sitesi](https://recursive-workshop.github.io/) — program ve belgeler.
- [Antropik — Uygulamada AI agent özerkliğinin ölçülmesi](https://www.anthropic.com/research/measuring-agent-autonomy) — hizalama sahteciliği bağlamını içerir.
...
- [DeepMind — Frontier Safety Framework v3](https://deepmind.google/blog/strengthening-our-frontier-safety-framework/) — yanıltıcı hizalama izleme.
