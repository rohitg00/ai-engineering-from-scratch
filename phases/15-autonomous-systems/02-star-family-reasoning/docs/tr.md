...
Çalışıyor. GSM8K ve CommonsenseQA'nın her ikisi de yeni insan açıklaması olmadan iyileştirildi. Ancak döngünün yerleşik bir önyargısı vardır: Akıl yürütmenin kendisinin sağlam olup olmadığına bakılmaksızın, doğru cevabı üreten herhangi bir mantık korunur. V-STaR (Hosseini ve diğerleri, 2024) bunu öğrenilmiş bir doğrulayıcıyla yamalar; Quiet-STaR (Zelikman ve diğerleri, 2024), fikri token'ye göre dahili rasyonellere genelleştirir.
...
Bildirilen delta: GSM8K ve MATH'de önceki kişisel gelişim temellerine göre yüzde +4 ila +17 puan; kazancın çoğu, ek jeneratör fine-tuning yerine inference zaman seçimi için doğrulayıcının kullanılmasından geliyor.

### Sessiz STaR: token başına dahili rasyoneller

Zelikman ve ark. (2024) şunu sordu: Ya model sadece problem ile cevap arasında değil, her token pozisyonunda kısa bir dahili mantık üretmeyi öğrenirse? Quiet-STaR, tahmin edilen her token'den önce gizli bir "düşünce" yaymak için bir modeli eğitir, ardından öğrenilen bir ağırlık aracılığıyla düşünce farkındalığına sahip tahmini temel tahminle karıştırır.

Sonuç: Mistral 7B, göreve özgü fine-tuning olmadan GSM8K'de %5,9'dan %10,9'a ve CommonsenseQA'da %36,3'ten %47,2'ye mutlak sıfır atış iyileştirmeleri elde etti. Model "ne zaman düşüneceğini" öğrendi; zor token'ler daha uzun dahili mantıklara sahip oluyor; kolay olanlar neredeyse hiç alamaz.
...
| Sessiz-STAR | token mantığına göre + karıştırma ağırlığı | 1,5-3x | minimum | hala cevap koşullu gradient |

### Bunun 2026 yığınında yeri nerede

STaR eskidir. Ancak bu model 2025-2026'da her yerde yeniden ortaya çıkıyor. Doğrulanabilir matematik problemlerindeki RL (DeepSeek-R1, Kimi-k1.5, o1), STaR'ın büyütülmüş cevap koşullu gradient sinyalidir. Süreç ödül modelleri (Lightman ve diğerleri, 2023; OpenAI'nin "Adım adım doğrulayalım") süreç denetimli alternatifidir. AlphaEvolve (Ders 3), kod için STaR'dir ve etiket yerine program değerlendiriciye sahiptir. Darwin Gödel Makinesi (Ders 4), agent iskelesinin kendisi için STaR'dir.
...
`code/main.py`, oyuncak aritmetik görevinde simüle edilmiş bir STaR döngüsü çalıştırır. Şunları izleyebilirsiniz:
...
- Bir doğrulayıcının (V-STaR stili) inference'de nasıl yardımcı olduğu ancak eğitim sırasında tanıtılan kısayolları tam olarak budayamadığı.

## Gönderin

`outputs/skill-star-loop-reviewer.md`, önerilen kendi kendine öğretilen muhakeme hattı üzerinde eğitim almadan önce onu denetlemenize yardımcı olur.
...
3. Quiet-STaR makalesini okuyun (arXiv:2403.09629) Bölüm 3. "Düşüncenin sonu" token'yi ve karıştırma ağırlığı başlığını üç cümleyle açıklayın.
...
| Rasyonalizasyon | "İpucu yeniden deneme" | Doğru cevabı enjekte edin ve temel modelin başarısız olduğu sorunların gerekçesini bulmak için yeniden prompt |
| V-STAR | "Doğrulayıcı STaR" | Doğrulayıcıyı hem doğru hem de yanlış gerekçelerle DPO eğitin, inference zaman seçimi için kullanın |
| Sessiz-STAR | "token başına gerekçeler" | Her token konumunda gizli düşünceler oluşturun; temel tahminle karıştırın |
| Yanıt koşullu gradient | "Sonuca dayalı sinyal" | Eğitim döngüsü, akıl yürütme adımlarını değil, nihai yanıtları ödüllendirir |
...
- [Zelikman ve ark. (2022). STaR: Akıl Yürütmeyle Önyükleme Muhakeme](https://arxiv.org/abs/2203.14465) — orijinal makale.
- [Hosseini ve ark. (2024). V-STaR: Kendi Kendini Öğretenler için Eğitim Doğrulayıcıları](https://arxiv.org/abs/2402.06457) — inference zaman seçimi için bir DPO doğrulayıcı ekler.
- [Zelikman ve ark. (2024). Quiet-STaR: Dil Modelleri Kendilerine Konuşmadan Önce Düşünmeyi Öğretebilir](https://arxiv.org/abs/2403.09629) — token dahili gerekçelerine göre.
- [Lightman ve ark. (2023). Adım Adım Doğrulayalım](https://arxiv.org/abs/2305.20050) — ödül modellerini işleyin, alternatif gradient sinyali.
