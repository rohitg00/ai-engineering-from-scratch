...
1. **Fikir üretme.** Yüksek Lisans, bir konu ve önceki literatüre dayalı araştırma fikirleri önerir. v1 kullanılan şablonlar; v2, bir hipotez alanı üzerinde agentic aramayı kullanır.
...
3. **Deneme planı.** agent deneysel bir protokol taslağı hazırlar ve kod yazar.
...
Önemli bağlam: Çalıştay bildirileri ana konferans bildirilerine göre daha düşük bir çıtaya sahiptir. Akran değerlendirmesi gürültülüdür; herhangi bir günde başvuruların küçük bir kısmı kabul edilir. Bir başarı, bir güvenilirlik iddiası değil, kavramın bir kanıtıdır. Nature 2026 makalesi uçtan uca döngüyü belgeliyor ve kendisi de insan araştırmacıların ortak yazarı; "sistem bir Nature makalesi yazdı" değildir.
...
v2, üçü arasında en zayıf otomatik değerlendiriciye, en geniş çıktı yüzeyine ve genel artifact'lare giden en kısa yola sahiptir. Operasyonel kontroller (korumalı alan, inceleme, açıklama) güvenlik çalışmalarının çoğunu gerçekleştiriyor.
...
2. Varsayılanlar zaten Beel ve diğerlerinin %42 / %25'ini kullanıyor. `--experiment-failure 0.20 --novelty-mislabel 0.10` ile ve ardından `--experiment-failure 0.60 --novelty-mislabel 0.40` ile yeniden çalıştırın. Gösterişli ama kusurlu pay iki çalışma arasında nasıl değişiyor?
...
5. Araştırma çıktıları için agent "doktora sahibi her makaleyi okur" ifadesinden daha iyi ölçeklenen bir insan inceleme protokolü önerin. Darboğazı tanımlayın ve etrafındaki tasarımı yapın.
...
| Yapay Zeka Bilimcisi v2 | "Şablonsuz araştırma agent" | AgentVLM şekil eleştirisi ile ic ağaç araması |
| Agentic ağaç araması | "Araştırmanın dallara ayrılması agent" | Birden fazla deneme planını paralel olarak genişletir; iç eleştirmenden kuru erik |
...
| Korumalı alandan kaçış | "LLM kodu çıkıyor" | Agent-yürütülen kod, döngü tasarımcısının amaçlamadığı şeyleri yapar |

## Daha Fazla Okuma

- [Yamada ve diğerleri. (2025). AI Scientist-v2](https://arxiv.org/abs/2504.08066) — makale.
...
- [Antropik — Yapay Zeka agent özerkliğinin ölçülmesi](https://www.anthropic.com/research/measuring-agent-autonomy) — açık uçlu araştırmaların agent daha geniş çerçevesi.
