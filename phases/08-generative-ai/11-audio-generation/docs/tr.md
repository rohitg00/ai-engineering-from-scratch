...
> Ses, 16-48 kHz'de 1 boyutlu bir sinyaldir. Beş saniyelik bir klip 80-240 bin örnektir. Hiçbir transformer bu diziye doğrudan katılmaz. 2026'da her prodüksiyon ses modeli için çözüm aynıdır: sinirsel bir codec bileşeni (Encodec, SoundStream, DAC), sesi 50-75 Hz'de ayrı token'lere sıkıştırır ve bir transformer veya difüzyon modeli, token'ler oluşturur.
...
1. **Metinden konuşmaya.** Verilen metin, konuşmayı üretir. Temiz konuşma dar bantlıdır ve güçlü bir fonetik yapıya sahiptir; transformer-over-token'ler tarafından iyi bir şekilde çözülmüştür. VALL-E (Microsoft), NaturalSpeech 3, ElevenLabs, OpenAI TTS.
...
3. **Ses efektleri / ses tasarımı.** prompt verildiğinde ortam sesi veya Foley üretin. AudioGen, AudioLDM 2, Sabit Ses Açık.

Üçü de aynı alt tabaka üzerinde çalışır: sinirsel ses codec'i + token-AR veya yayılma üreteci.

## Konsept

![Ses üretimi: codec tokens + transformer veya dağıtım](../assets/audio-generation.svg)
...
**Token-otoregresif.** RVQ token'leri bir sıraya düzleştirin, yalnızca kod çözücü transformer çalıştırın. MusicGen, K kod kitabı akışlarını akış başına ofsetlerle paralel olarak yaymak için "gecikmeli paralel"i kullanır. VALL-E, prompt metninden + 3 saniyelik ses örneğinden konuşma token'ler oluşturur.
...
2024-2026 trendi: akış eşleştirme müzikte kazanıyor (daha hızlı inference, daha temiz örnekler), token-AR ise doğal olarak nedensel olduğundan ve iyi bir şekilde aktarıldığından konuşmaya hakim olmaya devam ediyor.
...
| Sabit Ses 2.5 | Müzik / SFX | Gizli seslerde DiT + akış eşleştirmesi | ~1 dakikalık klip için 10s |
| Suno v4 | Tüm şarkılar | Açıklanmadı; token-AR şüpheli | ~şarkı başına 30sn |
...
`code/main.py` temel fikri simüle eder: iki farklı "tarzdan" (tarz A için alternatif düşük ve yüksek token'ler, tarz B için monoton rampa) oluşturulan sentetik "ses token" dizileri üzerinde küçük bir sonraki token transformer'yi eğitin. Stil ve örnek durumu.
...
### Adım 2: küçük bir token tahmincisini eğitin
...
token stili ve başlangıç token verildiğinde, tahmin edilen dağıtımdan sonraki token'yi örnekleyin. 20-40 token boyunca devam edin.

## Tuzaklar

- **Codec kalitesi çıktı kalitesini sınırlar.** Codec bir sesi aslına sadık bir şekilde temsil edemiyorsa, jeneratör kalitesinin hiçbir faydası olmaz. DAC şu anki açık en iyisidir.
...
- **Müzik yapısı.** 30 saniyelik token, 75 Hz'de 20k+ token'dir. transformer'ler için zor. MusicGen kayan pencere + prompt devamını kullanır; Stabil Ses, daha kısa klipler + çapraz geçiş kullanır.
- **Sınırlarda Artifact'ler.** Oluşturulan klipler arasındaki çapraz soldurma dikkatli bir şekilde üst üste bindirme eklemeyi gerektirir.
...
- **Ses klonlama etiği.** VALL-E / XTTS / ElevenLabs'ın sesi klonlaması için 3 saniyelik bir örnek ve prompt metni yeterlidir. Her üretim modelinin kötüye kullanım tespiti + devre dışı bırakma listelerine ihtiyacı vardır.
...
| Açık ağırlıklı müzik araştırması | MusicGen 3.3B, Sabit Ses Açık 1.0, AudioLDM 2 |
| Dublaj / çeviri | HeyGen, ElevenLabs Dublaj |

## Gönderin

`outputs/skill-audio-brief.md`'yi kaydedin. Skill bir sesli özet alır (görev, süre, stil, ses, lisans) ve çıktılar: model + barındırma, prompt formatı (tür etiketleri, stil tanımlayıcıları, yapısal işaretleyiciler), codec + jeneratör + ses kodlayıcı zinciri, tohum protokolü ve değerlendirme planı (MOS / CLAP puanı / TTS için CER / kullanıcı A/B).

## Egzersizler

1. **Kolay.** `code/main.py`'yi çalıştırın ve stili açıkça ayarlayın. Oluşturulan dizilerin stilin deseniyle eşleştiğini doğrulayın.
2. **Orta.** Gecikmeli paralel kod çözme ekleyin: 1 adımla dengede kalması gereken 2 token akışını simüle edin. Ortak bir tahminciyi eğitin.
3. **Zor.** MusicGen-small'ı yerel olarak çalıştırmak için HuggingFace transformer'leri kullanın. Üç farklı prompt ile 10 saniyelik bir klip oluşturun; Stil uyumu için A/B.

## Anahtar Terimler

| Dönem | İnsanlar ne diyor | Aslında ne anlama geliyor |
|------|-----------------|-----------------------|
| Kodlayıcı | "Sinir sıkışması" | Ses için kodlayıcı / kod çözücü; tipik çıkış 50-75 Hz token'dir. |
...
| Gecikmeli paralel | "Ofset kod kitapları" | Sıra uzunluğunu azaltmak için kademeli ofsetlerle K token akışı yayınlayın. |
...
- [Défossez ve ark. (2022). Kodlama: Yüksek Hassasiyette Nöral Ses Sıkıştırma](https://arxiv.org/abs/2210.13438) — codec standardı.
...
- [Kararlılık Yapay Zekası (2024). Stabil Ses 2.5](https://stability.ai/news/introducing-stable-audio-2-5) — Akış eşleştirmeyle 2025 metinden müziğe.
