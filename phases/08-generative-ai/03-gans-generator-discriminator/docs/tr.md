...
Goodfellow'un fikri: Gerçek görüntüleri sahte olanlardan ayırt etmek için bir sınıflandırıcıyı `D(x)` eğitin. `D`'yi kandırmak için bir `G(z)` generatorünü eğit. `G` için kayıp sinyali, `D`'nin şu anda bir şeyin gerçek gibi görünmesini sağladığını düşündüğü şeydir. Bu sinyal, `G` hareket eden bir hedefi kovalayarak geliştikçe güncellenir. Her iki ağ da birleşirse, `G`, `log p(x)`'yi hiç yazmadan veri dağıtımını öğrenmiştir.
...
2026'da GAN'lar artık SOTA oluşturucusu olmayacak (difüzyon ve akış eşleştirme bu tacı yedi). Ancak StyleGAN 2/3 şimdiye kadar gönderilen en keskin yüzlü modeller olmaya devam ediyor, GAN discriminatorları difüzyon eğitiminde *algısal kayıplar* olarak kullanılıyor ve çekişmeli eğitim, gerçek zamanlı difüzyon göndermenize olanak tanıyan hızlı 1 adımlı damıtmaları (SDXL-Turbo, SD3-Turbo, LCM) güçlendiriyor.

## Konsept

![GAN eğitimi: minimaks'ta oluşturucu ve ayırıcı](../assets/gan.svg)

**Generator `G(z)`.** Bir gürültü vektörünü `z ~ N(0, I)` bir örnek `x̂` ile eşler. Kod çözücü şeklinde bir ağ (yoğun veya aktarılmış dönüşüm).

**Ayırıcı `D(x)`.** Bir örneği skaler bir olasılıkla (veya puanla) eşler. Gerçek → 1, sahte → 0.

**Kayıp.** İki alternatif güncelleme:

- **`D`yi eğitin:** `loss_D = -[ log D(x) + log(1 - D(G(z))) ]`. Gerçek=1, sahte=0'da ikili çapraz entropi.
- **`G`yi eğitin:** `loss_G = -log D(G(z))`. Bu, Goodfellow'un kullandığı *doymayan* biçimdir (orijinal `log(1 - D(G(z)))`, `D` kendinden emin olduğunda gradient'ları doyurur ve öldürür.

**Eğitim döngüsü.** Bir adım `D`, bir adım `G`. Tekrarlamak.

**Neden işe yarıyor.** Eğer `G`, `p_data` ile mükemmel bir şekilde eşleşiyorsa, o zaman `D` şanstan daha iyisini yapamaz ve her yerde 0,5 çıktı verir; `G` artık gradient almıyor. Denge.

**Neden bozulur?** Modun çökmesi (`G`, `D`'nin sınıflandıramadığı bir modu bulur ve onu sonsuza kadar basar), gradient'nin ortadan kaybolması (`D` çok hızlı öğrenir ve `log D` doyurulur), eğitim istikrarsızlığı (öğrenme oranları, grup boyutları, herhangi bir şey).
...
| 2017 | WGAN, WGAN-GP | BCE'yi Wasserstein mesafesi + gradient ceza ile değiştirin. gradient'nin kaybolması düzeltildi. |
| 2017 | Spektral normalizasyon | Ayrımcıyı Lipschitz bağladı. 2026'daki discriminatorlarda hala kullanılıyor. |
...
`code/main.py` küçük bir GAN'ı 1 boyutlu veriler üzerinde eğitir: iki Gauss'un karışımı. Generator ve ayırıcı tek gizli katmanlı MLP'lerdir. İleri, geri ve minimax döngüsünü elle uyguluyoruz. Amaç, iki temel arıza modunu (mod çökmesi + yok ​​olma gradient) gerçekleştiği anda görmektir.

### Adım 1: Doymayan kayıp

D, G'nin sahtesini yüksek güvenle sahte olarak sınıflandırdığında vanilya Goodfellow kaybı `log(1 - D(G(z)))` 0'a gider. Bu noktada G için gradient temelde sıfırdır — G gelişemez. Doymamış `-log D(G(z))` biçiminin zıt asimptotu vardır: D kendinden emin olduğunda patlar ve G'ye güçlü bir sinyal verir.
...
### Adım 2: generator adımı başına bir ayırıcı adım
...
G için yeni sahteler, aksi halde gradient'lar bayattır.
...
- **Generator bir modu ezberler.** D girişlerine gürültü ekleyin, bir mini parti ayırıcı katmanı kullanın veya WGAN-GP'ye geçin.
...
| Anime / stilize yüzler | StilGAN-XL veya Stable Diffusion LoRA |
...
GAN'lar keskin ama dardır. Alanınız açıldığında (fotoğraflar, rastgele metinler prompt'ler, video) dağıtıma geçin. Düşmanlık hilesi, bağımsız bir generator olarak değil, bir bileşen (algısal kayıplar, damıtma) olarak varlığını sürdürüyor.

## Gönderin

`outputs/skill-gan-debugger.md`'yi kaydet. Skill, başarısız bir GAN çalışmasını (kayıp eğrileri, örnek ızgara, dataset boyutu) alır ve olası nedenlerin sıralı bir listesini, tek satırlık düzeltmeleri ve yeniden çalıştırma protokolünü çıkarır.
...
2. **Orta.** Goodfellow BCE kaybını WGAN kaybıyla değiştirin: `loss_D = E[D(fake)] - E[D(real)]`, `loss_G = -E[D(fake)]` ve D'nin ağırlıklarını `[-0.01, 0.01]` olarak kırpın. Eğitim daha istikrarlı mı? Duvar saati yakınsamasını karşılaştırın.
3. **Zor.** 1 boyutlu örneği 2 boyutlu verilere (bir halkadaki 8 Gauss'un karışımı) genişletin. Generatorün 1k, 5k, 10k adımlarında 8 moddan kaç tanesini yakaladığını takip edin. Mini parti ayrımını uygulayın ve yeniden ölçün.

## Anahtar Terimler

| Dönem | İnsanlar ne diyor | Aslında ne anlama geliyor |
|------|-----------------|-----------------------|
| Generator | "G" | Gürültüden örnekleme ağı, `G: z → x̂`. |
| Ayırıcı | "D" | Sınıflandırıcı `D: x → [0, 1]`, gerçek ve sahte. |
| Minimaks | "Oyun" | Ortak bir hedefin `min_G max_D`. |
| Doymamış kayıp | "Düzeltme" | G için `log(1 - D(G(z)))` yerine `-log D(G(z))` kullanın. |
| Mod daralt | "G bir şeyi ezberledi" | Generator, çeşitli verilere rağmen az sayıda farklı çıktı üretir. |
| WGAN | "Wasserstein" | BCE'yi Toprak Taşıyıcı mesafesi + gradient cezası ile değiştirin; daha yumuşak gradient. |
...
GAN'lar artık açık alan oluşturmada örnek kalitesinden kazanmıyor ancak yine de inference maliyetten kazanıyorlar. Üretim-inference literatüründe bir GAN'ın sahip olduğu kelime dağarcığı:
...
- [Goodfellow ve ark. (2014). Üretken Çekişmeli Ağlar](https://arxiv.org/abs/1406.2661) — orijinal GAN ​​makalesi.
