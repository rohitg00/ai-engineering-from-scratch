...
> ReAct düşünce ve eylemi tek bir akışta birleştirir. ReWOO onları ayırıyor: Önce büyük bir plan, sonra uygulama. HotpotQA'da 5 kat daha az tokens, +%4 doğruluk ve planlayıcıyı bir 7B modeline ayırabilirsiniz. Planla ve Uygula bunu genelleştirdi; Planla ve Harekete Geç bunu web navigasyonuna ölçeklendirdi.
...
- ReWOO'nun Planlayıcı / Çalışan / Çözücü ayrımının neden token'leri kurtardığını ve ReAct'in serpiştirilmiş döngüsü üzerindeki sağlamlığı geliştirdiğini açıklayın.
...
ReWOO (Xu ve diğerleri, arXiv:2305.18323, Mayıs 2023) bunu fark etti ve bir iddiaya girdi: Her şeyi önceden planlayın, paralel olarak kanıt toplayın, cevabı en sonunda yazın. Plan yapmak için bir LLM çağrısı, N aracı kanıt gerektirir (paralel olabilir), çözmek için bir LLM çağrısı. Çok daha iyi token verimliliği ve daha net arıza modları için ticaret daha az esnektir (plan statiktir).
...
Planlayıcı bir DAG üretir. Her düğüm bir aracı, onun bağımsız değişkenlerini ve hangi önceki düğümlere bağlı olduğunu adlandırır (`#E1`, `#E2` gibi referanslar). İşçiler düğümleri topolojik sıraya göre yürütürler. Çözücü her şeyi birbirine diker.

### Neden 5 kat daha az tokens

ReAct, adım sayısıyla birlikte doğrusal olarak prompt uzunluğunu artırır. 10. adımda, prompt, düşünce 1 artı eylem 1 artı gözlem 1 artı düşünce 2 artı eylem 2 artı gözlem 2 vb. içerir. Her ara adım ayrıca yedekli olarak orijinal prompt'yi içerir.

ReWOO bir planlayıcıya prompt (büyük), N küçük işçiye prompt (her biri yalnızca araç çağrısı, zincir yok) ve bir çözücüye prompt ödeme yapar. HotpotQA'da makale ~5 kat daha az tokens ölçerken +4 mutlak doğruluk puanı alıyor.
...
Makalenin ikinci sonucu: Planlayıcı gözlemleri görmediğinden, 175B öğretmeninin planlayıcı çıktılarına göre 7B modeline ince ayar yapabilirsiniz. Küçük model planlamayı yönetir; inference'da büyük modele ihtiyaç yoktur. Bu artık standart; çoğu 2026 yapımı agent küçük bir planlayıcı ve büyük bir uygulayıcı kullanıyor veya tam tersi.

### Planla ve Yürüt (2023)

LangChain ekibinin Ağustos 2023'teki gönderisi, ReWOO'yu bir kalıp adı olarak genelleştirdi: Planla ve Yürüt. Ön planlayıcı bir adım listesi yayınlar, yürütücü her adımı çalıştırır, isteğe bağlı bir yeniden planlayıcı sonuçları gözlemledikten sonra revize edebilir. Bu, ReWOO'dan ziyade ReAct'e daha yakındır (yeniden planlayıcı, gözlemleri planlamaya geri getirir) ancak token tasarrufunu korur.
...
| ReWOO | Bilinen araçlarla yapılandırılmış görevler, token-duyarlı, paralelleştirilebilir kanıtlar |
...
- `Planner` — prompt'dan bir plan DAG yayınlayan komut dosyasıyla yazılmış bir politika.
- `Worker` — her düğümün araç çağrısını kayıt defteri aracılığıyla gönderir.
- `Solver` — kanıtları okuyan ve nihai bir cevap üreten senaryolu kompozisyon.
...
3. `Planner`'yi küçük bir modelle (7B sınıfı) değiştirin ve `Solver`'yi bir sınır modelinde tutun. Uçtan uca kaliteyi karşılaştırın; bölünme nerede başarısız oluyor?
...
|------|----------------|------------------------|
| ReWOO | "Gözlem olmadan akıl yürütme" | Planlayın, ardından paralel olarak delil toplayın ve çözün — planlamada gözlem yok prompt |
...
| Token verimlilik | "Daha az gidiş dönüş" | Makalede HotpotQA'da ReAct'e kıyasla 5 kat daha az tokens |
...
- [Antropik, Etkili Oluşturma Agents](https://www.anthropic.com/research/building-effective-agents) — işe yarayan en basit modeli seçin
