...
**Önkoşullar:** Aşama 14 · 13 (LangGraph), Aşama 14 · 24 (Observability Platformlar)
**Süre:** ~60 dakika

## Öğrenme Hedefleri

- GenAI yayılma kategorilerini adlandırın: model/istemci, agent, araç.
- `invoke_agent` İSTEMCİ ve DAHİLİ aralıkları ve her birinin ne zaman geçerli olduğunu ayırt edin.
...
Her satıcı kendi yayılma adlarını icat eder. Operasyon ekipleri sonunda framework başına kontrol panelleri oluşturuyor. OpenTelemetry'nin GenAI SIG'si, tüm ekosistemin hedeflediği tek bir standart tanımlayarak bu sorunu çözer.
...
3. **Araç kapsamları.** Araç çağrısı başına bir; agent aralığına ebeveyn-çocuk ilişkisi ile bağlanır.

### Agent aralık adlandırma

- Span adı: `invoke_agent {gen_ai.agent.name}` eğer adlandırılmışsa; `invoke_agent`'a geri dönüş.
- Açıklık türü:
- **İSTEMCİ** — uzak agent hizmetleri için (OpenAI Assistants API, Bedrock Agent'ler).
- **DAHİLİ** — süreç içi agent framework'ler (LangChain, CrewAI, yerel ReAct) için.
...
- `gen_ai.data_source.id` — RAG için: hangi derlem veya mağazaya danışıldı.
...
Önerilen üretim modeli: içeriği harici olarak saklayın (S3, günlük deponuz), aralıklara referansları kaydedin (işaretçi kimlikleri, düzyazı değil). Bu, observability'a bağlanan Ders 27 içerik zehirlenmesi savunmasıdır.
...
- **Açıklıklarda tam prompt'ların yakalanması.** Operasyonların okuyabileceği izlerdeki PII, sırlar, müşteri verileri. Harici olarak saklayın.
- **Hayır `gen_ai.provider.name`.** İlişkilendirme eksik olduğunda çoklu sağlayıcı kontrol panelleri bozulur.
- **Üst bağlantıların olmadığı kapsamlar.** Artık araç kapsamları. Her zaman bağlamı yayın.
- **Kararlılık etkinleştirmesi ayarlanmıyor.** Arka uç yükseltmesinde nitelikleriniz yeniden adlandırılabilir.
...
- `start_span` ile `Tracer`, iç içe geçmiş bağlamlar.
- Komut dosyası içeren bir agent çalıştırması şunu yayar: `create_agent`, `invoke_agent` (DAHİLİ), araç başına yayılmalar, LLM çağrıları için {`chat` yayılmalar.
- prompt'ları harici olarak depolayan ve aralıklara kimlikleri kaydeden bir içerik yakalama modu.
...
`outputs/skill-otel-genai.md` kabloları OTel GenAI, içerik yakalama varsayılanları ve harici referans depolamasıyla mevcut agent'ya yayılır.

## Egzersizler

1. Ders 01 ReAct döngünüzü `invoke_agent` (INTERNAL) + araç başına aralıklarla donatın. Bir Jaeger örneğine gönderin.
2. "Yalnızca referanslar" modunda içerik yakalamayı ekleyin: SQLite'a prompts, yayılma nitelikleri yalnızca satır kimliklerini taşır.
3. `gen_ai.data_source.id` spesifikasyonunu okuyun. Bunu Ders 09 Mem0 aramanıza bağlayın.
...
| invoke_agent | "Agent aralık" | agent çalıştırmayı temsil eden yayılmanın adı |
| MÜŞTERİ aralığı | "Uzaktan arama" | Uzak bir agent hizmetine yapılan çağrının kapsamı |
| İÇ açıklık | "İşlemde" | Süreç içi agent çalıştırmanın aralığı |
...
- [AutoGen v0.4 (Microsoft Research)](https://www.microsoft.com/en-us/research/articles/autogen-v0-4-reimagining-the-foundation-of-agentic-ai-for-scale-extensibility-and-robustness/) — OTel aralıkları yerleşiktir
