...
> Janelas de contexto são finitas. Conversas, documentos e traces de ferramenta não são. MemGPT (Packer et al., 2023) enquadra isso como memória virtual de SO — contexto principal é RAM, armazenamento externo é disco, o agente faz paging entre eles. Esse é o padrão que todo sistema de memória de 2026 herda.
...
- Descrever como o agente emite "interrupções" pra consultar ou modificar memória externa e como o resultado é costurado de volta no próximo prompt.
...
Janelas maiores ajudam mas não resolvem isso. O paper de 2025 do Mem0 mediu que baselines com janela de 128k ainda perdem fatos de longo horizonte que um agente com memória externa de janela de 4k pega.
...
| Kernel do SO | loop de controle do agente | ReAct loop com ferramentas de memória |

O agente roda um ReAct loop normal. Uma classe extra de ferramentas permite fazer page in/out de dados do contexto principal.
...
- `archival_memory_search(consulta, top_k)` — recupera do armazenamento externo.
- `conversation_search(consulta)` — escaneia turnos anteriores.
...
- **Envenenamento de memória.** Memória externa é texto recuperado. Se conteúdo controlado por atacante cair numa nota de memória, o agente re-ingere na próxima sessão. Esse é o ataque de Greshake et al. (Aula 27) reenunciado ao longo do tempo.
...
- Um agente programado que preenche archival com fatos e depois responde uma pergunta chamando `archival_memory_search`.
...
O trace mostra o agente escrevendo três fatos, preenchendo o contexto principal até o limite (forçando evição) e depois respondendo uma pergunta de follow-up recuperando do archival — reproduzindo o workflow do MemGPT sem qualquer LLM real.
...
3. Adicione campos de `citation` (session_id, turn_id, source_url) nas inserções de archival. Faça o agente citar fontes em toda resposta baseada em recuperação.
...
| Memory ferramenta | "API de memória" | Chamada de ferramenta que o agente emite pra ler/escrever memória externa |
