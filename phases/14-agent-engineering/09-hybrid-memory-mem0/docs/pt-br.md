...
- **Similaridade semântica** — "o que a gente discutiu sobre deriva de agent semana passada?" Vector ganha; KV e graph perdem.
...
2. Armazenamento KV retorna hits diretos com chave em (user_id, type, entity) derivados da query.
3. Armazenamento graph retorna subgrafo acessível a partir de entidades da query.
...
Pesos são ajustados por produto. `w_recency` mais alto pra agents de chat; `w_importance` mais alto pra agents de compliance; `w_relevance` mais alto pra agents de recuperação.
...
Esse é o comportamento de nível de compliance que o padrão de invalidação da Letta generaliza.
...
- **Construções customizadas** — quando você precisa controle exato sobre o extractor (compliance) ou pesos de fusão (agents de voz onde recenticidade domina).
...
2. Adicione uma consulta temporal: `search(query, as_of=timestamp)`. Retorne só registros válidos naquele momento ou antes. Qual armazenamento precisa de mais trabalho?
...
| Embedding drift | "Decomposição de recuperação" | Qualidade vector degrada conforme corpus cresça; re-embenda periodicamente |
