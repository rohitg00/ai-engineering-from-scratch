...
- Tek bir depolamanın (yalnızca vektör, yalnızca grafik, yalnızca KV) agent belleği için neden yetersiz olduğunu açıklayın.
...
- **Anlamsal benzerlik** — "Geçen hafta agent sürüklenmesi hakkında neyi tartıştık?" Vektör kazanır; KV ve grafik özledim.
...
Üretim agent'ler üçünü de tek oturumda yayınlar. Tek depolu bellek bunlardan ikisi için her zaman yanlıştır. Mem0'ın katkısı, üçünü de tek bir `add`/`search` yüzeyinin arkasına, onları birleştiren bir puanlama işleviyle kablolamaktır.

## Konsept

### Paralel olarak üç mağaza

`add(text, user_id, metadata)`'de Mem0 (arXiv:2504.19413, Nisan 2025):

1. Aday gerçekleri metinden çıkarın (LLM odaklı bir adım).
2. Anlamsal arama için her bir olguyu vektör deposuna (embedding) yazın.
...
`search(query, user_id)`'de:

1. Vektör deposu embedding kosinüs ile en üst k değerini döndürür.
...
Ağırlıklar ürüne göre ayarlanır. Sohbet için daha yüksek `w_recency` agent; agent uyumluluğu için daha yüksek `w_importance`; agent'leri almak için daha yüksek `w_relevance`.
...
Karşılaştırma temel çizgilerinin (tam bağlamlı 128k LLM, düz vektör deposu, düz KV) tümü 10'dan fazla puan kaybeder. Benchmark'ler tek başına seçimi haklı çıkarmaz - operasyonel şekil öyledir - ancak sayılar, füzyon tasarımının bir yuvarlama hatası olmadığını gösterir.

### Kapsam sınıflandırması

Mem0 belleği kapsama göre böler:

- **Kullanıcı belleği** — `user_id`'ye anahtarlanarak oturumlar boyunca kalıcı olur.
...
- **Embedding kayması.** İlk yüz sorguda tam olarak görünen vektör sonuçları, derleme büyüdükçe kötüleşir. En çok kullanılan N kayıtların periyodik olarak yeniden embedding'sini ekleyin.
...
- **Grafik patlaması.** Gürültülü bir çıkarıcı, mesaj başına 50 kenar ekler. Kap grafiği `add` çağrısı başına yazar; güven düzeyi düşük kenarları bırakın.

## İnşa Et

`code/main.py`, stdlib'de üç mağaza modelini uygular:

- `VectorStore` — bir embedding yedek parçası olarak saf token örtüşme benzerliği.
- `KVStore` — `(user_id, fact_type, entity)`'ye anahtarlanmış dikte.
- `GraphStore` — yazılan kenarlar (konu, ilişki, nesne, geçerli).
- `Mem0` — `add()`, `search()`, füzyon puanlama ve kapsam bilinçli erişim ile üst düzey cephe.
...
- **Özel yapılar** — çıkarıcı (uyumluluk) veya füzyon ağırlıkları (yeniliğin hakim olduğu ses agent'ler) üzerinde tam kontrole ihtiyaç duyduğunuzda.

## Gönderin

`outputs/skill-hybrid-memory.md`, bir füzyon puanlayıcı, kapsam taksonomisi ve kablolu geçici geçersiz kılma özelliğine sahip üç depolu bir bellek iskelesi oluşturur.

## Egzersizler

1. Oyuncak vektör benzerliğini gerçek bir embedding modeliyle değiştirin (cümle-transformer'ler, Ollama, OpenAI embedding'ler). Sentetik uzun bir konuşmada geri çağırma@10'u ölçün. Sıralama 1000 yazmanın üzerinde değişiyor mu?
...
5. Mem0 belgelerini okuyun (`docs.mem0.ai`). Oyuncağı `mem0` istemci çağrılarına taşıyın. Aynı 20 test sorgusunda alma kalitesini karşılaştırın.
