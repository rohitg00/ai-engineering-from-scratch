...
- Escrever um contrato de escopo que um agent lê no início da tarefa e um verificador lê no final.
- Especificar arquivos permitidos, arquivos proibidos, critérios de aceitação, plano de rollback e limites de aprovação.
...
Scope creep é o modo de falha mais submonitorado no trabalho de agent porque o agent narra cada passo de boa fé. A solução não é um prompt mais rigoroso. A solução é um contrato no disco que diz o que foi prometido e uma verificação que compara o resultado com a promessa.
...
| `allowed_files` | Globs que o agent pode escrever |
| `forbidden_files` | Globs que o agent não pode tocar nem por acidente |
...
O agent escreve um diff. O verificador lê o diff, os globs permitidos, os globs proibidos e a lista de comandos de aceitação que rodaram. Cada violação é uma constatação marcada que o verification gate pode recusar.
...
Um praticante rodando "specsmaxxing" (contratos de escopo em YAML antes de invocar o agent) relata que a taxa de rabbit-hole caiu de 52% pra 21% em três semanas sem mudar o agent. O contrato fez o trabalho, não o modelo. Três padrões garantem que o ganho persista.
...
- **Interrupções no LangGraph.** Uma violação de escopo dispara uma interrupção; o handler pergunta ao humano se o contrato precisa crescer ou se o agent precisa recuar.
...
- [Dev|Journal, Preventing AI Agent Configuration Drift with Agent Contract Testing](https://earezki.com/ai-news/2026-05-05-i-built-a-tiny-ci-tool-to-keep-ai-agent-configs-from-drifting-in-my-repo/) — modo `--strict` sem deps externas
- [Agentic Coding Is Not a Trap (production logs)](https://dev.to/jtorchia/agentic-coding-is-not-a-trap-i-answered-the-viral-hn-post-with-my-own-production-logs-33d9) — receitas do specsmaxxing: 52% → 21%
...
- [Augment Code, AI Spec Template](https://www.augmentcode.com/guides/ai-spec-template) — sistema de limites em três níveis (must/ask/never)
