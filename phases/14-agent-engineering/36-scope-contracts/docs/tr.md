...
> Model işin nerede bittiğini bilmiyor. Kapsam sözleşmesi, işin nerede başladığını, nerede bittiğini ve taşması halinde nasıl geri alınacağını belirten, görev başına bir dosyadır. Sözleşme, "kapsam dahilinde kalmayı" bir dilekten çeke dönüştürüyor.
...
- Bir agent'nin görev başlangıcında okuyacağı ve bir doğrulayıcının görev sonunda okuyacağı bir kapsam sözleşmesi yazın.
...
Agents sürüngen. Görev "giriş hatasını düzeltmek"tir. Fark, oturum açma yoluna, e-posta yardımcısına, veritabanı sürücüsüne, README'ye ve sürüm komut dosyasına dokunur. Her dokunuşun o anda makul bir nedeni vardı. Hepsi birlikte, incelenenden farklı bir değişikliktir.

Kapsam kayması, agent işinde en az izlenen başarısızlık modudur çünkü agent her adımı iyi niyetle anlatır. Düzeltme daha katı bir prompt değil. Düzeltme, diskte neyin vaat edildiğini belirten bir sözleşme ve sonucu sözle karşılaştıran bir kontroldür.
...
Kapsam sözleşmesi bir görevi sınırlar. Projeyi bağlamaz. Bir agent, giriş düzeltmesi için bir sözleşmenin içinde mükemmel bir şekilde kalabilir ve yine de bir sonraki aşamada projenin bir ayarlar sayfasına, karanlık mod geçişine ve yönlendiricinin yeniden yazılmasına da ihtiyacı olduğuna karar verebilir. Sözleşmede hiçbir zaman hangi işin proje kapsamında olduğu sorulmadı, yalnızca hangi dosyaların görev kapsamında olduğu sorulmuştu.

Bu ikinci yüksekliğin kendine ait bir ilkel değeri olması gerekir: a `feature_list.json` agent oturum başlangıcında okunur. Makine tarafından okunabilen, sıralı bir dosya olarak proje biriktirme listesidir. agent, `status`'si {`todo` olan tam olarak bir özelliği seçer, `id`'sini aktif kapsam sözleşmesine yazar ve aynı oturumda ikinci bir özelliği başlatması yasaktır. "Her seferinde bir özellik", prompt'da bir satır olmaktan çıkar, agent geçmişi rasyonelleştirebilir ve diskten okuduğu ve geçidin uyguladığı bir kontrol olan bir değer haline gelir.
...
| `features[].id` | Kapsam sözleşmesinin `task_id` puanı |
...
- Dokunulan dosyaların listesini ve çalıştırma komutlarının listesini bir `RunSummary`'ye dönüştüren bir fark ayrıştırıcı.
...
Çıktı: sözleşme, iki çalıştırma, çalıştırma başına kararlar ve kaydedilmiş bir `scope_report.json`.

## Vahşi doğada üretim modelleri

"specsmaxxing" (agent'yi çağırmadan önce YAML'de kapsam sözleşmeleri) yürüten bir uygulayıcı, agent değiştirilmeden tavşan deliği oranının üç hafta içinde %52'den %21'e düştüğünü bildirmektedir. İşi model değil sözleşme yaptı. Üç model kazancın kalıcı olmasını sağlar.

**Bütçe ihlali, ikili hatalar değil.** `agent-guardrails` (Claude Code, Cursor, Windsurf, Codex tarafından MCP aracılığıyla kullanılan OSS birleştirme kapısı) görev başına bir `violationBudget` gönderir: bütçe içindeki küçük kapsam kaymaları uyarı olarak ortaya çıkar; ancak bütçe aşıldığında birleştirme kapısı reddeder. {`violationSeverity: "error" | "warning"` ile eşleştirin. Bütçe, gönderilen bir kapı ile ondan nefret eden ekip tarafından devre dışı bırakılan bir kapı arasındaki farktır.

**Yol ailesine göre önem derecesi asimetrisi.** `docs/**`'ye kapsam dışı yazmalar genellikle `warn`; {`scripts/**`, `migrations/**`, `config/prod/**`'ye kapsam dışı yazmalar her zaman `block`'tır. Bu asimetrinin çalışma zamanında değil sözleşmede olması gerekir çünkü projeye özeldir ve göreve göre değişir.
...
**Çok sözleşmeli birleştirme semantiği (en az ayrıcalık).** İki kapsam sözleşmesi uygulandığında (e.g., proje çapında bir sözleşme artı göreve özel bir sözleşme), birleştirme şu şekildedir: **kesişen** `allowed_files` (her iki sözleşme de yola izin vermelidir), **birleşim** {`forbidden_files` (her ikisi de yasaklayabilir), `time_budget_minutes` en kısıtlayıcıdır (min), `approvals_required` birikir. `network_egress`, yaptırım uygulanmaması için `None`, tümünü reddetme için `[]`, izin verilenler listesi olarak `[...]`; birleştirme altında, `None` diğer tarafa erteler, iki liste kesişir ve hepsini reddet, hepsini reddet olarak kalır. Birleştirmenin mekanik ve gözden geçirilebilir olması için bunu sözleşme şemasında belirtin.

## Kullan onu

Üretim modelleri:

- **Claude Kodu eğik çizgi komutları.** Bir `/scope` komutu sözleşmeyi yazar ve onu oturum bağlamı olarak sabitler. Altagentlar harekete geçmeden önce sözleşmeyi okurlar.
- **GitHub PR'ler.** Sözleşmeyi PR gövdesinde bir JSON dosyası olarak veya teslim edilmiş bir artifact olarak aktarın. CI kapsam denetleyicisini birleştirme farkına karşı çalıştırır.
- **LangGraph kesintiye uğrar.** Kapsam ihlali bir kesintiyi tetikler; işleyici insana sözleşmenin büyümesi mi gerektiğini yoksa agent'nin geri adım atması mı gerektiğini sorar.

Sözleşme görevle birlikte seyahat eder. Görev kapatıldığında sözleşme `outputs/scope/closed/` altında arşivlenir.

## Gönderin

`outputs/skill-scope-contract.md`, bir görev açıklaması için bir kapsam sözleşmesi ve her agent farkta CI'da çalışan, dünyaya duyarlı bir denetleyici oluşturur.
...
2. Testi `docs/**` üzerinde yumuşak ve `scripts/**` üzerinde sert başarısız olacak şekilde genişletin. Asimetriyi gerekçelendirin.
...
- [Dev|Journal, Agent Sözleşme Testi ile Yapay Zeka Agent Konfigürasyon Kaymasını Önleme](https://earezki.com/ai-news/2026-05-05-i-built-a-tiny-ci-tool-to-keep-ai-agent-configs-from-drifting-in-my-repo/) — harici depolar olmadan `--strict` modu
- [Agentic Kodlama Bir Tuzak Değildir (üretim günlükleri)](https://dev.to/jtorchia/agentic-coding-is-not-a-trap-i-answered-the-viral-hn-post-with-my-own-production-logs-33d9) — specsmaxxing makbuzları: %52 → %21
- [OpenCode izin globları](https://opencode.ai/docs/agents/) — ayrıntılı izin başına kapsam
- [Knostic, AI Coding Agent Güvenlik: Tehdit Modelleri ve Koruma Stratejileri](https://www.knostic.ai/blog/ai-coding-agent-security) — en az ayrıcalığın parçası olarak kapsam
...
- Aşama 14 · 27 — Dürbün kilitleriyle eşleşen prompt enjeksiyon savunması
