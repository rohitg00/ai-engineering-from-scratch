...
> 2026'da üç üretim bilgisayar kullanım modeli. Üçü de vizyon tabanlıdır. Üçü de ekran görüntülerini, DOM metnini ve araç çıktılarını güvenilmeyen girdi olarak ele alır. Yalnızca doğrudan kullanıcı talimatları izin olarak sayılır. Adım başına güvenlik hizmetleri normdur.
...
- Üç modelin benchmark numaralarını OSWorld / WebArena / Online-Mind2Web'de adlandırın.
...
Masaüstü ve web agent'lerin ekranı ve sürücü girişini görmesi gerekir. Son 18 ayda üç satıcı ürünleri sevk etti. Her biri gecikme, kapsam ve güvenlik konusunda farklı tavizler verdi. Seçmeden önce üçünü de öğrenin.
...
- Uygulama üç parça gerektirir: bir agent loop, `computer` aracı (modele yerleştirilmiş şema, geliştirici tarafından yapılandırılamaz), bir sanal ekran (Linux'ta Xvfb).
...
- Geliştirici API'si: Responses API aracılığıyla `computer-use-preview-2025-03-11`.
...
- **Hassas işlemlere ilişkin onay yok.** Döngüde insan olmadan oturum açma, satın alma ve dosya silme sorumluluk taşır.
...
`code/main.py`, agent loop vizyonunu simüle eder:

- Piksel koordinatlarında etiketli öğelere sahip bir `Screen`.
...
`outputs/skill-computer-use-safety.md`, herhangi bir bilgisayar kullanımına yönelik agent için adım başına bir güvenlik sınıflandırıcı + doğrulama kapısı iskelesi oluşturur.
...
2. İzin verilen URL'ler listesiyle bir "gezinme" eylemi uygulayın. agent bir yönlendirmeyi izlemeye çalışırsa ne bozulur?
3. `sensitive=True` etiketli eylemler için bir onay kapısı ekleyin. Reddedilen her onayı günlüğe kaydedin.
...
| Sanal ekran | "Xvfb" | agent için ekranları işlemek için kullanılan başsız X sunucusu |
