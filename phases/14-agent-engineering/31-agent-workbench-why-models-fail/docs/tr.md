...
> Yetenekli bir model yeterli değildir. Güvenilir agent'lerin bir çalışma tezgahına ihtiyacı vardır: talimatlar, durum, kapsam, geri bildirim, doğrulama, inceleme ve aktarma. Bunları ortadan kaldırın ve bir sınır modeli bile nakliyesi güvenli olmayan işler üretir.
...
- agent'nin gönderilip gönderilmeyeceğine karar veren yedi tezgah yüzeyini adlandırın.
...
Bir sınır modelini gerçek bir depoya bırakırsınız ve ondan giriş doğrulaması eklemesini istersiniz. Dört dosyayı açar, makul kod yazar, başarıyı bildirir ve durur. Testleri siz yapın. İki tanesi başarısız oldu. Doğrulamayla hiçbir ilgisi olmayan üçüncü bir dosyaya dokunuldu. agent'nin neyi varsaydığına, ilk olarak neyi denediğine veya ne yapması gerektiğine dair bir kayıt yok.
...
Bu bir model hatası değil. Bu bir tezgah hatasıdır. agent'nin etrafındaki yüzey, tek seferlik üretimi güvenilir, devam ettirilebilir mühendisliğe dönüştüren parçalardan yoksundur.
...
|---------|-----------------|----------------------|
| Talimatlar | Başlatma kuralları, yasak eylemler, yapılanın tanımı | Agent nakliyenin ne anlama geldiğini tahmin ediyor |
...
| Geribildirim | Döngüye yakalanan gerçek komut çıktısı | Agent, 400 |
...
### Tezgah ve prompt mühendisliği

Prompting modele bu turda ne istediğinizi söyler. Bir çalışma tezgahı, modele sıralar ve oturumlar arasında işin nasıl yapılacağını anlatır. agent başarısızlık öykülerinin çoğu, prompt mühendislik kıyafetleri giyen tezgah arızalarıdır.

### Tezgah ve framework

framework size bir çalışma zamanı sağlar (LangGraph, AutoGen, Agents SDK). Bir çalışma tezgahı, agent'ye bu çalışma süresi içinde çalışabileceği bir yer sağlar. İkisine de ihtiyacın var. Bu mini parça ikinciyle ilgili.
...
agent etiketini bir anlığına çıkarın. agent çalıştırması zamanı, süreçleri ve makineleri aşan bir hesaplamadır. Bunu güvenilir kılmak için herhangi bir üretim sisteminin ihtiyaç duyduğu aynı temel öğelere ihtiyacınız vardır.

| İlkel | Nedir | agent için neler taşır |
|-----------|------------|------------------------------|
...
| Tetikleyici | Bir işlevi çağıran olay kaynağı | Agent loop onay işareti, HTTP isteği, kuyruk mesajı, cron, dosya değişikliği, kanca |
...
- **İnceleme** — oluşturucu artifact'lerde salt okunur yetkilendirmeye ve inceleme raporlarında salt yazma yetkilendirmesine sahip ayrı bir çalışan.
...
|------------------------------|--------------------|
| Ralph Loop (Claude Code, Codex, agentic_harness kitabı) — agent erken durmaya çalıştığında orijinal amacı yeni bir context window'ye yeniden enjekte edin | Bir görevi temiz bir bağlamla yeniden sıraya koyan bir tetikleyici; oturum ısrarı hedefi ileriye taşır |
...
| Donanım-hesaplama ayrımı (OpenAI Agents SDK, Nisan 2026) — yürütme düzleminden bölünmüş kontrol düzlemi | Kontrol düzlemini / veri düzlemini yeniden ayarlama. agent etiketinden onlarca yıl öncesine dayanıyor |
| Agent Pasaportunu açın (OAP, Mart 2026) — her araç çağrısını yürütmeden önce bildirime dayalı bir politikaya göre imzalayın ve denetleyin | İmzalı bir denetim kuyruğuna sahip, eylem öncesi çalışan tarafından uygulanan bir yetkilendirme politikası |
| Kılavuzlar ve Sensörler (Birgitta Böckeler / Düşünce Çalışmaları) — ileri besleme kuralları + geri bildirim observability | Yetkilendirme politikası + doğrulama işlevleri + observability izleri |
...
| Kancalar / ara katman yazılımı (LangChain, Claude Code) — model ve araç çağrılarını engelleme | Tetikleyiciler + işlevler, çalışma zamanının çağırma yolunun etrafına sarıldı |
...
| Sandbox agent'ler (Codex, Sandcastle, Vercel Sandbox) | Hesaplama düzlemi: yalıtılmış dosya sistemi, ağ ve yaşam döngüsüne sahip bir çalışma zamanı |
| MCP sunucuları | Yetkilendirme olarak yetenek listeleriyle, kararlı bir RPC üzerinden işlevleri kullanıma sunan çalışanlar |

Bu tablodaki her giriş, agent topluluğunun, dağıtılmış sistemlerde zaten bir adı olan bir ilkele ulaşması ve ona yeni bir ad vermesidir. Pazarlama için faydalı etiketler; mühendislik sözlüğü olarak kullanışlı değil.
...
- Terminal Bench 2.0 — aynı model, kablo demeti değişikliği, agent kodlamasını ilk 30'un dışından beşinci sıraya taşıdı (LangChain, *Agent Kablo Demeti Anatomisi*).
- Vercel - agent araçlarının %80'ini sildi; başarı oranı %80'den %100'e yükseldi (MongoDB).
- Harvey — yasal agent'nin doğruluğu yalnızca kablo demeti optimizasyonu (MongoDB) sayesinde iki kattan fazla arttı.
- Kurumsal yapay zeka agent projelerinin %88'i üretime ulaşamıyor. Hatalar, mantık yürütmeye değil çalışma zamanına göre kümeleniyor (preprints.org, *Harness Engineering for Language Agents*, Mart 2026).
- Üç popüler açık kaynaklı framework üzerinde 2025 yılında yapılan bir benchmark çalışması, görevin ~%50 oranında tamamlandığını bildirdi; uzun bağlamlı WebAgent, uzun bağlam koşullarında çoğunlukla sonsuz döngüler ve hedef kaybı nedeniyle %40-50'den %10'un altına düştü (2026'nın başındaki yazılarda geniş çapta ele alındı).

Paket servisi "koşum takımı sonsuza kadar kazanır" değildir. Modeller zamanla koşum hilelerini absorbe eder. Çıkarılan sonuç, bugün yük taşıma mühendisliğinin modelin içinde değil, etrafında olduğu ve bu yükü taşıyan ilkellerin, her üretim sisteminin her zaman ihtiyaç duyduğu şeyler olduğudur.

### Satıcı yazmalarının kısa sürede bittiği yer

Bu, kibar olmanıza gerek olmayan kısımdır.

- LangChain'in *Agent Donanımının Anatomisi* on bir bileşeni sıralar: prompt'ler, araçlar, kancalar, sanal alanlar, orkestrasyon, bellek, beceriler, altagent'ler ve çalışma zamanı "aptal döngüsü." Kuyrukları, çalışanları bir deployment birimi olarak adlandırmaz, tetikleme semantiğini, ayrı bir sorun olarak oturum kalıcılığını veya yetkilendirme politikasını belirtmez. Kablo demetini konuşlandırdığınız bir sistem olarak değil, yapılandırdığınız bir nesne olarak ele alır.
- Addy Osmani'nin *Agent Kayış Takımı Mühendisliği*, `Agent = Model + Harness` çerçevesini ve mandal modelini yerleştiriyor, ancak koşumun neyden yapıldığını söylemeden duruyor. Bu bir spesifikasyon olarak değil, bir duruş olarak okunur.
...
- agentic_harness kitabı, emniyet kemerini bir yapılandırma nesnesi olarak ele alır (Jaymin West'in *Agentic Mühendisliği*, bölüm 6) ve buradaki en güçlü cümle "kablo demeti, bir agentic sistemindeki birincil güvenlik sınırıdır." Bu sadece yeniden ifade edilen yetkilendirme politikasıdır.
- Hacker News konuları aynı yere gelmeye devam ediyor. Nisan 2026 tarihli başlık *agent koşum takımı sandbox'ın dışına aittir*, koşum takımının "daha çok her şeyin dışında duran ve bağlama ve kullanıcıya dayalı olarak erişime izin veren bir hipervizör gibi" oturması gerektiğini savunuyor. Yani yine ayrı bir düzlemde yetkilendirme politikası.

Boşluğu fark etmek için bu parçalardan herhangi birine katılmamanıza gerek yok. Zaten var olan bir sistemin UX açıklamalarını yazıyorlar. Sistemi yazıyoruz. Sistem doğru kurulduğunda yedi yüzey ilkellerin dışına çıkar. Yanlış yapıldığında hiçbir `AGENTS.md` cilası eksik kuyruğu düzeltmez.

Yani başka bir yerde "koşum mühendisliği" kelimesini duyduğunuzda, bunu ilkel dillere çevirin. Prompt'ler ve kurallar politika ve işlevlerdir. İskele çalışma zamanıdır. Korkuluklar yetkilendirme + doğrulamadır. Kancalar tetikleyicilerdir. Bellek oturumun kalıcılığıdır. Ralph Loop yeniden kuyrukta. Subagent'ler işçilerdir. Korumalı alanlar hesaplama düzlemleridir. Kelime dağarcığı değişir; mühendislik öyle değil. Çalışma tezgahı, agent'ye bakan UX'tir; Bir sonraki satıcı yeniden çerçevesinde hayatta kalan anlamında koşum takımı, işlevler, çalışanlar, tetikleyiciler, çalışma zamanları, kuyruklar, kalıcılık ve politikanın doğru bir şekilde bir araya getirilmesinden oluşur.

## İnşa Et

`code/main.py` küçük bir repo görevini iki kez çalıştırır. İlk önce yalnızca prompt olarak, ardından yedi yüzey kablolu olarak. Aynı model, aynı görev. Komut dosyası, başarısız çalıştırmada hangi yüzeylerin eksik olduğunu sayar ve bir hata modu raporu yazdırır.
...
Çıktı: iki çalıştırmanın yan yana günlüğü, yalnızca prompt çalıştırmasını özetleyen bir `failure_modes.json` ve çalışma tezgahı çalıştırması için tek satırlık bir karar.

agent küçük, kural tabanlı bir taslaktır; önemli olan model değil yüzeylerdir. Bu mini yolun geri kalanında her yüzeyi gerçek, yeniden kullanılabilir bir artifact olarak yeniden inşa edeceksiniz.
...
- **LangGraph, OpenAI Agent SDK'sı.** Denetim noktaları ve oturum depoları durum yüzeyidir. Aktarımlar aktarım yüzeyidir.
...
1. Zaten agent çalıştırdığınız bir repo seçin. Yedi yüzeyi 0'dan (eksik) 2'ye (sağlıklı) kadar puanlayın. En zayıf yüzeyiniz hangisi?
2. `main.py`'yi, yalnızca prompt çalıştırmasının sahte bir "başarı" iddiası üreteceği şekilde genişletin. Doğrulama kapısının onu yakaladığını doğrulayın.
3. Kendi ürününüz için sekizinci bir yüzey ekleyin. Neden mevcut yedi kişiden birine dönüşmediğini gerekçelendirin.
4. Komut dosyasını, fazladan bir dosya yazma işlemi halüsinasyonu gösteren farklı bir agent saplaması ile yeniden çalıştırın. Hangi yüzey onu ilk yakalar?
...
- [LangChain, Agent Donanımının Anatomisi](https://blog.langchain.com/the-anatomy-of-an-agent-harness/) — on bir bileşen: prompt'ler, araçlar, kancalar, orkestrasyon, sanal alanlar, bellek, beceriler, altagent'ler, çalışma zamanı; kuyrukları atlar, deployment, authz
- [OpenAI, Harness mühendisliği: agent-birinci dünyada Codex'ten yararlanmak](https://openai.com/index/harness-engineering/) — Codex ekibinin çalışma süreleri etrafındaki yüzeylere ilişkin görünümü
- [OpenAI, agent loop Kodeksini Açmak](https://openai.com/index/unrolling-the-codex-agent-loop/) — agent loop, işlev çağrıları üzerinden `while`'ye düşürüldü
- [Uzun süre çalışan agent'ler için Antropik, Etkili koşum takımları](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents) — belirli bir çalışma süresi içindeki uzun ufuk yüzeyleri
...
- [LangChain Deep Agent'nin donanım yetenekleri](https://docs.langchain.com/oss/python/deepagents/harness) — çalışma zamanı yapılandırma yüzeyi

Kullanılabilir ayrıntılara sahip uygulayıcı parçaları:

- [Martin Fowler / Birgitta Böckeler, agent kullanıcılarını kodlamak için Harness mühendisliği](https://martinfowler.com/articles/harness-engineering.html) — kılavuzlar (ileri besleme) + sensörler (geri bildirim); en temiz kontrol teorisi çerçevesi
- [HumanLayer, Beceri Sorunu: Agent'leri Kodlamak için Harness Mühendisliği](https://www.humanlayer.dev/blog/skill-issue-harness-engineering-for-coding-agents) — "bu bir model sorunu değil, bir yapılandırma sorunu"
- [MongoDB, The Agent Harness: LLM Neden Agent Sisteminizin En Küçük Parçasıdır](https://www.mongodb.com/company/blog/technical/agent-harness-why-llm-is-smallest-part-of-your-agent-system) — alındılar: Vercel %80 ila %100, Harvey 2 kat doğruluk, Terminal Bench Top 30'dan İlk 5'e
- [Augment Code, AI Coding Agents için Donanım Mühendisliği](https://www.augmentcode.com/guides/harness-engineering-ai-coding-agents) — kısıtlama öncelikli izlenecek yol
...
- [ai-boost/awesome-harness-engineering](https://github.com/ai-boost/awesome-harness-engineering) — seçilmiş alternatif liste (araçlar, değerlendirmeler, bellek, MCP, izinler)
...
- [HKUDS/OpenHarness](https://github.com/HKUDS/OpenHarness) — yerleşik kişisel agent ile açık agent kablo demeti

Hacker News'te fikir birliği için değil, anlaşmazlıklar için okumaya değer konular:

- [HN: Uzun süre çalışan agent'ler için etkili emniyet kemerleri](https://news.ycombinator.com/item?id=46081704)
...
- [HN: agent koşum takımı sanal alanın dışına aittir](https://news.ycombinator.com/item?id=47990675) — ayrı bir düzlem olarak yetkilendirmeyi savunuyor
...
- Aşama 14 · 27 — Yetkilendirme politikası ilkelinde yer alan Prompt enjeksiyon savunmaları
- Aşama 14 · 29 — Üretim çalışma süreleri (kuyruk, olay, cron): bu dersteki ilkellerin deployment'de yaşadığı yer
