...
- agent'nin kontrolleri yeniden çalıştırmak yerine okuması için prob sonucunu sürdürün.
- Başlatma başarısız olduğunda yüksek sesle, hızlı ve bakacak tek bir yerden başarısız olun.
...
| Bağımlılık kullanılabilirliği | Kayıp bir paket daha sonra onu şimdi yakalamanın maliyetinin on katıdır |
| Test komutu | agent nasıl doğrulama yapılacağını bilmelidir; komut eksikse tezgah bozulur |
| Repo yolları | Sabit kodlanmış yollar sürükleniyor; bunları bir kez çözün ve sabitleyin |
| Ortam değişkenleri | Eksik `OPENAI_API_KEY` çalışma zamanı gizemi değil, başarısızlık yüzeyidir |
...
Prob arızası, insan için durup yüzeye çıkmak anlamına gelir. Hayır "agent bunu çözecektir." Başlatmanın asıl amacı, tezgah bozulduğunda başlamayı reddetmektir.
...
- Beş araştırma: Python sürümü, `importlib.util.find_spec` aracılığıyla listelenen bağımlılıklar, test komutu çözülebilirliği, gerekli env değişkenleri, durum dosyası güncelliği.
- Her prob `(name, status, detail)` değerini döndürür.
- Betik, tam araştırma seti ile `init_report.json` yazar ve herhangi bir blok önem derecesi araştırması başarısız olursa sıfırdan farklı olarak çıkar.
...
Betik, araştırma tablosunu yazdırır, `init_report.json` yazar ve başarısız araştırmaların listesiyle birlikte sıfırdan mutlu yolda veya sıfırdan farklı olarak çıkar.

## Vahşi doğada üretim modelleri

Yararlı bir başlangıç senaryosunu törenden üç kalıp ayırır.

**Bilinen son iyi işleme bağlantısı.** Son başarılı birleştirmede yazılan `LKG` dosyasına göre geçerli işlemeyi araştırın. Fark bir bütçeyi aşarsa (varsayılan 50 dosya), başlamayı reddedin ve bir kişinin yeni temeli onaylamasını isteyin. Cloudflare'in AI Kod İncelemesi, incelemeyi yapan agent'lerin kapsamını belirlemek için bunu kullanır: her inceleme oturumu, aynı bilinen son iyiye dayanır ve oturumlar arasındaki sapmaları hiçbir zaman birleştirmez.

**Dosyaları TTL ile kilitleyin.** İlk başarılı yoklama geçişinden sonra bir `prereqs.lock` yazın. Sonraki çalıştırmalar N saat boyunca kilide güvenir (varsayılan 24 saat) ve pahalı araştırmaları atlar. Başlatma betiği önce kilidi okur; eğer yeniyse ve bağımlılık bildirimi karma eşleşiyorsa, kısa devre yapar. Bu, Docker'ın katman önbellekleri için kullandığı modelin aynısıdır: idempotent probe + content hash = skip.
...
- **Claude Kodu kancalanır.** `pre-task` kancası, başlatma komut dosyasını çağırır ve başarısız olursa agent'yi başlatmayı reddeder.
- **GitHub Eylemleri.** Bir `setup-agent` işi başlatma betiğini çalıştırır; agent işi buna bağlıdır.
...
Başlangıç betiği taşınabilirdir çünkü belirli bir framework'ye çağrı yapmaz. Bash, Make veya bir görev dosyasının tümü onu sarabilir.

## Gönderin

`outputs/skill-init-script.md` projeyle röportaj yapar, kurulum çalışmasını araştırmalara göre sınıflandırır ve projeye özel bir `init_agent.py` artı onu herhangi bir agent adımından önce çalıştıran bir CI iş akışı yayar.
...
4. Araştırmaları sabit kodlu işlevlerden YAML kaydına taşıyın. Takas hakkını savun.
...
|------|----------------|------------------------|
| Prob | "Çek" | `(name, status, detail)` | değerini döndüren deterministik bir işlev |
...
| Kurulum vergisi | "Önyükleme maliyeti" | agent'nin oturum başına harcadığı token'ler bariz olanı yeniden keşfediyor |
...
- [microservices.io, GenAI geliştirme platformu: korkuluklar](https://microservices.io/post/architecture/2026/03/09/genai-development-platform-part-1-development-guardrails.html) — başlangıç olarak ön taahhüt + CI kontrolleri
- [Geliştirme Kodu, AGENTS.md'nizi Nasıl Oluşturabilirsiniz (2026)](https://www.augmentcode.com/guides/how-to-build-agents-md) — başlangıç beklentileri
