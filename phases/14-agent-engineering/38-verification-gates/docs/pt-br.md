...
> O agent não tem autoridade pra marcar o próprio trabalho como pronto. Um verification gate lê o contrato de escopo, o log de feedback, o relatório de regras e o diff, e responde uma única pergunta: essa tarefa tá realmente concluída? Se o gate diz que não, a tarefa não tá pronta, não importa o que o chat diz.
...
- Emitir um `verification_report.json` que tanto o agent revisor quanto o CI possam ler.
...
A solução no workbench é um único verification gate que lê os artifacts que o agent já produziu e toma a decisão. O gate é determinístico. O gate tá no version control. O gate tá conectado no CI. O agent não pode suborná-lo.
...
**Piso de cobertura como verificação de primeira classe.** Um `coverage_report.json` alimenta um check de `coverage_floor` (padrão 80%). O gate falha se a cobertura medida cair abaixo do piso ou abaixo do piso do merge anterior por mais de 1 ponto percentual. Sem esse check, agents silenciosamente deletam testes que falham e os relatórios de verificação continuam verdes.
...
- **Hook pré-handoff.** O runtime do agent chama o gate antes de gerar o documento de handoff. Sem veredicto verde, sem handoff.
- **Triagem manual.** Operadores leem o relatório quando um agent declara sucesso e um humano desconfia.
...
5. Rode o gate num diff real de agent do seu produto. Quantas constatações são reais e quantas são ruído? Onde o gate precisa crescer?
...
- [logi-cmd/agent-guardrails — merge gate spec](https://github.com/logi-cmd/agent-guardrails) — gates de escopo + mutation-testing
...
- Fase 14 · 39 — o agent revisor pra quem o gate transfere
