...
- Tezgah artifact'ler üzerinde deterministik bir fonksiyon olarak bir doğrulama geçidi tanımlayın.
- Kural raporunu, kapsam raporunu, geri bildirim kayıtlarını ve farkları tek bir kararda birleştirin.
- İnceleyen agent ve CI'nın her ikisinin de okuyabileceği bir `verification_report.json` yayınlayın.
- İstisnasız olarak herhangi bir blok ciddiyeti arızasında bir görevi ilerletmeyi reddedin.

## Sorun

Agent'ler başarıyı çok kolay ilan ediyor. Üç başarısızlık şekli hakimdir:
...
Tezgah düzeltmesi, agent'nin zaten ürettiği artifact'leri okuyan ve çağrı yapan tek bir doğrulama kapısıdır. Kapı deterministiktir. Kapı versiyon kontrolündedir. Geçit CI'ya bağlı. agent ona rüşvet veremez.
...
| Dokunulan dosyalar `scope.allowed_files` | ikisi de | uyar |

`warn` bulgusu karara açıklama getiriyor; `block` bulgusu `passed: true`'yi önler.
...
Blok önem derecesi bulguları agent tarafından geçersiz kılınamaz. Bunlar yalnızca kayıtlı bir `override_reason` ve `overridden_by` kullanıcı kimliğine sahip bir insan tarafından geçersiz kılınabilir. Geçersiz kılma, agent kararı değil, imzalı bir değişikliktir.
...
- Bir `verify(task_id, artifacts) -> VerdictReport` saf işlevi.
...
Dört desen, kapıyı "başka bir tüy bırakmayan işten" "belirleyici kenara" yükseltir.

**Derinlemesine savunma, tek kapı değil.** Ön işleme kancası → CI durum kontrolü → ön araç kimlik doğrulama kancası → ön birleştirme kapısı. Her katman deterministik olduğundan bir katmandaki hata diğer katman tarafından yakalanır. microservices.io'nin Mart 2026 başucu kitabı açıktır: ön işleme kancası atlanamaz çünkü model tarafı becerisinden farklı olarak agent'nin aşağıdaki talimatlarına bağlı değildir. Doğrulama kapısı CI / birleştirme öncesi katmanında bulunur.
...
**Slack iş parçacıkları değil, imzalı geçersiz kılma günlüğü.** Her geçersiz kılma, `outputs/verification/overrides.jsonl`'de aşağıdakileri içeren bir satır yayınlar: zaman damgası, bulma kodu, neden, imzalayan kullanıcı, mevcut HEAD taahhüdü. Çalışma zamanı, imzanın eksik olduğu tüm geçersiz kılmaları reddeder; denetim izi git ile izlenir. Bu, geçersiz kılma politikası ile geçersiz kılma tiyatrosu arasındaki çizgidir.

**Birinci sınıf kontrol olarak kapsama alanı.** Bir `coverage_report.json`, bir `coverage_floor` (varsayılan %80) kontrolünü besler. Ölçülen kapsama alanı tabanın altına veya önceki birleştirme tabanının yüzde 1'inden fazla altına düşerse geçit başarısız olur. Bu kontrol olmadan, agent'ler başarısız olan testleri sessizce siler ve doğrulama raporları yeşil kalır.

**`--strict` modu, bloklara yönelik uyarıları destekler.** Serbest bırakma dalları, gemi engelleme PR'leri veya olay sonrası önceliklendirme için `--strict`, her uyarıyı kesin bir başarısızlık haline getirir. Bayrak şubeye göre tercih edilir; küresel varsayılan değil, çünkü her şeyin katı olması günlük akışı yıpratıyor.

## Kullan onu

Üretim modelleri:

- **CI adımı.** Bir `verify_agent` işi, kapıyı agent'nin son artifact'lerine karşı çalıştırır. `passed: true` olmadan birleştirme koruması reddediliyor.
- **Ön geçiş kancası.** agent çalışma zamanı, geçiş belgesini oluşturmadan önce geçidi çağırır. Yeşil karar yok, devir yok.
- **Manuel triyaj.** agent başarılı olduğunu iddia ettiğinde ve bir insan bundan şüphelendiğinde operatörler raporu okur.

Kapı, tezgah akışında belirleyici kenardır. Diğer tüm yüzeyler onun yukarısındadır.

## Gönderin

`outputs/skill-verification-gate.md`, geçidi belirli bir projeye bağlar: hangi kabul komutları onu besler, hangi kurallar blok ciddiyetine sahiptir, hangi kapsam dışı yazma işlemlerine izin verilir, geçersiz kılma denetim günlüğünün nasıl depolandığı.
...
4. Bir `time_since_last_human_touch` kontrolü ekleyin: Bir insan tuş vuruşundan sonra 60 saniye içinde düzenlenen tüm dosyalar kapsam dışı işaretlerden muaftır.
5. Kapıyı ürününüzden gerçek bir agent fark üzerinde çalıştırın. Bulguların kaçı gerçek, kaçı gürültü? Kapının nerede büyümesi gerekiyor?

## Anahtar Terimler

| Dönem | İnsanlar ne diyor | Aslında ne anlama geliyor |
|------|----------------|------------------------|
| Doğrulama kapısı | "İşleri durduran kontrol" | artifact çalışma tezgahı üzerinde başarılı/başarısız kararı üreten deterministik fonksiyon |
| Engelleme ciddiyeti | "Zor başarısızlık" | `passed: true`'yi engelleyen ve imzalı geçersiz kılma gerektiren bir bulgu |
...
- [OpenAI Agents SDK korkulukları](https://openai.github.io/openai-agents-python/guardrails/)
- [microservices.io, GenAI geliştirme platformu: korkuluklar](https://microservices.io/post/architecture/2026/03/09/genai-development-platform-part-1-development-guardrails.html) — ön taahhüt ile CI arasında derinlemesine savunma
...
- [Akira, Agentic Sistemleri için Gerçek Zamanlı Korkuluklar](https://www.akira.ai/blog/real-time-guardrails-agentic-systems) — alet öncesi/sonrası kapıları
- Aşama 14 · 27 — prompt enjeksiyon savunmaları (kapının rakip çifti)
- Aşama 14 · 36 — bu geçidin uyguladığı kapsam sözleşmesi
- Aşama 14 · 37 — bu kapının puanladığı geri bildirim günlüğü
- Aşama 14 · 39 — incelemeyi yapan kişi agent kapıyı ona devreder
