...
> Düzyazı şeklinde yazılan talimatlar dilektir. Kısıtlamalar olarak yazılan talimatlar testlerdir. Çalışma tezgahı, her kuralı bir agent'nin çalışma zamanında kontrol edebileceği ve bir incelemecinin olaydan sonra doğrulayabileceği bir şeye dönüştürür.
...
Tipik bir `AGENTS.md`, katılım belgeleri gibi okunur. agent'ye "dikkatli olmasını", "iyice test etmesini" ve "emin değilseniz sormasını" söyler. Üç gün sonra agent hiçbir test yapmadan bir değişiklik gönderir, yasak bir dizine yazar ve hattın nerede olduğunu asla bilmediği için asla sormaz.
...
Kurallar, kısa kök yönlendiriciden uzakta, `docs/agent-rules.md`'ye aittir. Her kuralın bir adı, bir kategorisi ve bir kontrolü vardır.
...
| Belirsizlik | Emin olmadığınızda agent ne yapar? | "Tahmin etmek yerine soru notunu açın" |
...
İki test katmanlamayı dürüst tutar. Erişilebilirlik testi: Bir agent herhangi bir kurala yönlendiriciden en fazla iki atlamada ulaşmalıdır, bu nedenle yönlendirici her konu belgesini düzyazıyla açıklamamalı, yolla bağlamalıdır. Tazelik testi: Yönlendirici, bir incelemecinin onu her PR'de yeniden okumasına yetecek kadar kısadır; bu, onun sessizce yerini aldığı ansiklopediye geri dönmesini engelleyen tek şeydir. Artık çözümlenmeyen bir işaretçi, eksik bir kuraldan daha kötü bir başarısızlıktır; dolayısıyla yönlendiricideki bozuk bir bağlantının kendisi de bir başlangıç ​​denetimi ihlalidir.

## İnşa Et

`code/main.py` şunları gönderir:

- Kuralları bir veri sınıfına yükleyen `agent-rules.md` ayrıştırıcı.
- `rule_checker.py` tarzı denetleyici işlevleri, `check` referansı başına bir tane.
- İki kuralı ihlal eden bir agent demo çalışması ve bunları yakalayan bir kontrol geçişi.
...
**Yazma sırasında önem derecesi etiketlemesi.** Her kural `severity`: `block`, `warn` veya `info`'yi taşır. Kontrolör üçünü de rapor ediyor; çalışma zamanı yalnızca `block`'yi reddediyor. Çoğu ekip ciddiyeti erkenden abartır ve son teslim tarihi baskısı altında sessizce zayıflatır; Yazma sırasında etiketleme, kalibrasyonun ön plana çıkmasını sağlar. `block` kuralının geçersiz kılınmasını `overrides.jsonl` denetim günlüğüne imzalayan doğrulama kapısı (Aşama 14 · 38) ile eşleştirin.

**Zorlama işlevi olarak kuralın geçerlilik sonu.** Her kural bir `expires_at` tarihi taşır (varsayılan olarak yazma tarihinden itibaren 90 gün). Süresi dolmamış bir kuralın art arda 60 gün boyunca sıfır ihlali olması durumunda denetleyici bir uyarı verir; sonraki üç aylık inceleme ya onu tutmayı haklı çıkarır, onu `info`'ye kadar zayıflatır ya da siler. Cloudflare'in üretim AI Kod İncelemesi verileri (Nisan 2026, 30 günde 5.169 depoda 131.246 inceleme çalıştırıldı), açık sona erme tarihi olan kural kümelerinin depo başına 30 kuralın altında kaldığını gösterdi; çoğu hiç ateşlenmeyen setler 80'in üzerine çıktı.

**Kaynak olarak işaretle, önbellek olarak JSON.** `agent-rules.md` yazılan dosyadır; `agent-rules.lock.json` denetleyicinin sıcak yolda okuduğu bir önbellektir. Kilit, bir ön işleme kancasıyla yeniden oluşturulur. Markdown farkları gözden geçirilebilir; JSON ayrıştırma her fırsatta dışarıda kalır. `package.json` / `package-lock.json` ve `Cargo.toml` / `Cargo.lock` ile aynı şekle sahiptir.
...
- OpenAI Agent SDK korkulukları, giriş ve çıkış korkuluklarıyla aynı kontrolleri kaydeder. İşaretleme, dokümanın yüzeyidir; SDK çalışma zamanı yüzeyidir.
...
`outputs/skill-rule-set-builder.md` bir proje sahibiyle röportaj yapar, mevcut düzyazı talimatlarını beş kategoride sınıflandırır ve sürümlendirilmiş bir `agent-rules.md` artı bir kontrol taslağı yayınlar.
...
3. Denetleyiciyi CI'ya bağlayın: en son agent çalıştırmasında blok önem derecesi kuralı başarısız olursa derleme başarısız olur.
...
- [Rick Hightower, Agent RuleZ: Deterministik Bir Politika Motoru](https://medium.com/@richardhightower/agent-rulez-a-deterministic-policy-engine-for-ai-coding-agents-9489e0561edf) — üretimde engelleme/uyarma/bilgi ciddiyeti
...
- [Tip Kontrollü Uyumluluk: Deterministik Korkuluklar (arXiv 2604.01483)](https://arxiv.org/pdf/2604.01483) — Kontrol kuralında üst sınır olarak Yalın 4
