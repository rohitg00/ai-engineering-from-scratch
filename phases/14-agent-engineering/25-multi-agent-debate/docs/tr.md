...
> Du ve ark. (ICML 2024, "Society of Minds"), bağımsız olarak yanıtlar öneren N model örneğini çalıştırır, ardından yakınsamak için R turları üzerinden birbirini yinelemeli olarak eleştirir. Gerçekliği, kurallara uymayı ve akıl yürütmeyi geliştirir. Seyrek topoloji, token maliyetinde tam ağı yener.
...
- Tam örgü ve seyrek değişkenlere sahip, komut dosyasıyla yazılmış bir LLM üzerinde bir stdlib tartışması uygulayın; token maliyeti ve doğruluğu ölçün.
...
Orijinal deneylerde maliyet nedeniyle N=3, R=2 kullanıldı. Doğruluk, daha fazla agent ve zor problemler üzerinde daha fazla turla artar (MMLU, GSM8K, Satranç Hareketi Geçerliliği, biyografi oluşturma).

Çapraz model kombinasyonları, tek model tartışmalarını yener: ChatGPT + Bard birlikte > ikisi de tek başına.

### Seyrek topoloji

"Seyrek İletişim Topolojisi ile Çoklu Agent Tartışmasının Geliştirilmesi" (arXiv:2406.11776, 2024-2025), tam ağ tartışmasının her zaman optimal olmadığını gösterdi. Seyrek topolojiler (yıldız, halka, hub ve bağlı bileşen), daha düşük token maliyetle doğruluğu karşılayabilir. Her tartışmacı akranlarının yalnızca bir alt kümesini görür.

Çıkarımlar:

- Tam ağ N=5, R=3 = 5 × 3 = 15 teklif, her biri 4 eş okuma = 60 eleştiri işlemi.
...
- **Maliyete duyarlı ölçek.** Soru başına N × R token.
...
- **LangGraph süpervizörü** (Ders 13) — merkezi yönlendirici + uzman agent'ler tartışmayı bir düğüm olarak uygulayabilir.
- **OpenAI Agent SDK'sı** (Ders 16) — agent'nin yinelemeli eleştiri için ileri geri aktarımı.
- **Multi-agent değerlendirmeleri** — değerlendirme sinyali için ikili tartışma + değerlendirici-iyileştirici.
...
- `Debater` sınıfı (tartışmacı başına görüş sapması ile yazılı LLM).
...
`outputs/skill-debate.md`, yapılandırılabilir topoloji, N, R ve yakınsama kuralıyla çoklu agent tartışmasını destekler.

## Egzersizler

1. "Zorunlu anlaşmazlık" kuralını uygulayın: 1. turda her tartışmacı farklı bir öneri sunmalıdır. Yakınsama hızı üzerindeki etkiyi ölçün.
...
3. Bir "agent"yi, farklı görüşlere sahip farklı bir kodlu LLM ile değiştirin. Heterojenlik doğruluğu artırır mı?
4. 3 sorunuzda tam ağ ve seyrek için token maliyetini ölçün. Arsa maliyeti ve doğruluk.
...
|------|----------------|------------------------|
| Tartışma | "Çoklu agent eleştirisi" | N teklif sahibi, R çapraz eleştiri turu, yakınsama |
...
- [Antropik, Etkili Agent'ler Oluşturma](https://www.anthropic.com/research/building-effective-agents) — bir tartışma çeşidi olarak orkestratör-çalışanlar
