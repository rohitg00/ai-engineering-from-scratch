...
Chatbot mimarileri, her biri bir öncekinin gözle görülür şekilde başarısız olması nedeniyle tanıtılan dört paradigma arasında geçiş yaptı. Bu ders bunları sırayla açıklayacaktır. 2026 üretim manzarası son ikisinin bir melezidir.

## Konsept

![Chatbot evrimi: kural tabanlı → alma → sinirsel → agent](../assets/chatbot.svg)
...
**1950.** Turing yan adımlar "makineler düşünebilir mi?" operasyonel bir değişiklik önererek: Eğer bir sorgulayıcı makineyi teletip üzerinden bir kişiden ayırt edemiyorsa, felsefi soru tartışmalıdır. Alan bir isme sahip olmadan önce konuşma, alanın benchmark'si haline gelir.
...
**2001.** SmarterChild, tarifi 30 milyon anlık mesajlaşma kullanıcısının önüne koyuyor ve şablonlara eklenmiş arka uç aramaları (hava durumu, hisse senetleri, film saatleri) ekliyor. Şaşı ve 2001 kostümü giyen tool calling: Niyeti ayrıştırın, bir hizmeti arayın, sonucu cevaba dönüştürün.
...
**LLM agents.** Planlayan, araçları çağıran ve sonuçları doğrulayan bir döngüye sarılmış bir dil modeli. Uzun bir prompt'ye sahip bir sohbet robotu değil. Bir agent loop: planla → aracı çağır → sonucu gözlemle → bir sonraki adıma karar ver. Alma öncelikli topraklama (RAG), halüsinasyon görmesini önler. Araç çağrıları, bazı şeyleri gerçekten yapmasına olanak tanır. Bu 2026 mimarisi.

Dört paradigma ardışık ikameler değildir. 2026 yapımı bir sohbet robotu, dördünü de yönlendirir: kimlik doğrulama ve yıkıcı eylemler için kural tabanlı, SSS için erişim, doğal ifadeler için sinir oluşturma, belirsiz açık uçlu sorgular için LLM agent.
...
Bu açıklayıcı kod parçası `pip install sentence-transformers` (meşaleyi çeken) gerektirir. Bu ders için çalıştırılabilir `code/main.py` bunun yerine stdlib Jaccard benzerliğini kullanır, böylece ders harici bağımlılıklar olmadan çalışır.
...
Gerçek üretim şunları ekler: alma öncelikli topraklama (her LLM çağrısından önce ilgili belgeleri enjekte edin), korkuluklar (onay olmadan yıkıcı eylemleri reddedin), observability (her adımı kaydedin) ve değerlendirmeler (agent davranışının spesifikasyona uygun kaldığını gösteren otomatik kontroller).
...
Model: yıkıcı her şey için deterministik kurallar, hazır SSS'lere erişim, diğer her şey için LLM agent'ler. 2026 müşteri destek sistemlerinde sunulan şey budur.
...
| Açık uçlu yardım sohbeti | RAG + araç çağrılarıyla LLM agent |
| Dahili araçlar / IDE yardımcıları | LLM agent araç çağrılarıyla (arama, okuma, yazma) |
| Yardımcı / karakter sohbet robotları | Kişi sistemi prompt ile ayarlanmış LLM, bilgiye erişim |

Üretimde her zaman hibrit yönlendirmeyi kullanın. Hiçbir mimari her isteği iyi bir şekilde yerine getiremez. Yönlendirme katmanının kendisi genellikle küçük bir amaç sınıflandırıcıdır.
...
- **Prompt enjeksiyonu.** Kullanıcı, prompt sistemini geçersiz kılan metni ekler. LLM Uygulamaları 2025 için OWASP İlk 10'da LLM01 olarak derecelendirildi. İki çeşit: doğrudan enjeksiyon (sohbete yapıştırılır) ve dolaylı enjeksiyon (belgelerde, e-postalarda veya agent'nin okuduğu araç çıktılarında gizli).

  Saldırı oranları senaryoya göre değişir. Ölçülen başarı oranları, genel araç kullanımında ve benchmark kodlamasında sınır modellerinde ~%0,5-8,5 aralığındadır. Belirli yüksek riskli kurulumlar (agent kodlayan yapay zekaya karşı uyarlanabilir saldırılar, savunmasız orkestrasyon) ~%84'e ulaştı. Üretim CVE'leri arasında Microsoft 365 Copilot'ta saldırgan tarafından kontrol edilen bir e-posta tarafından tetiklenen sıfır tıklamayla veri filtreleme kusuru olan EchoLeak (CVE-2025-32711, CVSS 9.3) bulunmaktadır.

  Azaltıcı önlemler: döngü boyunca kullanıcı girdisine güvenilmeyen giriş olarak davranın; alet çağrılmadan önce sterilize edin; takım çıkışlarını ana prompt'den yalıtın; agent'nin önce planladığı, ardından yürütmeden önce her eylemi bu plana göre doğruladığı Planla-Doğrula-Yürüt (PVE) modelini kullanın (bu, aracın yeni planlanmamış eylemlerin eklenmesinden kaynaklanan sonuçları durdurur); yıkıcı eylemler için kullanıcı onayı gerektirmek; Araç kapsamlarına en az ayrıcalık uygulayın.
...
- **Kapsam kayması.** Agent, bir araç çağrısının teğetsel olarak ilgili bilgileri döndürmesi nedeniyle görev dışı kalıyor. Azaltma: dar araç sözleşmeleri; sistemi prompt odaklanmış halde tutun; görev dışı oran için değerlendirmeler ekleyin.
...
3. **Zor.** Yukarıdaki agent loop'yi üç araçla uygulayın (arama, kullanıcı verilerini okuma, e-posta gönderme). prompt enjeksiyon denemeleri dahil 50 test senaryosuyla bir değerlendirme yapın. Görev dışı oranı, başarısız görev oranını ve herhangi bir enjeksiyon başarısını raporlayın.
...
-[Turing (1950). Computing Machinery and Intelligence](https://academic.oup.com/mind/article/LIX/236/433/986238) — alanın benchmark konuşmasını sağlayan makale.
-[Weizenbaum (1966). ELIZA — Doğal Dil İletişimi Çalışmalarına Yönelik Bir Bilgisayar Programı](https://web.stanford.edu/class/cs124/p36-weizenabaum.pdf) — orijinal kural tabanlı sohbet robotu makalesi.
...
- [Yao ve ark. (2022). ReAct: Dil Modellerinde Akıl Yürütme ve Harekete Geçirme](https://arxiv.org/abs/2210.03629) — agent loop modelini adlandıran makale.
- [Anthropic'in etkili agent'ler oluşturma kılavuzu](https://www.anthropic.com/research/building-effective-agents) — 2026'da hala geçerli olan 2024 üretim kılavuzu.
- [Greshake ve ark. (2023). Kaydolduğunuz şey bu değil: Dolaylı Prompt Enjeksiyonu ile Gerçek Dünya Yüksek Lisans Bütünleşik Uygulamalarından Ödün Verme](https://arxiv.org/abs/2302.12173) — prompt enjeksiyon kağıdı.
- [LLM Uygulamaları 2025 için OWASP Top 10 — LLM01 Prompt Enjeksiyon](https://genai.owasp.org/llmrisk/llm01-prompt-injection/) — prompt enjeksiyonunu en önemli güvenlik sorunu haline getiren sıralama.
- [AWS — Amazon Bedrock Agent'leri Dolaylı Prompt Enjeksiyonlarına karşı koruma](https://aws.amazon.com/blogs/machine-learning/securing-amazon-bedrock-agents-a-guide-to-safeguarding-against-indirect-prompt-injections/) — Planla-Doğrula-Yürüt ve kullanıcı onayı akışlarını içeren pratik düzenleme katmanı savunmaları.
- [EchoLeak (CVE-2025-32711)](https://www.vectra.ai/topics/prompt-injection) — dolaylı prompt enjeksiyonundan standart sıfır tıklamalı veri filtreleme CVE'si. Yazma erişimli agent'lerin neden çalışma zamanı savunmalarına ihtiyaç duyduğuna ilişkin referans örnek olay.
