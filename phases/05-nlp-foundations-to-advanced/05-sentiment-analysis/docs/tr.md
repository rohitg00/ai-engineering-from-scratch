...
L2 düzenlemesi burada önemlidir. Metin özellikleri seyrektir; L2 olmadan model eğitim örneklerini ezberler. `0.01`'dan başlayın ve ayarlayın.

### Adım 4: Olumsuzlamayı ele alma (hata modu)

"İyi değil" ve "fena değil"i düşünün. Bir BoW sınıflandırıcı, `{not, good}` ve `{not, bad}`'yi görür ve eğitimde hangisi daha fazla ortaya çıkarsa onu öğrenir. Bir bigram sınıflandırıcı, `not_good` ve `not_bad`'yi görür ve bunları farklı özellikler olarak öğrenir. Bu genellikle yeterlidir.

Bigramlarınız olmadığında işe yarayan daha basit bir düzeltme: **olumsuzlama kapsamı**. Bir olumsuzluk sözcüğünden sonra gelen token ön eki, bir sonraki noktalama işaretine kadar `NOT_` ile birlikte kullanılır.
...
Artık `good` ve `NOT_good` farklı özelliklerdir. Sınıflandırıcı bunları zıt yönde ağırlıklandırabilir. Üç satırlık ön işleme, duyarlılık benchmark'larda ölçülebilir doğruluk atlaması.
...
Dikkat edilmesi gereken üç şey. `stop_words=None` olumsuzlamaları sürdürüyor. `ngram_range=(1, 2)` bigramlar ekler, böylece `not_good` bir özellik haline gelir. `sublinear_tf=True` tekrarlanan kelimeleri azaltır. Bu üç işaret, SST-2'de %75 doğru taban çizgisi ile %85 doğru taban çizgisi arasındaki farktır.

### transformer'ya ne zaman ulaşılmalı?
...
Yukarıdakilerden herhangi birine ihtiyacınız varsa 7. aşamaya (transformerderin dalış) geçin. Aksi takdirde, Naive Bayes veya TF-IDF artı bigramlar artı olumsuzlama yönetimi üzerindeki lojistik regresyon, 2026 üretim temelinizdir.
...
1. **Kolay.** Scikit-learn kanalına ön işleme adımı olarak `apply_negation`'yi ekleyin ve F1 deltasını küçük bir duyarlılıkla dataset ölçün.
2. **Orta.** Sınıf ağırlıklı lojistik regresyon uygulayın (scikit-learn'e `class_weight="balanced"` iletin veya gradient'yı kendiniz türetin). Sentetik 90-10 sınıfı dengesizlik üzerindeki etkiyi ölçün.
...
| Olumsuzluk kapsamı | Yakındaki token'ler geri gidiyor | Noktalama işaretine kadar "değil"den sonra `NOT_` ile token öneki. |
...
- [Pang ve Lee (2008). Fikir Madenciliği ve Duygu Analizi](https://www.cs.cornell.edu/home/llee/opinion-mining-sentiment-analysis-survey.html) — temel anket. Uzun ama ilk dört bölüm klasik olan her şeyi kapsıyor.
- [Wang ve Manning (2012). Temel Çizgiler ve Bigramlar: Basit, İyi Duygu ve Konu Sınıflandırması](https://aclanthology.org/P12-2018/) — bigramları + Naif Bayes'i gösteren makalenin kısa metinde yenilmesi zordur.
