...
Ders 09 ölçülen bir başarısızlıkla sona erdi. Bir oyuncak kopyalama görevi üzerinde eğitilmiş bir GRU kodlayıcı-kod çözücü, uzunluk 5'te %89 doğruluktan, uzunluk 80'de neredeyse şansa gider. Bunun nedeni bir eğitim hatası değil, yapısaldır: kodlayıcının topladığı her bilgi biti sabit boyutlu bir gizli duruma sığmak zorundadır ve kod çözücü asla başka bir şey görmez.

Bahdanau, Cho ve Bengio, 2014 yılında üç satırlık bir düzeltme yayınladılar. Kod çözücüye yalnızca son kodlayıcı durumunu vermek yerine, her kodlayıcı durumunu koruyun. Her kod çözücü adımında, ağırlıkların "kod çözücünün `i` kodlayıcı konumuna şu anda ne kadar bakması gerekiyor?" dediği kodlayıcı durumlarının ağırlıklı ortalamasını hesaplayın. Bu ağırlıklı ortalama bağlamdır ve kod çözücünün her adımını değiştirir.

Bütün fikir bu. Transformeruzattı. Kişisel dikkat bunu tek bir diziye uyguladı. Çok kafalı dikkat paralel olarak yürütüldü. Ancak 2014 sürümü zaten darboğazı aştı ve bir kez buna ulaştığınızda, transformers'ye geçiş kavramsal değil mühendisliktir.
...
1. **sorgu** olarak önceki kod çözücünün gizli durumunu `s_{t-1}` kullanın.
2. Bunu her kodlayıcı gizli durumuna `h_1, ..., h_T` göre puanlayın. Enkoder konumu başına bir skaler.
3. Toplamı 1 olan dikkat ağırlıklarını `α_{t,1}, ..., α_{t,T}` elde etmek için puanları Softmax yapın.
4. Bağlam vektörü `c_t = Σ α_{t,i} * h_i`. Kodlayıcı durumlarının ağırlıklı ortalaması.
5. Kod çözücü, `c_t` artı önceki çıktı token'yı alır ve sonraki token'yi üretir.
...
- `v_α`, `(d_attn,)` şeklindedir. `v_α` ile iç çarpım bir skalere daraltılır. **`v_α`'nin yaptığı budur.** Bu bir sihir değildir. Dikkatin sönük olduğu bir vektörü skaler bir puana dönüştüren projeksiyondur.
...
- `general`: `e_{t,i} = s_t^T * W * h_i` ifadesinde `W`, `(d_s, d_h)` shape'ine sahiptir. Eşit boyut kısıtını ortadan kaldırır.
- `concat`: esasen Bahdanau formu. İlk ikisi daha ucuz olduğundan nadiren kullanılır.

**Adlandırmaya değer bir Bahdanau / Luong var.** Bahdanau, `s_{t-1}` (mevcut kelimeyi oluşturmadan *önceki* kod çözücü durumu) kullanır. Luong, `s_t` (*sonraki* durumu) kullanır. Bunları karıştırmak, hata ayıklaması son derece zor olan, incelikli yanlış gradient'lar üretir. Bir kağıt seçin ve kurallarına sadık kalın.
...
Şekillerinizi yukarıdaki tabloya göre kontrol edin. `encoder_states`, `(T_enc, d_h)` şeklindedir. `projected_enc`, `(T_enc, d_attn)` şeklindedir. `projected_dec`, `(d_attn,)` şeklindedir ve yayın yapar. `combined`, `(T_enc, d_attn)` şeklindedir. `scores`, `(T_enc,)` şeklindedir. `weights`, `(T_enc,)` şeklindedir. `context`, `(d_h,)` şeklindedir. Gönderin.
...
Üç kodlayıcı durumu (kabaca "kedi", "sat", "mat") ve ilkiyle en fazla hizalanan kod çözücü durumu göz önüne alındığında, dikkat dağıtımı 0 konumuna odaklanır. Kod çözücü durumu sonuncuyla hizalanacak şekilde kayarsa dikkat 2. konuma hareket eder. Bağlam vektörü izler.
...
### 4. Adım: neden bu transformer'lara giden köprü?
...
Klasik dikkatte anahtarlar ve değerler aynı şeydir. Kişisel dikkat onları ayırır: K ve V için farklı öğrenilmiş yansıtmalarla bir diziyi kendisine karşı sorgulayabilirsiniz. Çok kafalı dikkat, bunu öğrenilmiş farklı yansıtmalarla paralel olarak çalıştırır. Transformer'lar tüm aşamayı birçok kez istifler ve RNN'leri bırakır.
...
- transformer'ların uymadığı cihaz içi sıralı görevler.
...
1. **Kolay.** Kodlayıcıdaki token'leri doldurmak için `softmax` maskelemeyi uygulayın, böylece dikkat ağırlığı sıfır olur. Değişken uzunluklu dizilere sahip bir parti üzerinde test yapın.
2. **Orta.** Luong `general` formuna çok kafalı dikkati ekleyin. `d_h`'yi `n_heads` gruba ayırın, kişi başına dikkati çalıştırın, birleştirin. Tek kafalı durumun önceki uygulamanızla eşleştiğini doğrulayın.
...
| Çarpımsal dikkat | Luong noktası / genel | Puan: `q^T k` veya `q^T W k`. Çoğu görevde daha ucuz ve aynı doğruluk. |
| Hizalama matrisi | Güzel resim | `(T_dec, T_enc)` ızgarası olarak dikkat ağırlıkları. Modelin neye katıldığını görmek için okuyun. |
...
- [Deep Learning — Bahdanau Dikkatine](https://d2l.ai/chapter_attention-mechanisms-and-transformers/bahdanau-attention.html) dalın — PyTorch ile çalıştırılabilir izlenecek yol.
