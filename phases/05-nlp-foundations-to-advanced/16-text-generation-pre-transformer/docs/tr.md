...
transformer'lerden önce, RNN'lerden önce, embedding kelimelerinden önce bir dil modeli, önceki `n-1` kelimelerinin ne sıklıkta takip edildiğini sayarak bir sonraki kelimeyi tahmin ediyordu. "Kedi" → "oturdu" 47 kez sayın, "kedi" → 12 kez "atladı", "kedi" → "buzdolabı" 0 kez sayın. Bir olasılık dağılımı elde etmek için normalleştirin.
...
İlginç sorun, görünmeyen n-gramlar hakkında ne yapılacağıdır. Ham sayıma dayalı bir model, görmediği herhangi bir şeye sıfır olasılık atar; bu felakettir çünkü cümleler uzundur ve neredeyse her uzun cümle en az bir görünmeyen dizi içerir. Elli yıllık yumuşatma araştırması bunu düzeltti. Sonuçta Kneser-Ney yumuşatma ortaya çıktı ve modern deep learning onun ampirik geleneğini miras aldı.
...
Shannon bunu 1951'de yürüttü ve hala bu alanda geçerli olan bir sayı elde etti. 27 sembollü bir alfabe (26 harf artı boşluk), harf başına `log2(27) ≈ 4.75` bit taşıyabilir. 100 harflik bağlamı olan insan tahminciler, harf başına 0,6 ila 1,3 bit arasında sonuç elde etti. İngilizce kabaca dörtte üçü zorunlu hareketlerden oluşur. Bir modelin öğrenmesi gereken yapı, herhangi bir modelin öğrenmesinden önce ölçüldü.
...
- **Şaşkınlık** `2^bits`'dir (veya `e^nats`): tahminden sonra modelin hâlâ karşılaştığı dallanma faktörü. 27 sembolün üzerinde tek tip tahmin yapmak şaşkınlıktır 27; Harf başına 1 bitlik bir oyuncunun şaşkınlığı 2'dir.
- **Bağlam uzunluğu oynatıcının hafızasıdır.** Bir trigram modeli iki token hafızayla oynatılır. Bir transformer, 100K token'lerle aynı oyunu oynar. Kurallar hiçbir zaman değişmedi; oyuncu daha iyi hale geldi.

İzlemeye bir birim geçiş: oyun, bit cinsinden harf başına puan alırken (`log2`), aşağıdaki n-gram formülleri nats'ta (doğal log) token kelime başına puan alırken - ve nats'taki `e^H` şaşkınlık, bit cinsinden `2^H`'ye eşit olduğundan, iki görünüm farklı birimlerde aynı ölçümdür.

```figure
prediction-game
```

**N-gram olasılığı:** `P(w_i | w_{i-n+1}, ..., w_{i-1})`. `n`'yi düzeltin (tipik olarak trigramlar için 3, 4 gram için 4). Sayımlardan hesaplayın:
...
5. **Mutlak indirim.** Tüm sayılardan `D` sabit bir indirimi çıkarın ve görülmeyenlere yeniden dağıtın.
6. **Kneser-Ney.** Mutlak indirim artı daha düşük dereceli model için akıllıca bir seçim: ham sıklık yerine *devam olasılığını* (bir kelimenin kaç bağlamda göründüğünü) kullanın.
...
Giriş, tokenize edilmiş cümlelerin bir listesidir. Çıktı n gram sayımları ve bağlam sayımlarıdır. `<s>` ve `</s>` cümle sınırlarıdır.
...
Daha düşük olması daha iyidir. Brown korpusu için, iyi ayarlanmış 4 gramlık bir KN modeli, 140 civarında şaşkınlığa ulaşıyor. Bir transformer LM, aynı test setinde 15-30'a ulaşıyor. Aradaki fark yaklaşık 10x. Bu boşluk, alanın ilerlemesinin nedenidir.
...
- **Temel çizgiler.** Sinirsel LM'nizin iyi olduğunu bildirmeden önce daima bir n-gram LM karışıklığını hesaplayın. transformer cihazınız KN'yi açık farkla geçemiyorsa bir şeyler ters gidiyor demektir.
...
|------|-----------------|-----------------------|
| N-gram | Kelime dizisi | Ardışık `n` token dizisi. |
...
| Devam olasılığı | KN'ye özgü | `P(w)`, ham sayıma göre değil, `w`'nin göründüğü bağlam sayısına göre ağırlıklandırılır. |
...
- [Chen ve Goodman (1998). Dil Modellemesi için Düzeltme Teknikleri Üzerine Ampirik Bir Çalışma](https://dash.harvard.edu/handle/1/25104739) — Kneser-Ney'i en iyi n-gram yumuşatıcı olarak belirleyen makale.
- [Kneser ve Ney (1995). M-gram Dil Modellemesi için Geliştirilmiş Destekleme](https://ieeexplore.ieee.org/document/479394) — orijinal KN makalesi.
