...
> Aşama 13 her parçayı öğretti. Bu kapak taşı bunları üretim şeklindeki tek bir sisteme bağlar: araçlar + kaynaklar + prompt'lar + görevler + kullanıcı arayüzü, uçta OAuth 2.1 içeren bir MCP sunucusu, bir RBAC ağ geçidi, bir çoklu sunucu istemcisi, bir A2A alt-agent çağrısı, bir toplayıcıya OTel izleme, CI'da araç zehirlenmesi tespiti ve bir AGENTS.md + SKILL.md paketi. Sonunda her mimari seçimi savunabilirsiniz.
...
- Araçları, kaynakları, prompt'leri ve bir `ui://` uygulamasıyla bir görevi açığa çıkaran bir MCP sunucusu oluşturun.
...
- Bir iş yükünün bir kısmını bir A2A alt-agent'ye devredin; opaklığın korunduğunu doğrulayın.
- Tüm yığını AGENTS.md + SKILL.md ile paketleyin, böylece diğer agent'lar onu yönlendirebilir.

## Sorun

"Araştırma ve raporlama" sistemini gönderin:

- Kullanıcı şunu sorar: "agent protokolleri hakkında en çok alıntı yapılan 2026 arXiv makalelerini özetleyin."
- Sistem: MCP aracılığıyla arXiv'i arayın; A2A aracılığıyla makale özetleme işini uzman bir yazara agent devredin; toplu sonuçlar; MCP Uygulamaları `ui://` kaynağı olarak etkileşimli bir rapor oluşturun; Her adımı OTel'e kaydedin.
...
Bir izleme kimliği. Her yayılmanın doğru `gen_ai.*` özelliği vardır.
...
- RBAC: `alice`, `research:read`'ye sahiptir, `research:write`, tüm araçları çağırabilir. `bob`'de `research:read` var, `generate_report` aranamıyor.
...
Kullanıcılar `docker compose up` ile dağıtım yapar. Claude Code, Cursor, Codex ve opencode kullanıcıları, `run-research` becerisini kullanarak sistemi yönlendirebilirler.
...
| 06-10 | MCP temel öğeleri, sunucu, istemci, aktarımlar, kaynaklar + prompts |
...
`code/main.py` önceki derslerin kalıplarını çalıştırılabilir tek bir demoda birleştiriyor. Tamamı stdlib, tamamı işlem halinde, böylece uçtan uca okuyabilirsiniz. Araştırma ve raporlama senaryosunun tüm akışını çalıştırır: ağ geçidiyle anlaşma, OAuth 2.1 simüle edilmiş, araçlar/liste birleştirilmiş, görev olarak rapor oluştur, yazara A2A çağrısı, ui:// kaynağı döndürüldü, OTel yayılımları yayıldı.
...
Bu ders `outputs/skill-ecosystem-blueprint.md` üretir. Bir ürün ihtiyacı göz önüne alındığında (araştırma, özetleme, otomasyon), beceri tam mimariyi üretir: hangi MCP temelleri, hangi ağ geçidini kontrol eder, hangi A2A çağırır, hangi telemetri, hangi paketleme.

## Egzersizler

1. `code/main.py`'yı çalıştırın. Tek izleme kimliğini ve yuvanın nasıl yayıldığına dikkat edin. Demonun 13. Aşamadan kaç ilkel öğeye dokunduğunu sayın.

2. Demoyu genişletin: ikinci bir arka uç MCP sunucusu (e.g. `bibliography`) ekleyin ve ağ geçidinin araçlarını aynı ad alanında birleştirdiğini doğrulayın.

3. Sahte A2A yazıcısını agent bir alt işlemde çalışan gerçek yazıcıyla değiştirin. Ders 19 emniyet kemerini kullanın.
...
5. Bu sistemin bakımını yapacak takım arkadaşınız için bir AGENTS.md yazın. Okumak ve onlara Cursor veya Codex'te kapak taşını sürmek için ihtiyaç duydukları her şeyi vermek beş dakikadan az sürecektir.
...
- [MCP — Şartname 2025-11-25](https://modelcontextprotocol.io/specification/2025-11-25) — birleştirilmiş referans
