...
4. Cross-agent installation. A tested `npx skills add <owner>/<repo>` command plus confirmed runtime-specific target paths.
5. Installation verification. Capture `npx skills list --json`, then compare its inventory with the files each target runtime reads.
6. Loading path. Prefer the installer-managed project path, `.agents/skills/<name>/SKILL.md`, unless the target runtime documents another path.
