...
> Bir agent beş aracı, üç MCP sunucusunu ve iki alt-agent'yi çağırır. Hepsinde tek bir ize ihtiyacınız var. OpenTelemetry GenAI semantik kuralları (v1.37 ve üzeri kararlı nitelikler), Datadog, Langfuse, Arize Phoenix, OpenLLMetry ve AgentOps tarafından yerel olarak desteklenen 2026 standardıdır. Bu ders gerekli nitelikleri adlandırır, yayılma hiyerarşisinde (agent → LLM → araç) yürür ve herhangi bir OTel dışa aktarıcısına takabileceğiniz bir stdlib yayılma yayıcı gönderir.
...
- agent loop, LLM çağrısını, araç çağrısını ve MCP istemci gönderimini kapsayan bir izleme hiyerarşisi oluşturun.
...
Şubat 2026'dan bir hata ayıklama: kullanıcı "agent cihazımın yanıt vermesi bazen 30 saniye sürüyor; diğer zamanlarda 3 saniye sürüyor." Hiçbir iz yok. Günlükler LLM çağrısını gösterir, ancak araç gönderimini, MCP sunucusunun gidiş dönüşünü veya alt-agent'yi göstermez. Tahmin et. Sonunda şunu bulursunuz: Bir MCP sunucusu ara sıra soğuk başlangıçta takılı kalıyor.

Uçtan uca izleme olmadan bunu bulamazsınız. OTel GenAI sorunu düzeltiyor.

Kurallar 2025-2026'da OpenTelemetry semantic-conventions grubu altında belirlendi. Datadog, Langfuse, Phoenix, OpenLLMetry ve AgentOps'un hepsinin aynı yayılma alanlarını ayrıştırması için kararlı özellik adlarını tanımlarlar. Enstrüman bir kez; herhangi bir arka uca gönderin.
...
- `gen_ai.response.model` — model gerçekten hizmet etti.
...
- Bir süreç sınırını aşan çağrılar için (LLM sağlayıcısı, MCP sunucusu) `SpanKind.CLIENT`.
- agent'nin kendi döngü adımları ve araç yürütmesi için `SpanKind.INTERNAL`.
...
Token düzeyindeki etkinlikler, yayılma etkinlikleri olarak eklenebilir:
...
- **Langfuse.** LLM-observability'ye özgü; token kullanımını görselleştirir.
...
Bir MCP istemcisi bir sunucuyu aradığında, isteğe W3C traceparent başlığını ekleyin. Akış yapılabilir HTTP standart başlıkları destekler. Stdio, HTTP başlıklarını yerel olarak taşımaz; spesifikasyonun 2026 yol haritası, JSON-RPC çağrılarına bir `_meta.traceparent` alanı eklemeyi tartışıyor.
...
### AgentOperasyon katmanı

AgentOps (kuruluşu 2024), GenAI observability konusunda uzmanlaşmıştır. OTel yayılmalarını otomatik olarak yaymak için popüler framework'leri (LangGraph, Pydantic AI, CrewAI) sarar. Yığınınız desteklenen bir framework kullanıyorsa kullanışlıdır; aksi takdirde manuel enstrümantasyon kullanın.
...
- Gerekli `gen_ai.*` öznitelikleri dolduruldu.
- İçerik yakalama varsayılan olarak kapalıdır; bir senaryo onu env var aracılığıyla açar.

## Gönderin

Bu ders `outputs/skill-otel-genai-instrumentation.md` üretir. Bir agent kod tabanı verildiğinde, beceri bir enstrümantasyon planı üretir: aralıkların nereye ekleneceği, hangi niteliklerin doldurulacağı ve hangi dışa aktarıcıların hedefleneceği.

## Egzersizler

1. `code/main.py`'yı çalıştırın. Aralıkları sayın ve hangisinin MÜŞTERİ ve DAHİLİ olduğunu belirleyin.

2. İçerik yakalamayı (env var) açın ve `gen_ai.content.prompt` ve `gen_ai.content.completion` etkinliklerinin göründüğünü onaylayın. Kişisel Bilgilere İlişkin Etkilere Dikkat Edin.
...
4. Bir izleme ebeveynini bir üst agent yayılma alanından bir MCP isteğinin `_meta.traceparent` alanına yayın. MCP sunucusunun aynı izleme kimliğini göreceğini doğrulayın.
...
| GenAI semconv | "GenAI anlamsal kuralları" | LLM / tool / agent span için kararlı nitelik adları |
...
- [OpenTelemetry — GenAI semconv](https://opentelemetry.io/docs/specs/semconv/gen-ai/) — GenAI aralıkları, metrikleri ve etkinlikleri için kanonik kurallar
- [OpenTelemetry — GenAI spans](https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-spans/) — LLM ve araç yürütme yayılma özelliği listesi
- [OpenTelemetry — GenAI agent yayılma alanı](https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-agent-spans/) — agent-seviye `invoke_agent` yayılma alanı
- [open-telemetry/semantic-conventions — GenAI yayılmaları](https://github.com/open-telemetry/semantic-conventions/blob/main/docs/gen-ai/gen-ai-spans.md) — GitHub tarafından barındırılan gerçek kaynağı
