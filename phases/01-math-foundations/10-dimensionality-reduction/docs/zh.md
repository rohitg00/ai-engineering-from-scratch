...
**类型：** 动手
**语言：** Python
**Prerequisites:** 阶段 1, Lessons 01 (Linear Algebra Intuition), 02 (Vectors, Matrices & Operations), 03 (Eigenvalues & Eigenvectors), 06 (Probability & Distributions)
**Time:** 约 90 分钟

## 学习目标

- 从零实现 PCA：数据中心化、计算协方差矩阵、特征分解和投影
- 使用解释方差比和肘部法则选择主成分数量
- 比较 PCA、t-SNE 和 UMAP 在二维中可视化 MNIST 数字的效果，并解释它们的权衡
- 使用 RBF 核的核 PCA 来分离标准 PCA 无法处理的非线性数据结构

## 问题背景

你有一个每个样本有 784 个特征的数据集。也许是手写数字的像素值。也许是基因表达水平。也许是用户行为信号。你无法可视化 784 个维度。你无法绘制它们。你甚至无法想象它们。
