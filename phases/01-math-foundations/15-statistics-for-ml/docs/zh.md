...
> 统计学让你知道你的模型是真的有效还是只是运气好。

**类型：** 动手
**语言：** Python
**前置要求：** 阶段 1，课程 06（概率与分布），07（贝叶斯定理）
**时长：** 约 120 分钟

## 学习目标

- 从零计算描述性统计、Pearson/Spearman 相关和协方差矩阵
- 执行假设检验（t 检验、卡方检验）并正确解释 p 值和置信区间
- 使用 bootstrap 重采样为任何指标构建置信区间，无需分布假设
- 使用效应量度量区分统计显著性和实际显著性

## 问题背景

你训练了两个模型。模型 A 在你的测试集上得分 0.87。模型 B 得分 0.89。你部署了模型 B。三周后，生产指标比以前更差。发生了什么？
