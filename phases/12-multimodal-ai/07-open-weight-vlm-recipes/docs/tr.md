...
- Bir MM1 / Idefics2 / Kambriyen-1 ablasyon tablosunu okuyun ve hangi düğmenin belirli bir benchmark'yi hareket ettirdiğini tahmin edin.
...
Yüzlerce açık ağırlıklı VLM mevcuttur. "İyi" ile "en gelişmiş" arasındaki farkın çoğu mimari değildir. Veri, çözünürlük programı ve kodlayıcı seçimidir. Modeliniz düşük performans gösterdiğinde ilk önce hangi düğmeyi çevireceğinizi bilmek sizi 5 milyon GPU saatlik hatadan kurtarır.
...
Kambriyen-1'in "Kambriyen Görüş Kodlayıcıları Eşleştirmesi" (Tong ve diğerleri, 2024), görüş merkezli bir benchmark (CV-Bench) üzerinde 20'den fazla kodlayıcı çalıştırdı. Skor tablosunun üst kısmı DINOv2 ve SigLIP'in bir karışımıdır; CLIP grubun ortasında yer alıyor; ImageBind ve ViT-MAE daha düşüktür. CLIP ViT-L ile DINOv2 ViT-g/14 arasındaki fark CV-Bench'te ~5-7 puandır.
...
MM1, Idefics2, Prismatic ve MM-Interleaved'in hepsi aynı sonuca ulaştı: sabit bir görsel-token sayısında bağlayıcı mimarisinin pek önemi yok. Ortalama havuzlu yamalardaki 2 katmanlı bir MLP, aynı token bütçeyle 32 sorgulu Q-Former'ın 1 noktası dahilinde performans gösterir.

Önemli olan token sayısıdır. Daha fazla görsel tokens = daha fazla LLM hesaplaması = bir noktaya kadar daha iyi performans, ardından getirilerin azalması. Görüntü başına 64 token, OCR için çok az. 576-1024 token'ler çoğu açık VLM için en uygun noktadır. 2048+ yalnızca belgeler ve grafikler için yardımcı olur.

Q-Former ve MLP bir kalite sorusu değil, bir maliyet sorusudur: Q-Former, görüntü çözünürlüğüne bakılmaksızın token'leri 32-64'te sınırlar; MLP tüm yama token'ları yayar. Yüksek çözünürlüklü girişler için Q-Former, LLM içeriğini kaydeder; düşük çözünürlükler için fark gürültüdür.

### Eksen 3: LLM boyutu tavanı belirliyor

LLM'yi 7B'den 13B'ye iki katına çıkarmak, her VLM belgesinde MMMU'ya güvenilir bir şekilde 2-4 puan ekler. 70B'de çoğu benchmark'yu doyurursunuz. VLM'nin çok modlu akıl yürütme tavanı, LLM'nin metin akıl yürütme tavanıdır; görsel kodlayıcı, bunun nedenini değil, yalnızca onu besleyebilir.

Bu nedenle Qwen2.5-VL-72B ve Claude Opus 4.7, MMMU-Pro ve ScreenSpot-Pro'yu eziyor: dil beyni çok büyük. Akıllı konnektör tasarımı sayesinde 7B VLM, 70B VLM'nin yerini alamaz.
...
Molmo-72B, 11 benchmarksn'nin 11'inde Llama-3.2-90B-Vision'ı yendi. Delta mimari değil, altyazı kalitesidir. Ayrıntılı insan altyazıları, kısa web altyazılarına göre görüntü başına 5-10 kat daha fazla bilgi içerir ve GPT-4V damıtma halüsinasyonlarının olduğu yerde gerçeklere dayalı kalır.
...
Idefics2'nin ablasyonları: 384 -> 448, 1-2 puan ekler. Görüntü bölmeli (AnyRes) 448 -> 980, OCR benchmark'lara 3-5 tane daha ekler. Orta doğrulukta düz çözünürlüklü eğitim platoları; çözünürlük artışı (başlangıç ​​224, bitiş 448 veya yerel) daha hızlı eğitir ve daha yüksekte biter.

Kambriyen-1, token'nin dengelemesine karşı bir çözünürlük çalıştırdı: sabit hesaplamada, daha düşük çözünürlükte daha fazla token'ye veya daha yüksek çözünürlükte daha az token'ye sahip olabilirsiniz. OCR için daha yüksek çözünürlük kazanır; genel sahneyi anlamak için daha düşük-res-more-tokens kazanır.
...
- Resim başına görsel-token sayısı varyansın ~%60'ını açıklar.
...
- Bağlayıcı: token yamasında 2 katmanlı MLP. token-sınırlı olmadığınız sürece Q-Former'ı atlayın.
...
- Program: Aşama 1 hizalama (yalnızca projektör), Aşama 2 tam fine-tuning, Aşama 3 göreve özel fine-tuning.
...
Çıktı, beklenen benchmark deltayı ve "önce ablate" önerisini içeren sıralanmış bir tarif listesidir.

## Gönderin

Bu ders `outputs/skill-vlm-recipe-picker.md` üretir. Bir hedef görev karışımı, bir işlem bütçesi ve bir gecikme hedefi göz önüne alındığında, her seçimi gerekçelendiren ablasyona ilişkin alıntılarla birlikte tam bir tarif (kodlayıcı, bağlayıcı, LLM, veri karışımı, çözümleme planı) yayınlar. Her yeni VLM projesi başladığında mühendislerin Idefics2 ablasyon tablosunu yeniden keşfetmesini engeller.
...
2. Kambriyen-1, DINOv2 + SigLIP'in birleştirilmesinin, görüş merkezli benchmark'lerde tek başına daha iyi performans gösterdiğini ancak MMMU'ya sinyal eklemediğini buldu. Hangi benchmark'nin kazanacağını ve hangilerinin sabit kalacağını tahmin edin.

3. Hedefiniz, 2B LLM'de bir mobil kullanıcı arayüzü agent. Kodlayıcıyı, konektörü, çözünürlüğü ve veri karışımını seçin. Her seçimi belirli bir ablasyon tablosuyla gerekçelendirin.

4. Molmo 4B ve 72B modellerini gönderiyor. 4B, kapalı 7B VLM'lerle rekabet edebilir; 72B, 11/11 benchmarks'de Llama-3.2-90B-Vision'ı yener. Bu size LLM boyutunda plato hipotezi hakkında ne söylüyor?
...
| Ayrıntılı insan yazısı | "Yoğun başlık" | Bir web alternatif metninden daha zengin, çok cümleli, insan tarafından yazılmış bir açıklama (genellikle 80-300 tokens) |
