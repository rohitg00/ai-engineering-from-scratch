...
> InternVL3'ten önceki her açık VLM, aynı üç adımlı tarifi izledi: Trilyonlarca metin token üzerinde eğitilmiş bir metin LLM'si alın, bir görüntü kodlayıcıyı takın ve ardından dikişlere ince ayar yapın. Bu işe yarıyor ancak hizalama borcu var - metin LLM ön eğitim bütçesinin tamamını saf metin üzerinde harcadı ve görsel token'leri yerel olarak anlamıyor. Post-hoc vizyonu eklediğinizde, LLM'nin metni unutmadan görsel girdiyi metin muhakemesi ile nasıl ilişkilendireceğini yeniden öğrenmesi gerekir. InternVL3 (Zhu ve diğerleri, Nisan 2025) post-hoc yaklaşımı reddeder: bir ön eğitim çalışması, birinci adımdan itibaren metin ve çok modlu serpiştirilmiş. Sonuç, 78B parametreleri açıkken MMMU-Pro'daki Gemini 2.5 Pro ile eşleşiyor. Bu derste yerel ön eğitimin durumu ve bunu yaptığınızda nelerin değiştiği anlatılmaktadır.
...
- Ölçülebilir üç semptomu (felaket boyutunda unutma, cevap kayması, görsel metin tutarsızlığı) öne sürerek post-hoc VLM eğitiminin neden uyum borcu biriktirdiğini açıklayın.
...
- Görsel Çözünürlük Yönlendiricisi (ViR) ve Ayrılmış Görüntü Dili (DvD) deployment optimizasyonlarını adlandırın.
...
1. Frozen LLM + frozen view kodlayıcı + eğitilebilir projektör, embedding'leri hizalamak için altyazı çiftleri konusunda eğitilmiştir.
2. LLM'yi çözün, talimat verileri üzerinde eğitim alın (LLaVA-Instruct, ShareGPT4V).
3. İsteğe bağlı göreve özel ince ayar.

Uyum borcunun üç belirtisi ortaya çıkıyor:

- Felaket derecede unutma. Post-hoc VLM salt metin becerilerini unutur. GSM8K puanları 5-10 puan düşüyor. Hellaswag puanları düşüyor. Saf metin agent'ler geriler.
...
- %35 aralıklı görüntü-metin verileri (OBELICS, MMC4 tarzı)
- %20 eşleştirilmiş resim yazısı verisi
- %5 video-metin verisi

Vision token'ler, metin token'ler ve modlar arası etkileşimlerin tümü, ilk gradient adımından itibaren aynı kayba katılır. Hizalama ön eğitimi yok, projektörün donma aşaması yok, toparlanmayı gerektiren feci bir unutma yok.
...
V2PE'nin ablasyon iddiası: Aynı hesaplamada M-RoPE üzerinden benchmark videolarında 1-2 puan. Bir devrim değil, daha temiz.

### Görsel Çözünürlük Yönlendiricisi (ViR)

Deployment optimizasyonu. Tüm görüntülerin tam çözünürlüklü kodlamaya ihtiyacı yoktur. Düşük ayrıntıya sahip tek nesne içeren bir fotoğraf, 1280 piksel yerel çözünürlükte kodlandığında token'leri boşa harcar. ViR, kodlamadan önce soruyu yanıtlamak için gereken minimum çözünürlüğü tahmin eden küçük bir sınıflandırıcıdır.

Yönlendirmenin üç katmanı vardır: düşük çözünürlüklü (256 token), orta (576), yüksek (2048+). Üretim trafiğindeki sorguların %60'ı için düşük veya orta yeterlidir. Net etki: Eşit kalitede 2-3 kat verim.

### Ayrık Görüş-Dil deployment (DvD)

Büyük bir VLM'ye hizmet verdiğinizde görüntü kodlayıcı görüntü başına bir kez çalışır ancak LLM her çıktı token için otomatik regresif olarak çalışır. İki bileşenin farklı darboğazları vardır (görüş = dönüşüm + dikkat için GPU bellek bant genişliği; LLM = KV önbellek). DVD, bunları aralarında akış olacak şekilde ayrı GPU'lara böler.
...
Uyum-borç hipotezi ölçülebilir: InternVL3-8B, görüş birimi-benchmark kazancı başına Qwen2.5-VL-7B'ye kıyasla daha az metin-benchmark puanı (MMLU, GSM8K) kaybediyor. Model daha çok genelcidir çünkü eğitim iki parça değil tek parçadır.
...
- Veri. Geniş ölçekte aralıklı resim-metin bütünleri nadirdir. OBELICS 141M belgedir; MMC4 571M'dir. Yalnızca metin 15T token'de gönderilir. Çok modlu ön eğitim veri kıtlığı zorlu bir kısıtlamadır.
...
`code/main.py` bir eğitim derlem karıştırıcısı ve ViR yönlendirici simülatörüdür. Bu:
...
Bu ders `outputs/skill-native-vs-posthoc-auditor.md`'yi üretir. Önerilen bir VLM eğitim planı göz önüne alındığında, yerel mi yoksa post-hoc mu gidileceğini denetler, uyum-borç riskini işaretler ve bir derlem karışımı önerir. Yeni bir açık VLM projesini boyutlandırırken ve eğitim stratejisini seçmeniz gerektiğinde bunu kullanın.
...
3. Unutmayla ilgili MM1.5 Bölüm 4'ü okuyun. Post-hoc eğitimin en büyük gerilemeyi gösterdiği benchmark'yi tam olarak adlandırın. Regresyonun maliyeti ne kadar oldu?
...
|------|-----------------|------------------------|
| Yerel çok modlu ön eğitim | "Sıfırdan birlikte" | Metin + resim + video token'ler 1. adımdaki kayba katılır, daha sonra eklenmez |
...
| Vir | "Çözünürlük yönlendiricisi" | Kodlamadan önce sorgu başına gereken minimum çözünürlüğü seçen ve inference token'leri kaydeden küçük sınıflandırıcı |
