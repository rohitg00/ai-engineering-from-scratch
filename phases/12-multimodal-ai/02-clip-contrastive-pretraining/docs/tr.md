...
- CLIP / SigLIP ön eğitiminin size sağladığı dört kolu adlandırın: toplu iş boyutu, sıcaklık, prompt şablon, veri kalitesi.

## Sorun

CLIP öncesi görüş denetlendi. Etiketli dataset'leri toplayın (ImageNet: 1,2 milyon görüntü, 1000 sınıf), bir CNN'yi eğitin ve gönderin. Etiketler pahalıdır, etiketler etiketleyicilerin üzerinde anlaşabileceği hususlara göre önyargılıdır ve etiketler, fine-tuning yapılmadan yeni görevlere aktarılmaz.

Resim yazısı webinde ücretsiz olarak bir milyardan fazla gevşek etiketli çift bulunur. "Köpeğim Max parkta" alternatif metnine sahip bir Golden Retriever'ın resmi, denetleyici bir sinyal taşıyor; metin, görüntüyü tanımlıyor. Soru şu: Bunu yararlı bir eğitime dönüştürebilir misiniz?
...
- Görüntü kodlayıcı `f`: ViT veya ResNet, görüntü başına bir D-dim vektörü üretir.
- Metin kodlayıcı `g`: küçük transformer, başlık başına bir D-dim vektörü üretir.

Her iki kule de çıktılarını birim uzunluğa göre normalleştirir. Her ikisi de birim norm olduğundan benzerlik `cos(f(x), g(y)) = f(x)^T g(y)` olur.

N (resim, resim yazısı) çiftinden oluşan bir grup için, `(N, N)` şeklinin benzerlik matrisini `S` oluşturun:

```
S[i, j] = cos(f(x_i), g(y_j)) / tau
```

burada `tau` öğrenilen bir sıcaklıktır (CLIP 0,07'ye başlar; günlük alanında öğrenilir).
...
SigLIP, softmax'ı eleman bazında sigmoid ile değiştirir: her `(i, j)` çifti için kayıp, "bunlar eşleşen çift mi?" şeklinde ikili bir sınıflandırmadır. pozitif sınıf etiketleri köşegendir, geri kalan her şey negatiftir. Kayıp şu:

```
L = -1/N sum over (i, j) [ y_ij log sigmoid(S[i,j]) + (1-y_ij) log sigmoid(-S[i,j]) ]
```

`y_ij = 1` eğer `i == j`, değilse 0. Her çiftin kaybı bağımsızdır. Hepsinin toplanmasına gerek yok. Her GPU kendi yerel bloğunu ve toplamlarını hesaplar. SigLIP 2, CLIP'in orantılı olarak daha fazla iletişime ihtiyaç duyacağı 32k-512k'yi toplu olarak ucuza ölçeklendirir.
...
Prompt şablonları önemlidir. CLIP'in orijinal makalesinde sınıf başına 80 şablon (sade, sanatsal, fotoğraf, resim vb.) kullanılmış ve embedding'ların ortalaması alınmıştır. +3 ImageNet puanı. Modern kullanım genellikle bir veya iki şablonu seçer.

### Doğrusal problar ve fine-tuning

Sıfır atış bir temeldir. Doğrusal bir araştırma (hedef sınıflarınız için donmuş CLIP özelliklerinin üstüne bir doğrusal katman eğitin), etki alanı içi görevlerde sıfır atıştan üstündür. Tam fine-tuning, alan içi doğrusal probu yener ancak sıfır atış aktarımına zarar verebilir. Üç değiş-tokuşu olan üç rejim.
...
2026 açık VLM'lerde SigLIP 2 SO400m/14 varsayılan vision towerdir. CLIP, belirli LAION-2B eğitim dağıtımının sorgu düzeninizle eşleştiği durumlarda saf görüntü metni alımı için varsayılan olarak kalır.
...
CLIP sınıfı modeller yaklaşık %76 ImageNet sıfır çekimini (CLIP-G, OpenCLIP-G) sınırlar. Ötesi ya çok daha büyük veriler (SigLIP 2 %80+ alır) ya da mimari değişiklikler (denetlenen kafalar, daha fazla parametre) gerektirir. benchmark doyuma ulaşıyor; gerçek değer, aşağı yöndeki VLM'lerin tükettiği embedding alanıdır.
...
4. Sıfır atışlı bir sınıflandırma rutini: tahmin için argmax olan prompt metin kümesine karşı kosinüs benzerliğini hesaplayın.

Çalıştırın ve kayıp eğrisini izleyin. Mutlak sayılar oyuncaktır; şekli gerçek bir CLIP antrenörünün yaydığı şekle uyuyor.

## Gönderin

Bu ders `outputs/skill-clip-zero-shot.md` üretir. Bir dizi görüntü (yol yoluyla) ve bir hedef sınıf listesi verildiğinde, CLIP şablonuyla prompt metinleri oluşturur, her iki tarafı da belirtilen bir kontrol noktasıyla (e.g., `openai/clip-vit-large-patch14`) gömer ve benzerlik puanlarıyla ilk 1 / ilk 5 tahminleri döndürür. Beceri, prompt listesinde olmayan sınıflar hakkında hak talebinde bulunmayı reddeder.
...
2. SigLIP sıcaklığa ek olarak bir eğilim parametresi `b` kullanır: `S'[i,j] = S[i,j]/tau + b`. Grupta büyük bir sınıf dengesizliği (satır başına pozitiflerden çok negatifler) olduğunda `b` nasıl bir rol oynar? SigLIP Bölüm 3'ü okuyun (arXiv:2303.15343).
...
|------|----------------|------------------------|
| BilgiNCE | "Karşılaştırmalı kayıp" | Bir partinin benzerlik matrisi üzerinden çapraz entropi; her öğenin pozitifi, eşleştirilmiş öğesidir, negatifler ise diğer her şeydir |
| Sigmoid kaybı | "SigLIP kaybı" | Çift başına ikili çapraz entropi; softmax yok, hepsi bir arada, dağıtılmış eğitimde ucuza ölçekleniyor |
| Sıcaklık | "tau" | Softmax/sigmoid'den önce logitleri ölçeklendiren skaler; dağıtımın keskinliğini kontrol eder |
| Sıfır atış | "fine-tuningsız sınıflandırma" | embedding sınıflarını oluşturmak ve kosinüs benzerliğine göre sınıflandırmak için prompts metnini kullanın; hedef sınıflara ilişkin eğitim yok |
| Prompt şablon | "bir fotoğraf ..." | Bir sınıf adının etrafındaki metin iskelesi; sıfır atış doğruluğunu 1-5 puan etkiler |
...
| Sıcaklık ölçeklendirme | "log parametreli tau" | CLIP, `log(1/tau)`'yi gradient'ların davranacağı şekilde parametreleştirir; sıfıra yakın tau'ya çökmeyi önleyen klipsler |
...
- [Jia ve diğerleri. — ALIGN (arXiv:2102.05918)](https://arxiv.org/abs/2102.05918) — gürültülü web verileriyle ölçekleme.
