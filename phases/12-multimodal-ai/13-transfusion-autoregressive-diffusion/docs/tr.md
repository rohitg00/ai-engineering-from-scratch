...
> Chameleon ve Emu3 her şeyi ayrı token'lere yatırıyor. Çalışıyorlar, ancak nicemleme darboğazı görülebiliyor; sürekli uzay difüzyon modellerinin altındaki görüntü kalitesi platoları. Transfüzyon (Meta, Zhou ve diğerleri, Ağustos 2024) ise tam tersini yapıyor: görüntüleri sürekli tutun, VQ-VAE'yi tamamen bırakın ve bir transformer'yi iki kayıpla eğitin. Metin token'ler bir sonraki token tahminini alır. Görüntü yamaları akış eşleştirme/yayılma kaybı yaşar. Her iki hedef de aynı ağırlıkları optimize eder. Kararlı Difüzyon 3'ün (MMDiT) altında yatan mimari yakın bir kuzendir. Bu derste Transfüzyon tezi okunur, oyuncak iki kayıplı eğitici yapılır ve bir transformer'nin her iki işi de yapmasına olanak tanıyan dikkat maskesinin izini sürer.
...
- Tek bir omurgada iki kayıp (metin token'lerde NTP, görüntü yamalarında difüzyon MSE) çalıştıran bir transformer bağlayın.
...
token'nin ayrık ve sürekli görüntü arasındaki tartışması Yüksek Lisans'tan daha eskidir. Sürekli gösterimler (ham pikseller, VAE gizlileri) ayrıntıları korur. Ayrık token'ler (VQ endeksleri), transformer'nin yerel sözlüğüne uyar ancak niceleme adımında ayrıntıları kaybeder.
...
Transfüzyon şunu sorar: İkisini de alabilir miyiz? Görüntüleri sürekli tutun, yine de bir modeli eğitin, tek bir gradient adımında birleştirilmiş iki kayıp kullanın.
...
- Metin token'ler (ayrı, BPE kelime haznesinden).
- Görüntü yamaları (doğrusal embedding aracılığıyla gizli karartmaya yansıtılan sürekli, 16x16 piksel bloklar — ViT kodlayıcının girişiyle aynı).
- `<image>` ve `</image>` etiketleri, sürekli yamaların nerede bulunduğunu işaretler.

İleri pas bir kez çalışır. Kayıp token başına iki kafadan birini seçer:

- tokens metni için: kelime bilgisi-logit kafasındaki standart çapraz entropi.
- Görüntü yamaları için: sürekli yamalardaki yayılma kaybı — her yamaya eklenen gürültüyü tahmin edin.

gradient, paylaşılan transformer gövdesi üzerinden akar. Her iki kayıp da paylaşılan ağırlıkları eş zamanlı olarak iyileştirir.
...
Eğitimde ve inference'de blok üçgen maske olarak uygulandı.
...
3. transformer v_theta(xt, t); kayıp = MSE(v_theta(xt, t), ε - x0).
4. Aynı dizideki metin NTP kayıplarının yanında Backprop.

inference'de üretim şu şekildedir:
- Metin tokens: standart otoregresif örnekleme.
...
Stabil Difüzyon 3 (Esser ve diğerleri, Mart 2024), MMDiT'yi (Multimodal Diffusion Transformer) Transfüzyon ile hemen hemen aynı zamanlarda piyasaya sürdü. Mimarlar kardeştir.
...
- Düzeltilmiş akış eğitimi. Bilinen örnekleme ve DDPM'den daha basit matematik ile belirli bir akış eşleştirme çeşidi.
- Ölçek. MMDiT, SD3'ün (2B ve 8B param varyantları) omurgasıdır. Transfüzyonun kağıdı 7B'ye ölçekleniyor.

Her ikisi de aynı temel fikir üzerinde birleşiyor: bir transformer, metin üzerinde NTP'yi çalıştırıyor ve sürekli görüntü temsilleri üzerinde difüzyonu çalıştırıyor.
...
- Inference, otoregresif görüntü token'lerin aksine görüntü yaması gürültüsünü paralel hale getirebilir.
...
Janus-Pro (Ders 12.15), transformer gövdesini paylaşırken, anlama ve oluşturma için görüntü kodlayıcıyı (biri için SigLIP, diğeri için VQ) ayırarak Transfusion'ın fikrini geliştiriyor. Show-o (Ders 12.14), difüzyonu ayrık difüzyonla değiştirir (maskeli tahmin). Birleşik nesil ailesi Transfüzyondan sonra hızla dallanır.
...
- Bir çift paylaşılan ağırlıklı doğrusal projeksiyon, transformer'nin vekili görevi görür; Metinde NTP kaybı, gürültülü yamalarda MSE kaybı.
...
transformer bir oyuncaktır. İki kayıplı tesisat, dikkat maskesi yapısı ve inference döngüsü gerçek artifact'lerdir.

## Gönderin

Bu ders `outputs/skill-two-loss-trainer-designer.md`'yi üretir. Yeni bir çok modlu eğitim görevi (metin + görüntü, metin + ses, metin + video) verildiğinde, iki kayıp programını (kayıp ağırlıkları, maske şekli, paylaşılan ve modaliteye özgü bloklar) tasarlar ve uygulama risklerini işaretler.
...
3. MMDiT'in modaliteye özgü QKV ağırlıkları vardır. Bu, Transfusion'ın tam paylaşımlı transformer'sine kıyasla hangi parametre sayımı yükünü artırıyor? 7B paramlarında buna değer mi?

4. Nesil: prompt metni verildiğinde, model 50 token için NTP'yi çalıştırır, ardından `<image>`'ye ulaşır ve ardından 20 gürültü giderme adımı üzerinden 256 yama üzerinde difüzyonu çalıştırır. Toplam kaç ileri pas var?

5. SD3 makalesini okuyun Bölüm 3. Düzeltilmiş akışı ve neden DDPM'den daha az inference adımında birleştiğini açıklayın.
...
| Blok-üçgen maske | "Nedensel metin + çift yönlü resim" | Metin genelinde nedensel olan ancak resim bölgelerinde çift yönlü olan dikkat maskesi |
