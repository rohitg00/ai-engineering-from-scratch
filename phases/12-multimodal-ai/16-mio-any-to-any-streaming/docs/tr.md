...
> GPT-4o, çoğu açık modelin kopyalayamayacağı bir ürün sunar: gerçek zamanlı olarak sesi duyan, videoyu gören ve yanıt veren bir agent. 2024 sonlarına doğru açık ekosistemin cevabı MIO idi (Wang ve diğerleri, Eylül 2024). MIO tokenmetni, görüntüyü, konuşmayı ve müziği birleştirir, bir nedensel transformer'yi aralıklı diziler üzerinde eğitir ve herhangi bir modalite için herhangi bir modalite üretir. AnyGPT (Zhan ve diğerleri, Şubat 2024) konseptin kanıtıydı; MIO ölçek büyütmedir; Unified-IO 2 (Allen AI, Aralık 2023), vizyon + eylem temelinin kuzenidir. Bu ders herhangi birinden herhangi birine modelini okur — dört tokenizer, bir transformer, akış dostu kod çözme.
...
- Metin, resim, konuşma ve müzik token'lerini çarpışma olmadan barındıran paylaşılan bir kelime dağarcığı tasarlayın.
- Sıkıştırma + yeniden yapılandırma ödünleşimlerinde SEED-Tokenizer (görüntüler) ve KonuşmaTokenizer artık-VQ (konuşma)'yı karşılaştırın.
- Herhangi bir kuşağı oluşturan dört aşamalı müfredatı açıklayın.
- Herhangi birinden herhangi birine açık olan üç tarifi ve bunların ana tercihlerini adlandırın: MIO, AnyGPT, Unified-IO 2.
...
- Tokenizer'ler her modalite için mevcut olmalı, yeniden yapılandırma için kayıpsız olarak yeterince sıkıştırılmalı ve transformer'nın tüketebileceği oranlarda token'ler üretmelidir.
...
- Inference, konuşma gecikmesi için (<500 ms ilk ses baytına kadar geçen süre) token çıkışını yeterince hızlı akışla yapmalıdır.
...
- Resim: SEED-Tokenizer (2023) — ayrı kod kitabına sahip nicemlenmiş VAE, 4096 giriş, görüntü başına 32x32 tokens.
- Konuşma: KonuşmaTokenizer artık-VQ (2023) — 16kHz dalga biçimini 8 hiyerarşik kod kitabına kodlar; ilk seviye kaba içeriktir, sonraki seviyeler ise prozodi ve konuşmacı kimliğini ekler.
- Müzik: benzer artık-VQ (Meta'nın MusicGen / Encodec ailesi), 4-8 kod kitabı.

Her modalite tokens tamsayısını üretir. token'ler, paylaşılan sözlükte ayrık kimlik aralıkları alır:
...
Konuşma oluşturma artık-VQ'yu kullanır. transformer, temel (katman 0) konuşmayı token'ları tahmin eder; paralel kodu çözülmüş bir artık niceleyici, sonraki katmanları tahmin eder. Her katman 0 token, 16kHz'de kabaca 50ms sestir.

Akış düzeni:

1. Kullanıcı mikrofona konuşur; gerçek zamanlı ses tokenizer her 50 ms'de bir konuşma token yayar.
2. MIO, token'ları ulaştıkça tüketir (prompt ön doldurma + artımlı ileri).
3. token akışının oluşturulduğu şekilde çıktısını alın; paralel bir konuşma kod çözücü, bunları ~50-150 ms gecikme süresiyle ses örneklerine dönüştürür.
4. İlk ses baytına kadar geçen süre: MIO kağıdında ~300-500 ms, GPT-4o'nun ~250 ms'sine yaklaşıyor.
...
1. Aşama 1 – hizalama. Büyük ölçekli modalite çifti bütünlemi: metin-görüntü, metin-konuşma, metin-müzik. Her çift kendi token kelime segmentini kullanır. Paylaşılan kelime dağarcığını eğitir.
...
MIO, görsel düşünce zincirini devreye sokar: model, bir akıl yürütme adımı olarak ara görüntü token'leri yayar. "Kedi ağaca mı tırmanıyor?" modeli:

1. Sahneyi oluşturan `<image>` tokens'yi yayar (giriş görüntüsünden veya bir çizimden).
2. Çizimi analiz eden metni yayınlar.
3. Son yanıtı verir.

Oluşturulan ara görüntü, karalama defteri görevi görür. Benchmarkuzaysal akıl yürütme görevlerinde gelişme gösterir. Fikir, metin muhakemesi için düşünce zincirini yansıtıyor.
...
MIO, saf-token herhangi birine en yakın olanıdır. AnyGPT onun kavramsal atasıdır.

### Gecikme bütçesi

Konuşmaya dayalı bir ürün için her bileşenin gecikmesi önemlidir:

- Mikrofondan sese tokens: ~50 ms.
- Önceden doldurma (ses tokens + geçmiş): 8B modelinde ~100 ms.
- İlk çıkış token: ~50ms.
- Paralel artık-VQ + konuşma kod çözücü: ~100-150ms.

İlk ses baytına kadar toplam süre: ~300 ms minimum. GPT-4o ~250ms olduğunu iddia ediyor. Moshi 160ms olduğunu iddia ediyor. MIO/AnyGPT, genel benchmark'ler başına 400-600 ms aralığındadır.
...
- Multimodal girişlerin (metin, resim, ses klibi, müzik) listesini tokenizer yönlendirici üzerinden yönlendirir.
...
Bu ders `outputs/skill-any-to-any-pipeline-auditor.md` üretir. Konuşmaya dayalı bir ürün spesifikasyonu verildiğinde (modaliteler içeri, modaliteler dışarıda, gecikme hedefi), MIO ailesi tasarım seçeneklerini denetler ve gecikme bütçesini hesaplar.
...
2. KonuşmaTokenizer artık-VQ 8 kod kitabı kullanır. Artık seviyelerin paralel kodunun çözülmesinin neden gerekli olduğunu (sıralıya karşı) ve bunun gecikmeden ne gibi tasarruflar sağladığını önerin.

3. Kelime dağarcığınızın 32k metin + 4k resim + 4k konuşması var. 8k müzik ve ~10 ayırıcı ekleyin. Gizli dim 4096'daki embedding-matrix parametresinin maliyeti nedir?

4. Görsel düşünce zinciri bir ara görüntü yayar. Ne tür sorular işe yarar? Fazladan token'lardan ne tür zararlar var?
...
| Artık-VQ | "Konuşma tokenizer yığını" | Her katmanın bilgi eklediği çoklu kod kitabı tokenoluşturma; temel katman içeriktir, sonraki katmanlar prozodidir |
...
| İlk ses baytına kadar geçen süre | "TTFAB" | Kullanıcı sesinden ilk ses çıkışına kadar gecikme; Konuşma hissi için <500ms |
