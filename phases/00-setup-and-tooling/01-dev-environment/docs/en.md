...
The fnm installer downloads a zip archive and stops with `unzip: command not found` when `unzip` is missing. macOS ships it; Ubuntu, Debian, and WSL2 get it from the Step 1 apt line (`sudo apt install -y unzip` if you skipped that step).
