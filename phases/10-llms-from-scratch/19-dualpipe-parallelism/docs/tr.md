...
- 8 PP kademesi ve 16 mikro parti için bir DualPipe programını elle takip edin ve ileri ve geri akışların birbirlerinin boş yuvalarını doldurduğunu doğrulayın.
...
3. **Düğümler arası hepsi bir arada.** Uzman paralelliğine sahip MoE, uzmanları düğümler arasında dağıtır. Her ileri geçiş, token'lerin uzmanlarına gönderilmesini ve bir başkasının birleştirilmesini tetikler. 2k GPU'larda bu kolaylıkla 1:1 hesaplama-iletişim oranına dönüşür.

Bunların her birinin ayrı çözümleri vardır: Bellek için gradient kontrol noktası oluşturma, boru hattı baloncukları için Zero Bubble (Sea AI Lab, 2023), hepsi bir arada için uzman-paralel iletişim çekirdekleri. DualPipe'ın yaptığı şey onların birlikte oynamasını sağlamaktır. Zamanlama, tek bir ileri-geri yığın içinde bilgi işlem ve iletişimi örtüştürür, ardışık düzenin her iki ucundan aynı anda mikro gruplar enjekte eder ve sonuçta ortaya çıkan zamanlamayı, hesaplama pencerelerinin içinde hepsi bir arada gizlemek için kullanır.
...
N katmanlı bir modeli P aygıtlarına bölün. `i` cihazı `i * N/P .. (i+1) * N/P - 1` katmanlarını tutar. Bir mikro yığın, cihazlar 0'dan P-1'e ileri doğru, ardından P-1'den 0'a geri doğru akar. Her cihaz ileri aşamasına ancak önceki cihaz çıkışını gönderdiğinde başlayabilir ve yalnızca aşağı akış cihazı yukarı akış gradient'yi gönderdiğinde geriye doğru başlayabilir.

GPipe (Huang ve diğerleri, 2019) tek seferde bir mikro partiyi planlar ve bu da GPU zamanının çoğunu boşa harcar. 1F1B (Narayanan ve diğerleri, 2021), birden fazla mikro parti için ileri ve geri geçişleri serpiştirir. Zero Bubble (Qi ve diğerleri, 2023), geri geçişi iki parçaya ayırır: girdi için geriye doğru (B) ve ağırlıklar için geriye doğru (W) ve bunları balonu dolduracak şekilde programlar. Zero Bubble'dan sonra boru hattı neredeyse sıkı.
...
- **Tümden herkese gönderim.** token'leri uzmanlarına gönderen düğümler arası iletişim.
- **MLP.** MEB uzman hesaplaması.
- **Hepsi bir arada.** Uzman çıktılarını geri getiren düğümler arası iletişim.

Geriye doğru bir yığın, bunların her birinin gradient versiyonlarını ekler. DualPipe bunları, hepsine gönderimin bir sonraki parçanın dikkat hesaplamasına paralel olarak gerçekleşmesini ve hepsinden hepsine birleştirmenin de sonraki parçanın MLP hesaplamasına paralel olarak gerçekleşmesini sağlayacak şekilde programlar.
...
Bunun çalışması için, `i` cihazının HEM erken ardışık düzen katmanını `i` HEM DE son ardışık düzen katmanını `P - 1 - i` tutması gerekir. Bu, DualPipe'ın "ikili" kısmıdır: her cihaz, hizmet vermesi gereken model katmanlarının iki kopyasını (her yön için bir tane) tutar. DeepSeek-V3 ölçeğinde bu, parametre çoğaltma maliyetinin 2 katıdır. Ekonomiktir çünkü Uzman Paralelliği zaten MEB uzmanlarını o kadar ince yaymaktadır ki, uzman olmayan katmanları iki kez kopyalamak küçük patateslerdir.
...
### 14.8T-token çalıştırması ne anlama gelir?

DeepSeek-V3'ün ön eğitimi, yaklaşık 2,8 milyon GPU saatinde 2.048 H800 GPU'da 14,8 T token tüketti. Saf 1F1B ile bunun %12-15'ini boru hattı baloncukları nedeniyle kaybederlerdi; 340-420K GPU saati, tam bir 70B modelini eğitmeye yetecek kadar. DualPipe bunların çoğunu kurtardı. Katkıyı doğrudan ölçmek, dahili günlükler olmadan zordur, ancak makaledeki iddia, eğitim boyunca ortalama %95 GPU kullanımının üzerindedir.
...
- **ZeRO-3** gradient parçalamayla uyumludur. İki kopyalı çoğaltma için defter tutmanın ZeRO'nun parçalanmış gradient'leri ile işbirliği yapması gerekir.
...
`code/main.py` bir boru hattı zamanlama simülatörüdür. `(P, n_micro_batches, schedule)`'yi alır ve 1F1B, Zero Bubble, DualPipe ve DualPipeV'nin her biri için kararlı faz kullanımını yazdırır. Bu bir öğretim aracıdır; sayılar, makalelerdeki nitel iddialarla örtüşmektedir; bunlar, üretimin ölçülen hızlanmasıyla ilgili bir iddia değildir.
...
Bu ders `outputs/skill-dualpipe-planner.md`'yi üretir. Bir eğitim kümesi spesifikasyonu (GPU sayısı, topoloji, ara bağlantı, model şekli) göz önüne alındığında, bir ardışık düzen paralellik stratejisi, kullanılacak planlama algoritması ve hedef ölçekte beklenen kabarcık oranı önerilir.

## Egzersizler

1. `(P=8, micro_batches=16, schedule=dualpipe)` ve `(P=8, micro_batches=16, schedule=1f1b)` üzerinde `code/main.py`'yi çalıştırın. GPU kullanım farkını hesaplayın ve bunu milyon token eğitim başına kurtarılan GPU saati olarak ifade edin.

2. `(P=4, micro_batches=8, schedule=dualpipe)` için zamanlama tablosunu elle çizin. Her zaman aralığını mikro parti kimliği ve yönü ile işaretleyin. Baloncukların olmadığı ilk zaman aralığını belirleyin.
...
|------|----------------|------------------------|
| Boru hattı balonu | "Seviye başına boşta kalma süresi" | Bir ardışık düzen aşamasının girişini beklemesi nedeniyle GPU döngüleri boşa gidiyor veya gradient |
...
| Hepsinden herkese gönderim | "token'leri uzmanlara gönderin" | token'leri atanmış MoE uzmanlarına yönlendiren çapraz düğüm iletişimi |
...
- [Qi ve ark. — Zero Bubble Boru Hattı Paralelliği (arXiv:2401.10241, Sea AI Lab 2023)](https://arxiv.org/abs/2401.10241) — Zero Bubble öncülü
...
- [Narayanan ve ark. — PipeDream / 1F1B (arXiv:1806.03377, 2018-2021)](https://arxiv.org/abs/1806.03377) — 1F1B programı DualPipe ile karşılaştırılır
- [Huang ve ark. — GPipe (arXiv:1811.06965, 2018)](https://arxiv.org/abs/1811.06965) — orijinal ardışık düzen paralelliği makalesi ve kabarcık sorunu
