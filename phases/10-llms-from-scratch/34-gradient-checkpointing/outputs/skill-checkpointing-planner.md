...
Refuse full checkpointing every layer when selective on attention alone closes the budget; the FLOP cost is 5x higher for the same memory. Refuse offload when measured PCIe bandwidth is under 16 GB/s effective; the activation transfer time dominates and recompute is faster. Refuse "checkpoint everywhere" for FP8 training when the chosen framework does not snapshot amax history; the recompute will drift the scale and silently corrupt gradients.
