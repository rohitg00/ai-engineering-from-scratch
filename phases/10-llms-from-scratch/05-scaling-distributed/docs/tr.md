...
- Minimum donanımı belirlemek için belirli bir model boyutu (ağırlıklar + optimize edici durumları + gradient'lar + aktivasyonlar) için bellek bütçesini hesaplayın
...
FP16'daki 7B parametreli modelin yalnızca ağırlıklar için 14 GB'a ihtiyacı vardır. Adam optimizer, her parametrenin (birinci ve ikinci moment tahminleri) iki ek kopyasını saklar. Bu başka bir 28GB. backpropagation sırasındaki Gradients, 14 GB daha ekler. Tek bir aktivasyon saklanmadan önce 56 GB'tasınız.

NVIDIA A100'ün 80 GB belleği vardır.

80GB'ın 56GB'ı tüketildi. Geriye aktivasyonlar için 24 GB kalıyor; ileri geçiş sırasında hesaplanan ve backpropagation boyunca canlı tutulması gereken ara değerler. 4096 boyutlu bir modele sahip bir 2048-token dizisi için, tek bir katmanın aktivasyonları yaklaşık 64MB kullanır. 32 katmanla örnek başına 2 GB'a ihtiyacınız vardır. 8'lik bir parti boyutu 16 GB gerektirir. 24 GB'ınız var. 12'lik bir parti büyüklüğü patlıyor.

Şimdi 70B parametrelerini deneyin. Yalnızca ağırlıklar: FP16'da 140 GB. Tek bir GPU'ya sığmaz. Ağırlıkları taşıyabilmek için en az 2 adet A100'e (2 x 80GB = 160GB) ihtiyacınız var. Optimize edici durumlarını ve gradient'leri eklediğinizde çok daha fazlasına ihtiyacınız olur: minimum 3'ten fazla GPU ve parçalama stratejisine bağlı olarak gerçekçi olarak 8-16 GPU.

Llama 3 405B, 16.384 NVIDIA H100 GPU'yla eğitildi. Eğitim çalıştırması, mimari (Uzmanların Karması, token başına parametrelerin yalnızca bir kısmının etkinleştirilmesi anlamına gelir) ve eğitim verimliliği konusunda akıllı olunması nedeniyle tahmini olarak $100 million in compute. DeepSeek V3 trained a comparable model for roughly $5,6 milyona mal oldu.

Bu ders, büyük ölçekli eğitimi mümkün kılan dört stratejiyi kapsar: veri paralelliği, tensör paralelliği, ardışık düzen paralelliği ve tamamen parçalanmış veri paralelliği. Dağıtılmış bir eğitime framework dokunmadan önce mekaniği anlamak için her birini saf Python'da simüle edeceksiniz.
...
| Modeli | Parametreler | Ağırlıklar (FP16) | Adam Devletleri | Gradients (FP16) | Toplam (aktivasyon yok) |
|-------|--------|----------------|-------------|------------------|----------------------|
| GPT-2 Küçük | 124M | 248 MB | 992 MB | 248 MB | 1,5GB |
| Llama 3 8B | 8B | 16GB | 64GB | 16GB | 96GB |
| Llama 3 70B | 70B | 140GB | 560 GB | 140GB | 840GB |
...
En basit dağıtılmış strateji. Modelin tamamını N GPU'lara kopyalayın. Her eğitim grubunu N eşit parçaya bölün. Her GPU, kendi veri parçasında ileri ve geri geçiş gerçekleştirir. Geriye doğru geçişten sonra, tüm GPU'lardaki gradient'lerin ortalamasını alın. Her GPU, ağırlıkların kopyasını aynı ortalama gradient'lerle güncelleyerek tüm kopyaları senkronize halde tutar.

**İyi yönleri:** Doğrusal aktarım hızı ölçeklendirmesi. N GPU'lar adım başına N kat daha fazla veri işler. İletişim, hesaplamayla örtüşen gradient ortalamayla sınırlıdır.

**Kötü tarafı:** Her GPU modelin, optimize edici durumlarının ve gradient'ların tam bir kopyasını içerir. 70B modeli için her GPU'nun 840 GB'a ihtiyacı vardır. Veri paralelliği, GPU başına belleği azaltmak için hiçbir şey yapmaz. Sadece eğitim süresini azaltır.

**Matematik:** Etkin toplu iş boyutu = per_gpu_batch_size x N. GPU başına toplu iş sayısı 16 olan N=64 GPU için etkin toplu iş 1.024'tür. Llama 3, adım başına 16 milyon tokens'lik etkin toplu iş boyutunu kullandı.
...
Bir ileri besleme katmanındaki şekil ağırlık matrisini (8192, 8192) düşünün. 4 yönlü tensör paralelliğiyle her GPU bir (8192, 2048) parçayı tutar. Her GPU, girişi kendi parçasıyla çarparak kısmi bir sonuç üretir. Kısmi sonuçlar, tam çıktıyı üretmek için birleştirilir (hepsini azaltma veya hepsini toplama yoluyla).

**İyi yönleri:** Model ağırlıkları için GPU başına belleği azaltır. 8 GPU'ya bölünmüş bir 70B modeli, her bir GPU'nun ~8,75B parametre değerinde ağırlık taşıdığı anlamına gelir.
...
Modeli katmanlara bölün. GPU 1, 1-8 arasındaki katmanları çalıştırır. GPU 2, 9-16. katmanları çalıştırır. GPU 3, 17-24. katmanları çalıştırır. GPU 4, 25-32. katmanları çalıştırır. Veriler ardışık düzen üzerinden akar: GPU 1, katmanlarını hesaplar ve etkinleştirmeleri GPU 2'ye gönderir, GPU da kendi katmanlarını hesaplar ve GPU 3'e gönderir, vb.

**İyi tarafı:** GPU'lar arasında minimum iletişim; yalnızca gradient'lara veya ağırlıklara kıyasla küçük olan katman sınırlarındaki aktivasyonlar. Bant genişliği gereksinimleri düşük olduğundan düğümler arasında çalışır.
...
FSDP, veri paralelliğinin ölçeklenebilirliğini parçalamanın bellek verimliliğiyle birleştirir. Her GPU modelin tam bir kopyasını tutmak yerine, her GPU parametrelerin, gradient'ların ve optimize edici durumlarının yalnızca 1/N'sini tutar.

Bir katmanın ileri geçişinden önce FSDP, tüm GPU'lardaki tüm parametreleri her bir GPU'nun belleğine toplamak için **hepsi toplama** işlemini çalıştırır. İleri geçişten sonra her GPU yerel olmayan parametreleri atar. Geriye doğru, tüm toplama işlemi gradient hesaplaması için parametreleri yeniden oluşturmak üzere yeniden çalışır. Geriye doğru geçişin ardından, **reduce-scatter** gradient parçayı dağıtır, böylece her GPU gradient'ların yalnızca 1/N'sini depolar.
...
| Gradients (FP16) | GPU başına 140 GB | GPU başına 17,5 GB |
...
Her katmandan önce tümünün toplanması nedeniyle iletişim maliyeti sıradan veri paralelliğinden daha yüksektir. Ancak bellek tasarrufu, daha önce imkansız olan eğitim çalıştırmalarını mümkün kılıyor.
...
| Sıfır-2 | + Gradients | ~8x azalma | Biraz daha |
...
| Hassasiyet | Ağırlıklar | Optimize Edici | Gradients | Toplam |
|-----------|---------|-----------|-----------|-------|
| FP32 her yerde | 28GB | 56GB | 28GB | 112GB |
| Karışık (BF16 + FP32 ana) | 14GB | 56GB | 14GB | 84GB |

Karma hassasiyet bu modelde 28 GB tasarruf sağlar. Optimize edici durumları ne olursa olsun FP32'de kalır; belleğin çoğunun gittiği yer burasıdır.
...
DeepSeek V3 farklı bir yaklaşım benimsedi. Uzman Karması mimarisi, token başına 671B parametreden yalnızca 37B'sini etkinleştirir. Bu, her GPU'nun yalnızca aktif parametreleri hesaplaması (ve aktivasyonları saklaması) gerektiği anlamına gelir. $5.6M vs Meta's estimated $100 milyon boyunca 2.048 H800 GPU (Meta'nın GPU sayısının 1/8'inden az) üzerinde eğitim aldılar.
...
Bir grubu simüle edilmiş GPU'lara bölün. Her GPU, kendi parçası üzerinde bir ileri geçiş hesaplar. "gradient'ların" ortalamasını alın (bunları kayıp değerleri olarak simüle ederiz).
...
Tümünü azaltma işlemi (ortalama gradients) veri paralelliğinde tek iletişimdir. Pratikte bu, NVIDIA GPU'lar üzerindeki NCCL kitaplığını kullanır ve bu kitaplık, halkanın tamamını azaltmayı uygular: her GPU, gradient'lerinin 1/N'sini komşusuna gönderir, diğer komşusundan 1/N alır ve N-1 adımdan sonra her GPU, tam ortalamaya sahip olur. Toplam iletişim hacmi: 2 x gradient _size x (N-1)/N, büyük N için gradient boyutunun 2 katına yaklaşıyor.
...
Bu ders, bir model boyutu ve kullanılabilir donanım alan bir `outputs/prompt-distributed-training-planner.md` -- bir prompt üretir ve ardından tam bir dağıtılmış eğitim planı üretir: paralellik stratejisi, bellek bütçesi, iletişim ek yükü ve beklenen verim.
...
3. Bir gradient birikim simülatörü uygulayın. Her mikro partiden sonra hepsini azaltmak yerine, K adım için gradient'leri yerel olarak biriktir, sonra hepsini azalt. Bunun iletişimi nasıl K kat azalttığını ancak aynı son gradient'leri (ve dolayısıyla aynı eğitimi) nasıl ürettiğini gösterin.

4. Bir maliyet tahmin aracı oluşturun. Model boyutu, hedef token sayısı, GPU türü (A100, $2/hr, H100 at $3,50/saat) ve paralellik stratejisi göz önüne alındığında, toplam eğitim maliyetini dolar cinsinden tahmin edin. Bilinen maliyetlere göre doğrulama: Llama 3 405B'nin maliyetinin ~$100M, DeepSeek V3 cost ~$5,6 milyon olduğu bildiriliyor.
...
| Sıfır | "DeepSpeed'in FSDP sürümü" | 3 aşamalı Sıfır Artıklık Optimize Edici: parça optimize edici (1. Aşama), + gradients (2. Aşama), + parametreler (3. Aşama) |
...
| Karışık hassasiyet | "Yarı hassasiyetle eğitim verin" | İleri/geri için FP16/BF16'yı ve optimize edici durumları için FP32'yi kullanın; optimize edici baskın olduğundan %50 değil ~%25 bellek tasarrufu sağlar |
...
- [Rajbhandari ve diğerleri, 2020 -- "ZeRO: Trilyonlarca Parametre Modelinin Eğitimine Yönelik Bellek Optimizasyonları"](https://arxiv.org/abs/1910.02054) -- üç parçalama aşamasını tanımlayan DeepSpeed ​​Zero makalesi
- [Shoeybi ve diğerleri, 2020 -- "Megatron-LM: Model Paralelliğini Kullanarak Milyarlarca Parametreli Dil Modellerinin Eğitimi"](https://arxiv.org/abs/1909.08053) -- NVIDIA'nın transformer'lar için tensör paralelliği
- [Narayanan ve diğerleri, 2021 -- "Megatron-LM Kullanan GPU Kümelerinde Verimli Büyük Ölçekli Dil Modeli Eğitimi"](https://arxiv.org/abs/2104.04473) -- Verileri, tensörü ve ardışık düzeni birleştiren 3 boyutlu paralellik
