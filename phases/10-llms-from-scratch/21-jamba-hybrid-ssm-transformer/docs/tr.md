...
> Durum alanı modelleri (SSM'ler) ve transformer'ler farklı şeyler ister. Transformer'ler kaliteyi ikinci dereceden maliyetle dikkat yoluyla satın alırlar. SSM'ler, yineleme ancak gecikme kalitesi yoluyla doğrusal zamanlı inference ve sabit bellek satın alır. AI21'in Jamba (Mart 2024) ve Jamba 1.5 (Ağustos 2024) bunları aynı modele yerleştirir: her 7 Mamba katmanı için 1 Transformer katmanı, her diğer blokta MoE ve tek bir 80 GB GPU'ya sığan 256k context window. Mamba-3 (ICLR 2026), SSM tarafını karmaşık değerli durum alanları ve MIMO projeksiyonlarıyla sıkılaştırır. Bu ders, her iki mimariyi de uçtan uca okur ve saf SSM ve saf Transformer uzun bağlamlı denemelerde başarısız olmasına rağmen hibrit tarifin neden üç yıllık ölçeklendirmede hayatta kaldığını açıklar.
...
- Yüksek düzeyde bir SSM yinelemesinin nasıl göründüğünü ve neden inference sabit belleğe olanak sağladığını belirtin.
- 256k bağlamında bir Jamba modelinin KV önbellek ayak izini hesaplayın ve saf bir Transformer modelinin ihtiyaç duyacağı şeylerle karşılaştırın.
...
Dikkat dizi uzunluğu bakımından ikinci derecedendir. Durum uzayı modelleri doğrusaldır. Bu fark daha da artıyor: 256 bin token'de, bir Transformer dikkat haritası kişi başına 65B giriştir; bir SSM'nin yinelenen durumu, dizi uzunluğundan bağımsız olarak sabit boyuttadır.

Pure-SSM modelleri (Mamba, Mamba-2), Transformer karışıklığını küçük ölçeklerde eşleştirir, ancak durum izleme görevlerinde gecikme yaşar ve bazı bağlam içi erişim kategorilerinde başarısız olur. Sezgi: SSM'ler geçmişi sabit bir duruma sıkıştırır ve geçmiş uzun olduğunda bilgi sızıntısı olur. Dikkat her şeyi tam olarak hatırlar ancak ikinci dereceden maliyet öder.

Açıkça görülen düzeltme: ikisini de kullanın. Transformer katmanlarını tam geri çağırmanın önemli olduğu yerlere yerleştirin. SSM katmanlarını başka yerde kullanın. Oranı ayarlayın. Jamba, bu hibrit tarifi geniş ölçekte sunan ilk üretim sınıfı modeldir (52 milyar toplam, 12 milyar aktif, 256 bin bağlam, tek 80 GB GPU). Jamba 1.5, aileyi toplam 398 milyar / aktif 94 milyara çıkarıyor. Mamba-3 (ICLR 2026), hibritlerin yeniden oluşturulabileceği mevcut en iyi saf SSM temelidir.
...
Bir durum uzay modeli, sabit boyutlu bir durum `h` aracılığıyla bir `x_1, ..., x_N` dizisini işler:
...
Her adımda durum `A` doğrusal dinamik aracılığıyla gelişir, `B x_t` girişini alır ve `C h_t` çıkışını yayar. `A, B, C` öğrenilebilir. Kritik özelliğe dikkat edin: `y_t` hesaplaması yalnızca `h_{t-1}` ve `x_t`'ye ihtiyaç duyar, daha önceki `x`'ye ihtiyaç duymaz. Bellek sabittir. Inference, token başına O(1)'dir.

Modelleme kalitesinin püf noktası `A`'nin yapısıdır. S4 (Gu 2021), eğitim sırasında uzun bir evrişim olarak verimli bir şekilde değerlendirilebilecek oldukça yapılandırılmış bir matris kullandı. Mamba (Gu, Dao 2023), sabit `A, B, C`'yi verilere bağımlı olanlarla ("seçici" kısım) değiştirdi. Mamba-2 (2024) yapıyı daha da basitleştirdi. Mamba-3 (2026), belirli yerlerde karmaşıklığı yeniden katıyor.
...
- `l`: Mamba'ya olan dikkat oranı. Jamba, `l = 8` kullanır; bu, her 7 Mamba katmanı için 1 Transformer katmanı anlamına gelir (7 Mamba + 1 Dikkat = grup başına 8 katman).
- `e`: MoE frekansı. Jamba, `e = 2` kullanıyor; bu, diğer tüm katmanların MoE uyguladığı anlamına geliyor.
...
Her Jamba bloğu 8 katmandan oluşur. 4 blok derinliğinde (toplam 32 katman), 28 Mamba ve 4 Dikkat katmanı elde edersiniz. Bunlardan 16'sı MEB kullanıyor.
...
Sezgi: Transformer katmanları tam geri çağırma ve durum takibini gerçekleştirir. Mamba katmanları, ucuz işlem hacmini yönetir.
...
- KV önbelleği (yalnızca dikkat katmanları): 256k BF16'da `2 * 4 * 32 * 128 * 256k * 2 = 8.4 GB`. Yalnızca 4 dikkat katmanı katkıda bulunur.
- SSM durumu: token öneki başına `28 * hidden * state_size`, ancak bu, dizi uzunluğuna göre ölçeklendirme değil, katman başına sabit bir boyuttur. Tipik Mamba durumu özellik başına 16'dır, gizli 4096: `28 * 4096 * 16 * 2 = 3.7 MB` toplam.

32 katmanlı, aynı gizli, 32 kafalı tam MHA'ya sahip saf Transformer ile karşılaştırın: 256k BF16'da `2 * 32 * 32 * 128 * 256k * 2 = 128 GB`. KV önbelleğinde 8 kat azalma. Çoğu 2024 modelinin kullandığı GQA(8) temeline (`2 * 32 * 8 * 128 * 256k * 2 = 32 GB`) kıyasla bile, Jamba'nın 16 GB'taki 1:7 hibriti hala 2 kat daha küçüktür.

AI21'in "tek bir 80 GB GPU'da 256k bağlam" ile kastettiği budur. Tam MHA saf Transformer'nin KV önbelleği sığmaz; bir DKG temel çizgisi bile ağırlıklara ve aktivasyonlara yer bırakmaz; Jamba'da öyle.
...
2. **Karmaşık değerli durum güncellemesi.** Önceki Mambalar, durum matrisini karmaşıktan (S4) gerçek köşegen (Mamba) ve ölçekli kimliğe (Mamba-2) indirgemişti. Mamba-3, karmaşık değerleri yeniden ekler; durum üzerinde veriye bağlı bir embedding dönerine eşdeğerdir. Bu, önceki gerçek değerli basitleştirmelerin maliyeti olan durum izleme yeteneklerini geri yükler.

3. **Çok girişli, çok çıkışlı (MIMO) projeksiyonlar.** Özellik başına skaler projeksiyonlar yerine matris değerli projeksiyonlar kullanın. Kod çözme gecikmesini artırmadan modelleme gücünü ve inference süresinde donanım kullanımını artırır.
...
- Bağlam, saf Transformer KV önbelleğinin acı verici hale gelmesine yetecek kadar uzundur (64k+).
...
- Bağlam kısa (16k'nin altında). SSM yükü boşa harcanır; saf Transformer gayet iyi.
...
| Modeli | Aile | Ölçek | Benzersiz iddia |
|-------|--------|------|-------------|
| Mamba-2 | saf SSM | 3B | doğrusal zaman, sabit bellek |
| Jamba | hibrit | 52B/12B | 80 GB'ta 256 bin |
| Jamba 1.5 Büyük | hibrit | 398B/94B | kurumsal düzeyde uzun bağlam |
| Mamba-3 | saf SSM | 1,5B (kağıt) | durum takibi geri yüklendi |
| DeepSeek-V3 | saf Transformer + MoE | 671B/37B | sınır yeteneği |

2026 manzarası: saf-Transformer MoE sınıra hakimdir, ancak hibritler 256 binden fazla bağlam nişine sahiptir. Mamba-3'ün durum izleme kazanımları, gelecek nesilde hibrit oranlarının daha düşük olmasına (daha fazla SSM, daha az dikkat) neden olabilir.
...
- Pure-Transformer temel çizgisi (KV önbelleği N ile birlikte büyür).
...
- 256k bağlamında, Jamba'nın bellek avantajı eşzamanlı istek veriminde ortaya çıkıyor. Aynı VRAM'e Transformer dizilerinden daha fazla Jamba dizisi sığdırabilirsiniz.
- Bağımsız bir model olarak Mamba-3 henüz üretime geçmiyor; araştırma önizlemesi 1,5B'de.

## Gönderin

Bu ders `outputs/skill-hybrid-picker.md`'yi üretir. Bir iş yükü spesifikasyonu (bağlam uzunluğu profili, görev karışımı, bellek bütçesi) göz önüne alındığında, bellek ve kalite değiş tokuşları hakkında açık bir akıl yürütme ile saf bir Transformer, Jamba tarzı bir hibrit ve saf bir SSM arasında öneride bulunur.

## Egzersizler

1. 32 katmanlı saf Transformer (gizli 4096, 32 kafa) ve aynı şekle sahip bir Jamba-1 hibriti için 256k bağlamda KV önbelleğini hesaplamak üzere `code/main.py`'yi çalıştırın. AI21 belgesinin iddia ettiği ~8 kat bellek azaltımını doğrulayın.
...
- [Lieber ve ark. — Jamba: A Hybrid Transformer-Mamba Dil Modeli (arXiv:2403.19887)](https://arxiv.org/abs/2403.19887) — orijinal Jamba makalesi, oran ablasyonları, 256k bağlam iddiası
...
- [Gu ve ark. — Yapılandırılmış Durum Uzaylarıyla Uzun Dizileri Verimli Bir Şekilde Modellemek (arXiv:2111.00396)](https://arxiv.org/abs/2111.00396) — S4 makalesi, LLM'ler için SSM şeceresinin başlangıç ​​noktası
