...
- Önceki on bir dersi (tokenizer, veri, ön eğitim, ölçeklendirme, SFT, RLHF, DPO, CAI, değerlendirme, niceleme, inference) tek bir tekrarlanabilir ardışık düzen spesifikasyonuna birleştirin
- Aşamalar arasındaki artifact sözleşmesini tanımlayın: her aşamanın ne tükettiği, ne ürettiği ve bir sonraki aşamanın girdiyi nasıl doğruladığı
- Deneyleri izleyen, artifact'leri karma hale getiren ve değerlendirme eşiklerinde gemi kararlarını kontrol eden bir orkestratör oluşturun
...
Önceki derslerin her biri çalışır. Tokenizer eğitildi. Minik GPT önceden eğitilmiştir. SFT dataset monte edildi. Ödül modeli eğitildi. DPO çalıştırması. Değerler ölçüldü. Nicelenmiş ağırlıklar dışa aktarıldı. Inference sunucusu açıldı. Her biri bir defter. Her birinin kendi gelenekleri, kendi çıktı yolları, kendi tohumları vardır.

Sınır antrenman koşusu bir not defteri değildir. Llama 3 405B yaklaşık 54 günde 30 milyon H100 saat sürdü. DeepSeek-V3 yaklaşık 2,8 milyon H800 saat kullandı. Bu süre zarfında, bozuk bir kontrol noktası, bir veri kirliliği, bir değerlendirme regresyonu, bir takıma bir haftalık duvar saatine ve bir aylık GPU bütçesine mal olabilir. Ekiplerin bu durumdan kurtulmasının yolu boru hattı hijyeninden geçer: Her aşamada deterministik bir girdi, deterministik bir çıktı, bir bildirim, bir karma ve bir geçit bulunur.

Bu kapak taşı. Boru hattını bir dizüstü bilgisayarda uçtan uca çalıştırmayacaksınız. Aşamaları koordine eden orkestratörü, çalışmayı açıklayan bildirimi, gemi kararlarını denetleyen doğrulayıcıyı ve üçüncü bir tarafın çalışmanızı tek bir dosyadan yeniden yürütmesine olanak tanıyan tekrar planını yazacaksınız. Kod küçüktür; disiplin büyüktür.

Desen, 100M'den 1T parametrelerine değişmeden ölçeklenir. Aynı dört bileşen (bildirim, orkestratör, değerlendirme kapısı, artifact deposu) Llama 3'ü çalıştırır ve aynı zamanda hobi GPT'nizi de çalıştırır. Aradaki fark, boru hattının şekli değil, her aşamanın yapılandırmasındaki sayıların boyutudur.
...
Bildirim, bir çalıştırmayı yeniden yürütmeye yetecek kadar tamamen tanımlayan tek bir dosyadır. Boru hattının ürettiği hiçbir şey manifestte yer almayan duruma bağlı olmamalıdır. Alanlar sıkıcı ve zorunludur.
...
N aşamasının çıkış karma değeri, N+1 aşamasının giriş karma değeridir. Herhangi bir sapma olursa boru hattı durur. Veri bozulmasını bu şekilde erken yakalarsınız. Bu aynı zamanda farklı bir kıtadaki bir takım arkadaşının kendi tekrarının sizinkiyle aynı artifact'yi ürettiğini doğrulama şeklidir.
...
| Sahne | Artifact Türü | Anahtar Alanlar |
|-------|--------------|-----------|
...
| 10 | Değerlendirme Raporu | benchmark puanları + regresyon farkları + değerlendirme veri karması |
| 11 | Nicelenmiş Model | nicelenmiş ağırlıklar + kalibrasyon verileri + doğruluk deltası vs FP16 |
| 12 | Sunucu Özellikleri | uç nokta + model karma + yapılandırma + observability kancalar |

Yazma, en yaygın hata modunu önler: aşama 08 çıkışını aşama 06 girişi olarak kullanmak, DPO tarafından eğitilmiş bir modeli SFT yolu üzerinden göndermek. Yazılan artifact'ler ve yazılan aşama imzaları, bu hataların beşinci gün hataları değil, derleme zamanı hataları olmasına neden olur.
...
Her kapı sayısal bir eşiktir. "İyi görünüyor" kapıları yok. Sübjektif imza yok. Her geçit geçerse artifact gönderilebilir olarak işaretlenir. Herhangi bir geçit başarısız olursa, çalıştırma, kendisi de bildirimde günlüğe kaydedilen, adlandırılmış bir gözden geçiren tarafından açık bir şekilde geçersiz kılınıncaya kadar bekletilir.
...
Manifest'i okuyan, aşamaları gönderen, artifact'leri izleyen ve herhangi bir sözleşme ihlalini durduran küçük bir kod parçası. Bu Hava Akışı değil. Bu Kubeflow değil. Boru hattı hijyeni için yazdığınız sıkıcı bir şeyi istiyorsunuz.
...
Bu 200 satır Python'dur. Bu derste `code/main.py` dosyasına benzeyecektir. Gerçek işlem hattı, kümeler üzerinde ayrı ayrı aşamaları yürütmek için `torchrun` veya `ray` kullanıyor ancak orkestratörün kendisi tek bir kutu üzerinde çalışıyor.

### Deney Takibi ve Artifact Depolama

İki harici sistem boru hattını sabitler.

**Deney izleyici (wandb, neptune, mlflow).** Kayıp eğrilerini, değerlendirme ölçümlerini ve aşama başına sistem telemetrisini günlüğe kaydeder. İzleyici, A çalışmasını üç hafta sonraki B çalışmasıyla karşılaştırmanız gerektiğinde gideceğiniz yerdir. Takımlar bunun için neredeyse her zaman barındırılan bir izleyici kullanır; kendi yazınızı yazmak, eğitime harcanması gereken zamanı kaybeder.
...
Orkestracı her ikisine de yazar. İzleyici, grafiklere bakan insanlar içindir. artifact deposu bir sonraki aşamada giriş arama içindir.

### Maliyetlendirme

Sınır koşusuna bir dolar numarası eklenir. Bütçe disiplini iki yerde olur.

**Çalıştırma öncesi tahmin.** Bildiriden, beklenen FLOP'ları (ön eğitim için: 6 x parametre x token), beklenen GPU saatlerini (FLOP'lar / en yüksek verim / kullanım) ve geçerli kiralama fiyatı üzerinden dolar maliyetini hesaplayın. Tahmin bütçe sınırını aşarsa boru hattı başlamayı reddeder.
...
Llama 3'ün ana eğitim öncesi çalışması için bildirilen maliyeti $61M. DeepSeek-V3 reported $5,6 milyon oldu. Oran çoğunlukla donanım verimliliği artı uzmanların karışımından oluşuyor; ancak her iki ekip de bunu çalışma başına değil aşama başına takip ettiği için spesifik maliyet görülebilir.
...
- Tokenizer: Bayt geri dönüşüyle 128k BPE. Küçük, dengeli çok dilli bir dilimde eğitim aldım.
- Ön eğitim: 10-20T token'ler, çoğunlukla web artı kod artı sentetik. Muon veya AdamW iyileştirici. FSDP2 veya DeepSpeed ​​Zero-3. Gradient kontrol noktası belirleme. BF16 ağırlıkları, FP32 master.
- SFT: 500k-2M talimat çifti, insan ve sentetik karışımı, değerlendirme setine karşı katı tekilleştirme ile.
...
Tam uygulama için `code/main.py`'ye bakın. Anahtar parçalar:

- `Manifest` veri sınıfı: boru hattı sürümü, tohum, git taahhüdü, aşamalar, kapılar.
- `Stage` veri sınıfı: ad, tür, girişler (karma), çıkış (karma), duvar saati, maliyet.
...
Her seferinde ilk önce `plan` komutunu çalıştırın. Çoğu ardışık düzen hatası plan zamanında ortaya çıkıyor; eksik geçit eşikleri, eski karmalar, bütçe aşımları. `plan`'yi çalıştırmak ücretsizdir. `run`'yi çalıştırmak pahalıdır. Ucuz taraftaki hataları yakalayarak paradan tasarruf edin.

`gate`'nin çıkışı `SHIP` veya `HOLD: <reason>`'dir. Bekletilen bir koşu bir başarısızlık değildir; bir karar noktasıdır. Adlandırılmış bir gözden geçiren kişi ya geçersiz kılar (ve geçersiz kılma günlüğe kaydedilir) ya da geri almayı onaylar.

## Gönderin

Bu ders `outputs/skill-llm-pipeline-reviewer.md`'yi üretir. Önerilen bir işlem hattı manifestosunu besleyin ve tüm sözleşmeleri kontrol etsin: aşama yazma, karma zinciri, kapılar, geri alma planı, maliyet tahmini. Eksik değerlendirme kapısına, sınırsız KL bütçesine veya değerlendirme ve eğitim verilerini karıştıran bir çalıştırmaya sahip bir bildirimi onaylamayı reddeder.
...
2. Bir "kirlilik kontrolü" kapısı ekleyin. Değerlendirme dataset karması ve eğitim dataset parçaları göz önüne alındığında, örtüşmeyi hesaplayın (tam dize eşleşmesi veya 13 gramlık eşleşme). Örtüşme %0,1'i aşarsa geçit başarısız olur. Ona kirlenmiş bir eğitim seti verin ve kapının koşuyu sürdürdüğünü doğrulayın.

3. İlk ilkelerden yola çıkarak bir maliyet tahmin aracı uygulayın. Aşama 04 (eğitim öncesi) için, FLOP'ları 6 x parametre x token olarak tahmin edin, H100'de 989 TFLOP BF16'da 2,50 ABD Doları/GPU saati düzeyinde %40 MFU (model FLOP kullanımı) olduğunu varsayalım. 2T token'ler üzerinde eğitilmiş bir 7B modeli için tahmininizi bildirin. Yayınlanan Lama 2 sayılarıyla karşılaştırın.

4. Kısmi bir geri alma oluşturun. Aşama 09'da (CAI) bir arıza simülasyonu yapın, ardından 01-08'i önbellekte bırakarak 09'dan 12'ye kadar olan aşamaları yeniden çalıştırın. Orkestratörün önbelleğe alınmış artifact'leri hash ile algılaması ve bunları atlaması gerekir. Tam yeniden çalıştırmaya karşı kaydedilen duvar saatini ölçün.

5. observability'yi ekleyin. Her aşama için parametreler, token görülen token'ler, kayıp ve maliyet öznitelikleriyle birlikte OpenTelemetry yayılmalarını yayınlayın. Açıklıkları yerel bir toplayıcıya borulayın. Önemli olan gösterge tabloları değil; Önemli olan, her aşamanın sağlığının tek bir izleme kimliğinden izlenebilmesidir.
...
| İçeriğe yönelik | "İsme göre değil karmayla" | İçerikleri SHA-256 tarafından depolanan Artifact'lerdir; böylece A sürümünü hiçbir zaman B sürümüyle karıştıramazsınız.
| Değerlendirme kapısı | "Gemi kriterleri" | artifact'nin gönderilebilir olarak işaretlenmesinden önce geçmesi gereken benchmark metrikleri ve güvenlik puanlarına ilişkin sayısal eşikler |
...
| Artifact mağazası | "Ağırlıklar için S3 sürümü" | Değişmez içerik adresli nesne deposu — kontrol noktaları, dataset'ler, değerlendirme raporları için tek gerçek kaynak |
...
| Maliyet kapısı | "X'i geçemezsiniz" | Çalıştırma öncesi maliyet tahmini artı çalıştırma içi izleyici — tahmin bütçeyi aşarsa boru hattı başlamayı reddeder |

## Daha Fazla Okuma

- [Dubey ve diğerleri, 2024 -- "The Llama 3 Herd of Models"](https://arxiv.org/abs/2407.21783) -- veriler, eğitim, hizalama ve değerlendirmeyi içeren bir sınır hattının en ayrıntılı kamuya açık açıklaması
- [DeepSeek-AI, 2024 -- "DeepSeek-V3 Teknik Raporu"](https://arxiv.org/abs/2412.19437) -- Llama 3 sınıfı eğitiminin kabaca 1/10'u kadar maliyetle verimlilik öncelikli boru hattı
...
- [Ağırlıklar ve Önyargılar Yüksek Lisans Raporları](https://wandb.ai/site/llms) -- açık kaynaklı Yüksek Lisans çalışmaları için gerçek bildirimler ve deneme izleyici çıktısı; intihal edilebilir şablonlar olarak kullanışlıdır
