...
agent sesinin her 20 ms'lik parçada verdiği üç farklı karar:
...
- **Eşik.** Silero bir olasılık çıktısı verir; Konuşmayı &gt; 0,5 (varsayılan) veya >; 0,3 (hassas). Daha düşük eşik = daha az ilk kelime klibi, daha fazla yanlış pozitif.
...
Akışlı STT modellerinde ileriye dönük bir gecikme bulunur (Kyutai STT-1B için 500 ms, STT-2.6B için 2,5 sn). Normalde konuşmanın bitiminden sonra transkripsiyon için bu kadar uzun süre beklersiniz. Temizleme hilesi: VAD konuşmanın sonunu tetiklediğinde, **STT'ye anında çıkışı zorlayan bir temizleme sinyali gönderin**. STT ~4× gerçek zamanlı olarak işlenir, böylece 500 ms'lik arabellek ~125 ms'de tamamlanır.
...
- **Çok kısa sessizlik akşamdan kalma.** Agent cümlenin ortasında kesintiye uğruyor. 500-800 ms, konuşma konuşması için en uygun noktadır.
...
`outputs/skill-vad-tuner.md` olarak kaydedin. Bir iş yükü için VAD modeli, eşik, akşamdan kalma, videodan önce gösterilen reklam ve dönüş algılama stratejisini seçin.

## Egzersizler

1. **Kolay.** `code/main.py`'yi çalıştırın. Bir konuşma + sessizlik + konuşma + öksürük dizisini simüle eder ve üç VAD katmanını test eder.
2. **Orta.** `silero-vad`'yi yükleyin, 5 dakikalık bir kayıt işlemi yapın, hem ilk kelime kliplerini hem de yanlış tetikleyicileri en aza indirecek şekilde eşiği ayarlayın. Hassasiyeti/geri çağırmayı bildirin.
3. **Zor.** Mini bir dönüş dedektörü oluşturun: Silero VAD + son 10 kelimenin embedding'leri üzerine 3 katmanlı bir MLP (cümle-transformer'leri kullanın). Elle etiketlenmiş bir dönüş ucu dataset üzerinde eğitim alın. Yalnızca Silero'yu %10 F1 yendi.
...
- [pyannote segmentasyonu](https://github.com/pyannote/pyannote-audio) — günlük oluşturma dereceli segmentasyon.
