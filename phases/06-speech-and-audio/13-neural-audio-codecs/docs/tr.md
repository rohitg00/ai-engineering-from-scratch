# Nöral Ses Codec'leri — EnCodec, SNAC, Mimi, DAC ve Semantik-Akustik Ayrım

> 2026 ses üretiminin neredeyse tamamı token'lardır. EnCodec, SNAC, Mimi ve DAC, sürekli dalga formlarını bir transformer'nin tahmin edebileceği ayrık dizilere dönüştürür. Semantik-akustik token ayrımı - semantik olarak ilk kod kitabı, akustik olarak geri kalanı - ses için Transformer'den bu yana en önemli mimari değişimdir.

**Tür:** Öğren
**Diller:** Python
**Önkoşullar:** Aşama 6 · 02 (Spektrogramlar), Aşama 10 · 11 (Kuantizasyon), Aşama 5 · 19 (Alt Kelime Tokenleştirme)
**Süre:** ~60 dakika

## Sorun

Dil modelleri ayrık token'lar üzerinde çalışır. Ses süreklidir. Konuşma/müzik için Yüksek Lisans tarzı bir model istiyorsanız (MusicGen, Moshi, Susam CSM, VibeVoice, Orpheus) öncelikle bir **nöral ses codec bileşenine** ihtiyacınız vardır: sesi token'lerden oluşan küçük bir kelime dağarcığına ayrıştıran öğrenilmiş bir kodlayıcı ve dalga biçimini yeniden oluşturan eşleşen bir kod çözücü.
...
2. **Semantik öncelikli codec bileşenleri** — Mimi (Kyutai), KonuşmaTokenizer. İlk kod kitabını dilsel/fonetik içeriği kodlamaya zorlayın (genellikle WavLM'den ayrıştırarak). Sonraki kod kitapları akustik ayrıntılardır.

2024-2026 öngörüsü: **saf bir yeniden yapılandırma codec bileşeni, metinden oluşturmayı denediğinizde size bulanık konuşma sağlar.** Codec üzerinden LLM token'ler, ölçeklenmeyen aynı kod kitabında hem dil yapısını hem de akustik yapıyı öğrenmek zorundadır. Bunları ayırmak (anlamsal kod kitabı 0, akustik kod kitapları 1-N) Moshi ve Susam CSM'nin çalışmasını sağlayan şeydir.

## Konsept

![Dört codec ortamı: EnCodec, DAC, SNAC (çok ölçekli), Mimi (semantik+akustik)](../assets/codec-comparison.svg)

### Temel püf noktası: Artık Vektör Nicelemesi (RVQ)

Tüm modern ses codec'leri, tek bir büyük kod kitabı yerine (iyi kalite için milyonlarca koda ihtiyaç duyar) **RVQ** kullanır: bir dizi küçük kod kitabı. İlk kod kitabı kodlayıcı çıktısını niceler; ikincisi artığı nicemler; vb. Her kod kitabı 1024 koddan oluşur. 8 kod kitabı = 1024^8 = 10^24 etkili kelime dağarcığı.

inference zamanında, kod çözücü yeniden oluşturmak için çerçeve başına seçilen tüm kodları toplar.
...
Bir AR LM, önce anlamsal token'yi tahmin eder (metne göre koşullandırılmış), ardından akustik token'leri (anlamsal + konuşmacı referansına göre koşullandırılmış) tahmin eder. Bu faktörleştirme, modern TTS'nin sesleri sıfır atışla klonlayabilmesinin nedenidir: semantik model içeriği yönetir; akustik model tınıyı yönetir.
...
Opus gibi geleneksel codec'ler algısal kalite açısından hâlâ bit başına kazanıyor. Sinir kodlayıcıları **ayrık token'lar** (Opus'un üretmediği) ve **üretken model kalitesi** (LM'nin bu token'lerle yapabilecekleri) ile kazanır.
...
Anlamsal kod kitabı 0, WavLM ile hizalanmıştır. Metinden anlambilime transformer geçiş yapabilirsiniz; bu, doğrudan sese geçişten çok daha küçük bir sözcük dağarcığıdır. Daha sonra hoparlör referansında ayrı bir akustikten dalga biçimine kod çözücü koşulları sağlanır.

### Adım 4: codec token'ler üzerinden AR LM neden çalışıyor?
...
1000 tokens, bir transformer için önemsiz bir bağlamdır. 256M parametreli bir transformer, modern bir GPU'da milisaniyede 10 saniyelik konuşma üretebilir.
...
Temel kural: **Eğer üretken bir model oluşturuyorsanız, Mimi veya SNAC ile başlayın. Bir sıkıştırma hattı oluşturuyorsanız Opus'u kullanın.**
...
- **Kare hızı uyumsuzluğu.** LM'yi 12,5 Hz Mimi'de eğitmek, ardından 50 Hz EnCodec'te fine-tuning eğitimi sessizce başarısız oluyor.
...
`outputs/skill-codec-picker.md` olarak kaydet. Belirli bir üretken veya sıkıştırma görevi için bir codec bileşeni seçin.

## Egzersizler

1. **Kolay.** `code/main.py` komutunu çalıştırın. Bir oyuncak skaler + artık niceleyici uygular ve siz kod kitapları ekledikçe yeniden yapılandırma hatasını ölçer.
2. **Orta.** `encodec`'yi yükleyin ve 1, 4, 8, 32 kod kitabını uzatılmış bir konuşma klibinde karşılaştırın. PESQ veya MSE ile bit hızının grafiğini çizin.
...
| Kare hızı | Codec hızı | Saniyede kaç token-kare. Daha düşük = daha hızlı LM. |
...
- [Kumar ve ark. (2023). Tanım Ses Codec Bileşeni (DAC)](https://arxiv.org/abs/2306.06546) — en yüksek kalitede açık.
