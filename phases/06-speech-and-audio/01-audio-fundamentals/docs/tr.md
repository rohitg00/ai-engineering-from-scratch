...
**Dalga formu.** `[-1.0, 1.0]`'daki tek boyutlu kayan dizi. Örnek numarasına göre indekslenmiştir. Saniyeye dönüştürmek için örnekleme hızına bölün: `t = n / sr`. 16 kHz'deki 10 saniyelik bir klip, 160.000 kayan nokta dizisidir.
...
**Nyquist-Shannon.** `sr` örnekleme hızı, `sr/2`'a kadar olan frekansları açık bir şekilde temsil edebilir. `sr/2` sınırı *Nyquist frekansıdır*. Nyquist'in üzerindeki enerji *takma ad alır* (daha düşük frekanslara katlanır) ve sinyali bozar. Altörneklemeden önce daima alçak geçiren filtre.

**Bit derinliği.** 16 bit PCM (int16 işaretli, aralık ±32.767) evrensel değişim formatıdır. Müzik için 24 bit, dahili DSP için 32 bit kayan nokta. `soundfile` gibi kütüphaneler int16'yı okur ancak `[-1, 1]` içindeki float32 dizilerini açığa çıkarır.

**Fourier Dönüşümü.** Herhangi bir sonlu sinyal, farklı frekanslardaki sinüzoidlerin toplamıdır. Ayrık Fourier Dönüşümü (DFT), `N` örnekleri için, `N` karmaşık katsayıları (frekans kutusu başına bir tane) hesaplar. `bin k`, `k · sr / N` Hz frekansıyla eşleşir. Büyüklük o frekanstaki genliktir, açı ise fazdır.
...
`code/main.py`, demo bağımlılığını ortadan kaldırmak için yalnızca stdlib `wave` modülünü kullanır. Üretim için `soundfile` veya `torchaudio.load` kullanacaksınız (her ikisi de `(waveform, sr)` tuple'larını döndürür):
...
`O(N²)` — `N=256`'nin doğruluğunu onaylaması iyi, gerçek ses için işe yaramaz. Gerçek kod `numpy.fft.rfft` veya `torch.fft.rfft`'yi çağırır.

### Adım 4: Baskın frekansı bulun

Büyüklük tepe indeksi `k_star`, frekans `k_star * sr / N` ile eşleşir. Bunu 440 Hz sinüste çalıştırmak, `440 * N / sr` bölmesinde bir tepe noktası döndürmelidir.

### Adım 5: takma adı gösterin

10 kHz'de (Nyquist = 5 kHz) 7 kHz sinüsü örnekleyin. 7 kHz tonu Nyquist'in üzerindedir ve `10 − 7 = 3 kHz`'a katlanır. FFT zirvesi 3 kHz'de görünür. Bu, klasik örtüşme demosudur ve her DAC/ADC'nin tuğla duvarlı alçak geçiş filtresiyle birlikte gönderilmesinin nedenidir.
...
|------|---------|-----|
| WAV/FLAC/OGG okuma/yazma | `soundfile` (libsnddosyası sarmalayıcısı) | En hızlı, kararlı, float32'yi döndürür. |
...
`outputs/skill-audio-loader.md` olarak kaydet. Bu beceri, ses girişinin alt modelin beklentileriyle eşleşip eşleşmediğini kontrol etmenize ve eşleşmediğinde doğru şekilde yeniden örneklemenize yardımcı olur.
...
2. **Orta.** Sesinizin 3 saniyelik WAV'ını 48 kHz'de kaydedin. `torchaudio.transforms.Resample` (kenar yumuşatma ile) kullanarak 16 kHz'e alt örnekleme yapın, ardından saf yok etme kullanarak (her üç örnekte bir) 16 kHz'e alt örnekleme yapın. Her ikisi de FFT'dir. Takma ad nerede görünüyor?
...
| DFT | Diziler için Fourier dönüşümü | `N` örnek → `N` karmaşık frekans katsayıları. |
...
- [Smith — The Scientist and Engineer's Guide to Digital Signal Processing](https://www.dspguide.com/ch8.htm) — ücretsiz, standart DSP ders kitabı.
- [librosa docs — ses kılavuzu](https://librosa.org/doc/latest/tutorial.html) — kodla pratik anlatım.
