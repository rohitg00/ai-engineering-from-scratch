...
4. Projete uma stack de moderação para um deploy de assistente de código (ex., GitHub Copilot). Identifique as categorias mais e menos relevantes e proponha regras customizadas.
...
| Moderação customizada | "regras de domínio" | Regras específicas do deploy (regex, lista de permissão, política) |
| Moderação em camadas | "as três camadas" | Padrão padrão de deploy em produção |
...
- [Google Jigsaw Perspective API](https://perspectiveapi.com/) — pontuação de toxicidade
