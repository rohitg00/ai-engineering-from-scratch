...
> Três referências de grau de produção pra estudar end-to-end, cada uma ilustrando uma fatia diferente de engenharia multi-agente. **O sistema Research da Anthropic** (orchestrator-worker, 15x tokens, +90.2% sobre agent único Opus 4, deploys rainbow) é o caso canônico de supervisor. **MetaGPT / ChatDev** (especialização de papéis via SOPs pra engenharia de software; a "comunicative dehallucination" do ChatDev; extensão MacNet pra >1000 agents via DAGs, arXiv:2406.07155) é o caso canônico de decomposição de papéis. **OpenClaw / Moltbook** (originalmente Clawdbot por Peter Steinberger, novembro 2025; renomeado duas vezes; 247k stars no GitHub até março 2026; agents locais de loop ReAct; Moltbook como rede social só-de-agents com ~2.3M contas de agent em dias de lançamento, adquirida por Meta em 2026-03-10) ilustra o que acontece em escala populacional: atividade econômica emergente, riscos de prompt-injection, regulação de nível estatal (China restringiu OpenClaw em computadores governamentais, março 2026). **Paisagem de frameworks abril 2026:** LangGraph e CrewAI lideram produção; AG2 é a continuação comunitária do AutoGen; Microsoft AutoGen está em modo manutenção (mesclado no Microsoft Agent Framework, RC fev 2026); OpenAI Agents SDK é o sucessor produtivo do Swarm; Google ADK (abril 2025) é o entrante nativo em A2A. Todo framework maior agora entrega suporte a MCP; a maioria entrega A2A. Esta aula lê cada caso end-to-end e destila os padrões comuns pra você escolher a referência certa pro seu próximo sistema de produção.
...
- **+90.2%** de melhoria sobre agent único Opus 4 em avaliações internas de pesquisa.
...
- **15x tokens por query** vs agent único.
- **Deploy rainbow** porque agents são de execução longa e stateful.

Lições de design codificadas:

1. **Escale o esforço à complexidade da query.** Simples → 1 agent com 3-10 chamadas de ferramenta. Médio → 3 agents. Pesquisa complexa → 10+ sub-agents.
...
3. **Deploys rainbow.** Mantenha versões antigas de runtime vivas até que seus agents em andamento terminem.
...
Contribuição do ChatDev: **comunicative dehallucination**. Agents pedem especificações antes de responder — um agent designer pergunta ao programador qual linguagem é pretendida antes de esboçar a UI, ao invés de adivinhar. O artigo reporta isso reduz alucinação em pipelines multi-agente mensuravelmente.

MacNet (arXiv:2406.07155) estende o ChatDev pra **>1000 agents via DAGs**. Cada nó do DAG é uma especialização de papel; arestas codificam contratos de handoff. A escala é possível porque routing é explícito e computável offline.
...
- **Fev 2026:** Moltbook lança como rede social só-de-agents nas mesmas primitivas; ~2.3M contas de agent em dias.
...
Isso é o que multi-agente parece quando você coloca milhões de agents num substrato compartilhado:

- **Atividade econômica emergente.** Agents compram, vendem e servem uns aos outros usando pagamentos por token.
- **Riscos de prompt-injection em escala populacional.** Um prompt malicioso num perfil viral de agent se propaga pra milhares de interações agent-a-agent em horas.
...
2. **Prompt injection é o novo XSS.** Trate perfis de agent e mensagens cross-agent como input não-confiável por padrão.
...
| **Anthropic Claude Agent SDK** | Produção | agent único + extensão Research | veja o post do sistema Research |
...
1. **Orchestrator + workers** (supervisor explícito da Anthropic, PM-como-supervisor do MetaGPT, agents individuais do OpenClaw + efeitos de rede).
...
- **Custo é a restrição central de engenharia.** Custo de token por tarefa, tempo de parede por interação, overhead de deploy rainbow. Multi-agente vence em acurácia mas perde em custo — e esse trade é a decisão de negócio.
- **Regulação é input de curto prazo, não preocupação de fundo.** Jurisdições estão se movendo mais rápido que ciclos de deploy individuais.
...
- **Deploy rainbow pra agents de execução longa.** Espere execuções de agent de múltiplas horas serem rotina.
- **Leia WMAC 2026 e os follow-ups do MAST.** A disciplina está evoluindo rápido.

## Exercícios

1. Leia o post do sistema Research da Anthropic end-to-end. Identifique três decisões de design que mudariam se você substituísse o Opus 4 por um modelo menor (ex: Haiku 4).
...
|------|----------------|------------------------|
| Research da Anthropic | "A referência do supervisor" | Claude Opus 4 + sub-agents Sonnet 4; 15x tokens; +90.2% sobre agent único. |
...
| MacNet | "Escale ChatDev via DAG" | arXiv:2406.07155; 1000+ agents via routing explícito por DAG. |
...
| Deploy rainbow | "Múltiplas versões concorrentes" | Mantenha versões antigas de runtime vivas pra agents de execução longa em andamento. |
| Comunicative dehallucination | "Pergunte antes de responder" | Agents pedem especificações dos peers ao invés de adivinhar. |
...
- [MacNet — escalando agents baseados em papéis pra 1000+](https://arxiv.org/abs/2406.07155) — escala baseada em DAG
