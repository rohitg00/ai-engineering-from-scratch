...
> A herança do aprendizado por reforço em coordenação multi-agente, que ainda informa sistemas LLM-agent em 2026. **MADDPG** (Lowe et al., NeurIPS 2017, arXiv:1706.02275) introduziu Treinamento Centralizado, Execução Descentralizada (CTDE): cada critic vê estados e ações de todos agents durante o treinamento; no teste só os atores locais rodam. Funciona pra cenários cooperativos, competitivos e mistos. **QMIX** (Rashid et al., ICML 2018, arXiv:1803.11485) é decomposição de valor com uma mixing network monotônica; Qs por agent se combinam em Q conjunto pra que `argmax` distribua limpo — dominante no StarCraft Multi-Agent Challenge (SMAC). **MAPPO** (Yu et al., NeurIPS 2022, arXiv:2103.01955) é PPO com função de valor centralizada; "surpreendentemente eficaz" em particle-world, SMAC, Google Research Football, Hanabi com mínimo tuning. Esses são a base pra treinar políticas de equipes de agents que precisam agir descentralizadamente. MAPPO é o **baseline cooperativo-MARL padrão em 2026**. Esta aula constrói cada um a partir de um grid-world simples e fixa as três ideias na memória muscular antes de tocar no treinamento de LLM-agent.
...
- RL independente (cada agent aprende sozinho) é não-estacionário da perspectiva de cada agent. Ruim.
- RL centralizado (um agent controla tudo) não escala e viola restrições de execução.
...
Cada agent `i` tem um ator `mu_i(o_i)` que mapeia sua observação para ação. Cada agent também tem um critic `Q_i(x, a_1, ..., a_n)` que vê todas observações e todas ações durante o treinamento. O ator é atualizado por policy gradient contra a avaliação do critic.
...
Por que CTDE: no treinamento, a gente conhece todas ações; usa isso pra reduzir variância em cada critic. No deploy, cada agent só vê `o_i` e chama `mu_i(o_i)`.

Modo de falha: critics crescem com N agents (input inclui todas ações). Não escala passando ~10 agents sem aproximações.
...
A monotonicidade garante que `argmax_a Q_tot` pode ser computado por cada agent escolhendo `argmax_{a_i} Q_i` independentemente. Essa é **exatamente a propriedade de execução descentralizada** que você precisa. No treinamento, uma mixing network produz `Q_tot` a partir dos Qs por agent.

Por que QMIX vence no SMAC: microgerenciamento cooperativo do StarCraft tem agents homogêneos, observações locais, recompensa global — encaixe perfeito pra decomposição de valor.

Modo de falha: a restrição de monotonicidade é restritiva; algumas tarefas têm estruturas de recompensa não decomponíveis monotonamente (um agent se sacrificando pela equipe). Extensões (QTRAN, QPLEX) relaxam isso.

### MAPPO (2022) — o default esquecido

Multi-Agent PPO: PPO com função de valor centralizada. Cada agent tem sua política; todos agents compartilham (ou têm por agent) funções de valor que vêem o estado completo. Yu et al. 2022 compararam MAPPO contra MADDPG, QMIX e suas extensões em cinco benchmarks e acharam:
...
2. **Emergência de papéis.** Em simulações de agentes generativos, treinar agents pra adotar papéis complementares ao longo do tempo é um problema de MARL disfarçado. Decomposição de valor estilo QMIX força complementaridade por construção.
3. **Uso de ferramentas multi-agente.** Quando agents compartilham ferramentas e competem por orçamento, treiná-los via CTDE produz políticas locais despacháveis que respeitam restrições de recursos.
...
- Em *runtime*, força execução descentralizada: cada agent só vê `o_i`.

O padrão força você a manter estado por agent explícito e a pensar em observabilidade parcial desde o começo. Muitos sistemas multi-agente em produção assumem silenciosamente estado compartilhado em toda parte — a disciplina de CTDE previne isso.

### O problema de não-estacionaridade

Quando múltiplos agents aprendem simultaneamente, o ambiente de cada agent (que inclui as políticas dos outros) é não-estacionário. Provas clássicas de agent único quebram. Os algoritmos de MARL nesta aula todos lidam com isso:
...
Em sistemas LLM-agent, não-estacionaridade se manifesta como "meu agent funcionava no mês passado, agora aquele outro agent upstream mudou, o meu desandou." Treinar MARL com CTDE é a correção princípiada; correções no nível de prompt são mais rápidas mas menos duráveis.
...
- Ambiente: 2 agents em um grid 4x4, uma pellet de recompensa. Recompensa = 1 se qualquer agent alcança a pellet; tarefa termina.
- `IndependentAgents` — cada agent trata os outros como ambiente. Baseline.
...
Saída esperada: agents independentes levam ~6 passos em média; variantes CTDE convergem pra ~3.5 passos (o ótimo pra grid 4x4 é 3). A diferença de padrão aparece mesmo com políticas scriptadas.
...
- **Logue a observação e o fluxo de ações de cada agent.** Debugar MARL sem rastros por agent é desesperador.
...
- **Cuidado com reward shaping.** MARL é extremamente sensível a design de recompensa. Um bug de coordenação no shaping e agents aprendem a explorá-lo. Rode testes adversariais.
...
1. Execute `code/main.py`. Meça a diferença de passos-ao-objetivo entre agents independentes e estilo MAPPO. A diferença cresce ou diminui em um grid 6x6?
...
5. Aplique CTDE como padrão de design a um sistema LLM-agent hipotético (ex: agent de pesquisa + resumidor + codificador). Qual é a informação conjunta disponível no design que não está disponível no runtime?
...
| MADDPG | "Multi-Agent DDPG" | CTDE com critic por agent vendo todas observações + ações. |
...
| Não-estacionaridade | "Alvos em movimento" | Ambiente de cada agent muda enquanto outros aprendem. O problema central do MARL. |
