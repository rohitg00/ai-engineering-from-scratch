...
> Park et al. 2023 (UIST '23, arXiv:2304.03442) populou **Smallville**, um sandbox de 25 agents, com uma arquitetura de três partes: **stream de memória** (log em linguagem natural), **reflexão** (sínteses de ordem superior que o agent gera sobre seu próprio stream) e **plano** (comportamento em nível de dia, depois sub-planos). O resultado marcante foi a emergência da festa de Dia dos Namorados: um agent com a semente "quer dar uma festa de Dia dos Namorados", sem roteirização adicional, produziu convites que se espalharam pela população, coordenaram datas, e a festa aconteceu — a partir de 24 agents que começaram sem saber nada disso. Ablações mostram que todas as três componentes são necessárias pra verossimilhança. As falhas documentadas são erros de normas espaciais (entrar em lojas fechadas, compartilhar banheiros individuais). Essa é a arquitetura de referência pra simulações de agents e avaliação social multi-agent em 2026.
...
A maioria dos sistemas multi-agent são times roteirizados: planejador planeja, coder codifica, reviewer revisa. Isso funciona pra tarefas bem definidas. Não captura o comportamento emergente, não-roteirizado, que surge quando agents têm memória, prioridades e um mundo aberto. Pesquisa, simulação de sociedade e cada vez mais game AI precisam desse segundo tipo.

A arquitetura Smallville é o benchmark pra isso. Até Park 2023, as melhores simulações de agents eram seguidores de scripts rasos; depois disso, o padrão virou o default pra agents generativos em mundos abertos. Se você constrói uma simulação de agents em 2026, está usando as três componentes do Smallville ou justificando explicitamente por que não.

## Conceito

### As três componentes

**Stream de memória.** Um log append-only de observações, ações, reflexões e planos. Cada entrada tem timestamp, tipo, descrição (linguagem natural) e metadados derivados: **recência**, **importância** (auto-avaliada 1-10 pelo agent) e **relevância** (similaridade cosseno à query atual).
...
**Reflexão.** Periodicamente (a cada N memórias ou em eventos importantes), o agent gera sínteses de ordem superior a partir de memórias recentes. Entradas de reflexão voltam ao stream e são recuperáveis como qualquer outra memória. É assim que agents constroem "entendimentos" — o equivalente da arquitetura pra crenças de longo prazo.

**Plano.** Decomição top-down. Primeiro, um plano em nível de dia em traços amplos ("ir trabalhar, jantar com Klaus"). Depois planos em nível de hora. Depois planos em nível de ação. Planos são revisáveis: quando uma observação contradiz um plano, o agent replaneja o segmento afetado.
...
- Sem **observação** o agent perde contexto e age em crenças desatualizadas.
- Sem **reflexão** o agent não consegue formar crenças de ordem superior; interações ficam rasas.
...
Um agent, Isabella Rodriguez, recebe a semente do objetivo "quer dar uma festa de Dia dos Namorados no Hobbs Cafe em 14 de fev às 5pm." Os 24 outros agents não recebem essa semente. Ao longo de dias simulados:
...
6. Às 5pm em 14 de fev, vários agents convergem no Hobbs Cafe.
...
Esses são modos de falha relevantes pra produção: qualquer simulação de agents de 2026 os herda.
...
- **NPC AI pra games.** RPGs com agents Smallville produzem enredos emergentes em vez de quests roteirizadas.
- **Benchmarks de avaliação de agents generativos.** Em vez de acurácia de tarefa, a métrica vira verossimilhança + coerência de comportamento em runs longas.
...
`code/main.py` implementa as três componentes em Python stdlib com políticas de agent roteirizadas (sem LLM real). A demo reproduz a emergência da festa em miniatura:
...
- Cenário: 5 agents. Agent 1 começa com "dar festa às 5pm." Ao longo de ticks simulados, o convite se espalha e agents convergem.
...
Saída esperada: trace tick-a-tick. No tick final, pelo menos 3 dos 5 agents mostram a festa em seu plano, e convergem no local da festa. A única semente produziu a chegada coordenada sem nenhum orquestrador.

## Use

`outputs/skill-simulation-designer.md` desenha uma simulação de agents generativos: número de agents, schema de memória, cadência de reflexão, horizonte de plano e métrica de avaliação.
...
- **Orçamento de tokens por agent.** Cada retrieve + reflect + plan por tick de cada agent é O(k) chamadas de LLM. N agents × T ticks × chamadas-por-tick pode engolir seu orçamento.
...
1. Execute `code/main.py`. Confirme que 3+ agents convergem na festa. Aumente pra 10 agents — a emergência ainda acontece?
2. Remova o passo de reflexão. Como fica o comportamento? Mapeie pra o achado de ablação do Park 2023.
3. Introduza um objetivo semente competidor ("Klaus quer dar uma palestra de pesquisa às 5pm"). Os agents se dividem ou um objetivo domina? O que determina?
...
| Importância | "Quanto o agent se importa" | Auto-avaliada 1-10 na hora da escrita. Cached. |
| Relevância | "Quão relacionada à query atual" | Similaridade cosseno (baseada em embedding). |
...
| Smallville | "O sandbox do Park 2023" | Simulação de 25 agents que produziu a emergência do Dia dos Namorados. |
| Verossimilhança | "A métrica de qualidade" | Score de avaliadores humanos sobre se o comportamento parece de um agent plausível. |
...
- [Hayes-Roth 1985 — A Blackboard Architecture for Control](https://www.sciencedirect.com/science/article/abs/pii/0004370285900639) — arte anterior pra agents com memória estruturada
