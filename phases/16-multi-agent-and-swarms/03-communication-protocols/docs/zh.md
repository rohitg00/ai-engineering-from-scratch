...
> 无法说同一种语言的智能体不是团队。它们只是对着虚空呐喊的陌生人。
...
- 比较 MCP（工具访问）、A2A（智能体间）、ACP（企业审计）和 ANP（去中心化信任），并解释每种协议解决什么问题
- 在单个系统中将多种协议串联起来，智能体通过 MCP 发现工具，通过 A2A 委托任务

## 问题

你将系统拆分为多个智能体。一个研究型、一个编码型、一个审查型。它们各自擅长自己的工作。但现在你需要它们真正相互交流。

你的第一次尝试很直观：传递字符串。研究者返回一段文本，编码者尽其所能地解析它。这种方法在编码者误解研究摘要、或两个智能体相互等待导致死锁、或你需要不同团队构建的智能体协作之前，都还能工作。突然之间，"只传递字符串"就崩溃了。

这就是通信协议问题。没有共享的信息交换契约，多智能体系统就会变得脆弱、不可审计，并且无法扩展到超过你自己编写的少数几个智能体。

AI 生态系统已经给出了四种协议，每种解决不同方面的问题：
...
本节课将深入探讨。你将阅读每个规范的真实线格式，构建可工作的实现，并将全部四种协议连接成一个统一的系统。

## 概念

### 协议全景

把这四种协议想象成层次结构，每一层解决一个不同的问题：

```mermaid
flowchart TD
  ANP["ANP — 智能体如何信任陌生人？<br/>去中心化身份（DID）、端到端加密、元协议"]
  A2A["A2A — 智能体如何在目标上协作？<br/>智能体卡片、任务生命周期、流式传输、协商"]
...
它们不是竞争关系。它们在不同层次解决不同问题。

### MCP（回顾）

MCP 在第 13 阶段已详细介绍。快速回顾：MCP 标准化了 LLM 如何连接到外部工具和数据源。它是一种**客户端-服务器**协议，智能体（客户端）发现并调用服务器暴露的工具。
...
MCP 是**智能体到工具**的通信。它不帮助智能体之间相互通信。

### A2A（智能体到智能体协议）

**创建者：** Google（现归 Linux 基金会管理，协议名为 `lf.a2a.v1`）
**规范版本：** 1.0.0
**问题：** 自治智能体如何协作、协商和相互委托任务？

A2A 是**点对点智能体协作**的协议。MCP 连接智能体与工具，而 A2A 连接智能体与其他智能体。每个智能体在知名 URL 上发布一张**智能体卡片**，其他智能体可以发现、协商并向其委托任务。
...
以下是 A2A 智能体卡片的真实样貌。在 `GET /.well-known/agent-card.json` 上提供：

```json
{
  "name": "Research Agent",
  "description": "搜索文档并汇总发现",
  "version": "1.0.0",
  "supportedInterfaces": [
    {
...
需要关注的关键点：
- **技能（Skills）** 是智能体能做的事情。每个技能有 ID、标签和支持的输入/输出 MIME 类型。客户端智能体据此判断该远程智能体能否处理它的请求。
- **supportedInterfaces** 列出多个协议绑定。单个智能体可以同时使用 JSON-RPC、REST 和 gRPC。
- **安全（Security）** 内置于卡片中。客户端在发出第一个请求之前就知道需要什么认证。

#### 任务生命周期

任务是 A2A 中的核心工作单元。它们经历已定义的状态：
...
一旦任务达到终态，它就不可变。不再有后续消息。后续操作在同一个 `contextId` 内创建一个新任务。
...
**问题：** 智能体如何在完全可审计性、会话连续性和轨迹跟踪下进行通信？

ACP 是**企业级协议**。与许多摘要声称的不同，ACP **不**使用 JSON-LD。它通过 OpenAPI 定义了一个直接的 REST/JSON API。它的特别之处在于**轨迹元数据（TrajectoryMetadata）**：每个智能体响应都可以携带一份详细日志，记录产生该响应的推理步骤和工具调用。
...
ACP 使用"运行（Runs）"而不是"任务（Tasks）"。一个 Run 是一次智能体执行，有三种模式：
...
这是 ACP 的关键区别。每个消息部分都可以包含元数据，精确显示智能体做了什么：
...
对于受监管的行业来说，这是金矿。每个答案都附带可证明的推理链：调用了哪些工具，使用了什么输入，收到了什么输出。没有黑箱。
...
每个引用都精确指向答案中受影响的范围，使审计员能够逐句验证来源。

### ANP（智能体网络协议）

**创建者：** 开源社区（由 GaoWei Chang 创立）
**仓库：** [github.com/agent-network-protocol/AgentNetworkProtocol](https://github.com/agent-network-protocol/AgentNetworkProtocol)
**问题：** 来自不同组织的智能体如何在没有中心化权威的情况下相互信任？

ANP 是**去中心化身份协议**。它使用 W3C 去中心化标识符（DID）和端到端加密来构建信任。与 A2A 通过已知端点发现智能体不同，ANP 允许智能体以加密方式证明其身份。
...
ANP 使用一种称为 `did:wba`（基于 Web 的智能体）的自定义 DID 方法。DID `did:wba:example.com:user:alice` 解析为 `https://example.com/user/alice/did.json`：
...
- **`humanAuthorization`** 是 ANP 独有的。这些密钥需要明确的人工批准（生物识别、密码、HSM）才能使用。资金转账等高危操作通过此路径执行。
- **`keyAgreement`** 密钥用于 HPKE 端到端加密（RFC 9180）。
- **service** 部分链接到智能体描述文档。

#### ANP 中的信任如何工作

ANP **不**使用信任网络或背书图。信任是双向的，每次交互时验证：
...
sequenceDiagram
    ...
    B-->>A: 颁发访问令牌
    A->>B: 后续请求使用令牌
    Note over A,B: 信任 = TLS 域名验证<br/>+ DID 签名验证<br/>+ 最少信任原则
```
...
没有基于闲聊的信任传播或 PageRank 评分。你通过其 DID 直接验证每个智能体。

#### 元协议协商

这是 ANP 最具创新性的功能。当来自不同生态系统的两个智能体相遇时，它们不需要预先商定的数据格式。它们用自然语言进行协商：
...
sequenceDiagram
    ...
    Note over A,B: 智能体动态生成代码<br/>以处理商定的格式。<br/>最多 10 轮，之后超时。
```

智能体来回交互（最多 10 轮）直到它们就格式达成一致，然后动态生成代码来处理它。状态值：`negotiating`（协商中）、`rejected`（已拒绝）、`accepted`（已接受）、`timeout`（超时）。

这意味着两个以前从未见过的智能体可以在没有任何人预先定义共享模式的情况下，找出如何通信。
...
| **审计追踪** | 无 | 基础（任务历史） | 轨迹元数据（工具调用、推理） | 未正式规定 |
| **状态机** | 无 | 9 种任务状态 | 7 种运行状态 | 无 |
| **流式传输** | 无 | SSE | SSE | 传输无关 |
...
注意：`MessagePart` 是多模态的（文本、结构化数据、文件），就像真实的 A2A 和 ACP 规范一样。`TrajectoryEntry` 捕获推理链，与 ACP 的 TrajectoryMetadata 匹配。
...
这比简单的名称到能力映射要丰富得多。你可以按技能标签、按输入 MIME 类型或按名称发现智能体，就像真实的 A2A 规范所支持的那样。
...
这实现了真实的 A2A 任务生命周期：已提交、进行中、需要输入、终态。处理程序是异步生成器，产生与 SSE 流模型匹配的事件（状态更新和工件块）。
...
每次智能体执行都会生成一个完整的审计条目：输入了什么、输出了什么、以及中间所有工具调用和推理步骤的完整轨迹。你可以按智能体、按会话或按单个运行进行查询。
...
这反映了真实的 ANP 身份模型：智能体拥有 DID 文档，包含独立的认证、密钥协商和人工授权密钥。`IdentityRegistry` 模拟 DID 解析（在生产中，这将是通过 HTTP 获取智能体域名的文档）。
...
class ProtocolGateway {
  ...
  async delegateTask(
    ...
    if (!card) {
      return { error: `注册表中未找到智能体 ${targetAgent}` };
    }
  ...
  taskManager.registerHandler(
    ...
    async function* (task, message) {
      ...
      researchTrajectory.push({
        ...
        toolName: "web_search",
        toolInput: { query: "React 19 编译器特性" },
        toolOutput: {
...
协议解决了理想路径。以下是生产环境中可能出问题的地方：

**模式漂移。** 智能体 A 发布了一张智能体卡片，声明输出为 `application/json`。但 JSON 模式在不同版本之间发生了变化。智能体 B 解析旧格式，得到垃圾数据。修复方式：对技能和输出模式进行版本管理。A2A 规范支持在 Agent Card 上使用 `version` 正是为此。

**状态机违规。** 一个智能体处理程序产生了 `completed` 事件，然后又试图产生更多工件。任务是不可变的。你的代码要么静默丢弃更新，要么抛出异常。修复方式：在产生事件之前检查终态。上面的 `TaskManager` 通过在终态之后执行 `break` 来强制执行此规则。

**信任解析失败。** 智能体 A 试图验证智能体 B 的 DID，但智能体 B 的域名宕机了。DID 文档无法获取。你是故障开放（接受未验证的智能体），还是故障关闭（拒绝一切）？ANP 推荐故障关闭，并遵循最少信任原则。

**轨迹膨胀。** ACP 轨迹日志功能强大但代价高昂。一个复杂的智能体每次运行可能进行 200 次工具调用，产生庞大的审计条目。修复方式：以可配置的详细级别记录轨迹。为合规记录工具名称和 IO，为非监管工作负载跳过推理步骤。

**发现惊群效应。** 50 个智能体在启动时同时查询 `GET /agents`。修复方式：使用可配置的 TTL 缓存智能体卡片，错开发现间隔，或使用基于推送的注册而非轮询。
...
**ACP** 正在合并到 A2A 中。IBM 的 [BeeAI 项目](https://github.com/i-am-bee/acp)创建了 ACP 作为 REST 优先的替代方案，但轨迹元数据概念正在被吸收到 A2A 生态系统中。即使你使用 A2A 作为传输层，也可以使用 ACP 模式（轨迹日志、运行生命周期）。

**ANP** 是最实验性的。[社区仓库](https://github.com/agent-network-protocol/AgentNetworkProtocol)提供了一个 Python SDK（AgentConnect）。元协议协商概念确实是新颖的。对于跨组织的智能体部署值得关注。

**MCP** 已在第 13 阶段介绍过。如果你希望智能体使用工具，MCP 是标准。
...
1. **多跳任务委托。** 扩展 `TaskManager`，使智能体处理程序可以将子任务委托给其他智能体。研究者接收一个任务，将"搜索"和"总结"子任务委托给两个专业智能体，等待两者都完成，然后将结果合并到自己的工件中。

2. **流式审计追踪。** 修改 `AuditableRunner` 以支持流式模式。不是等待完整结果，而是在添加上下文条目时实时产生 `AuditEntry` 更新。使用一个产生审计快照的异步生成器。

3. **DID 轮换。** 向 `IdentityRegistry` 添加密钥轮换功能。智能体应该能够发布带有更新密钥的新 DID 文档，同时保留一个 `previousDid` 引用。在宽限期内，验证者应同时接受当前和之前的密钥的签名。

4. **协议协商。** 实现 ANP 的元协议概念。两个智能体交换带有候选格式的 `protocolNegotiation` 消息（例如，"我能说 JSON-RPC" vs "我更喜欢 REST"）。最多 3 轮后，它们达成一致的格式或超时。协商的格式决定它们使用哪个 `TaskManager` 或 `AuditableRunner`。

5. **限速发现。** 添加一个 `RateLimitedRegistry` 包装器，使用可配置的 TTL 缓存智能体卡片查找，并限制每个智能体每秒的发现查询次数。模拟 100 个智能体在启动时相互发现的惊群效应，并测量差异。

## 关键术语

| 术语 | 人们说的 | 实际含义 |
|---|---|---|
| MCP | "AI 工具的协议" | 一种客户端-服务器协议，用于智能体发现和使用工具。智能体到工具，而非智能体到智能体。 |
| A2A | "Google 的智能体协议" | 一种在 Linux 基金会下的点对点智能体协作协议。通过智能体卡片发现、9 状态任务生命周期、通过 SSE 流式传输。支持 JSON-RPC、REST 和 gRPC 绑定。 |
| ACP | "企业级智能体消息" | IBM/BeeAI 的 REST API，用于带 TrajectoryMetadata 的智能体运行：每个响应携带完整的推理链和工具调用。正在合并到 A2A。 |
| ANP | "去中心化智能体身份" | 一种社区协议，使用 `did:wba`（DID）进行加密身份、HPKE 进行端到端加密，以及 AI 驱动的元协议协商，适用于从未见过对方的智能体。 |
| Agent Card | "智能体的名片" | 位于 `/.well-known/agent-card.json` 的 JSON 文档，描述技能、支持的 MIME 类型、安全方案和协议绑定。 |
| DID | "去中心化 ID" | W3C 标准，用于托管在智能体自身域名的加密可验证身份。ANP 使用 `did:wba` 方法。 |
| TrajectoryMetadata | "审计收据" | ACP 的机制，用于将推理步骤、工具调用及其输入/输出附加到每个智能体响应上。 |
| 元协议 | "智能体协商如何对话" | ANP 的方法，智能体使用自然语言动态商定数据格式，然后生成代码来处理它们。 |
| Task | "工作单元" | A2A 的有状态对象，从提交到完成追踪工作。一旦进入终态则不可变。 |
...
- [模型上下文协议文档](https://modelcontextprotocol.io/) -- Anthropic 的 MCP 规范（在第 13 阶段介绍）
