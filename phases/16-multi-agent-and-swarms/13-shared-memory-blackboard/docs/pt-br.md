...
**Pool de mensagens completo.** Cada agent lê cada mensagem. AutoGen GroupChat e MetaGPT usam isso. Simples, transparente, inspecionável, mas não escala além de ~10 agents porque o contexto de cada agent se enche do trabalho dos outros.
...
- **Pool completo** vence quando são poucos agents (< 10), heterogêneos, e a conversa é de curto prazo. Raciocinar sobre quem disse o que é trivial quando todo mundo vê tudo.
...
Três agents trabalham numa tarefa de pesquisa. Agent A é um agent de retrieval. Agent B é um sumarizador. Agent C é um analista.
...
Nenhum agent crashou. Nenhum teste falhou. O sistema "funcionou." A alucinação cruzou do contexto de um agent pra cada raciocínio downstream via estado compartilhado.

### Por que isso é estrutural

Sem estado compartilhado, a alucinação do agent A ficaria no contexto do A. Agents downstream refariam o retrieval ou o raciocínio e poderiam pegar o erro. Com estado compartilhado ingênuo, o contexto do A vira o contexto de todo mundo, e a alucinação é lavada em fato.
...
1. **Atribua proveniência em cada escrita.** Cada entrada no estado compartilhado registra quem escreveu, quando, sob qual prompt, e (se aplicável) qual fonte o agent citou. Agents downstream leem com ceticismo calibrado pela proveniência.
...
3. **Mantenha pelo menos um agent que não pode escrever no estado compartilhado.** Um agent verificador read-only amostra entradas, refaz buscas de fontes e sinaliza inconsistências. Porque não pode escrever no pool, não pode ser envenenado pelo pool.

### Precedente de blackboard (Hayes-Roth, 1985)

O padrão blackboard precede agents LLM em quatro décadas. Hayes-Roth (1985, "A Blackboard Architecture for Control") descreveu Knowledge Sources especializadas que observam um blackboard global, contribuem soluções parciais, e disparam outras sources. O blackboard de 2026 (CA-MCP, Matrix) é o mesmo padrão com agents LLM como Knowledge Sources e blobs JSON como soluções parciais. A literatura antiga tem soluções documentadas pra contenção de escritas, controle oportunístico e consistência que sistemas modernos redescobrem.

### Projeção vs visão completa

Um blackboard puro dá a cada subscriber a mesma projeção (limitada por tópico). Um design mais agressivo é **projeção por agent**: cada agent recebe uma visão customizada pro seu papel. Os state reducers do LangGraph são a implementação canônica de 2026 — a função reducer dobra o estado global num slice específico pro papel.

A projeção por agent escala mais, mas precisa de schema. Sem um, você reconstrói projeção ad-hoc no prompt de cada agent.

### Padrões de contenção de escrita

Múltiplos agents escrevendo simultaneamente é um problema de concorrência, não só de LLM. Três padrões funcionam:

- **Escritor sequencial (produtor único).** Todas as escritas passam por um agent coordenador que serializa. Simples, mas gargalo.
...
- As saídas do verificador são roteadas pra um humano ou pra um agent de decisão separado, nunca voltam pro pool.
...
- `PoisoningScenario` — roda uma tarefa de pesquisa de três agents onde o agent A alucina uma casa decimal. Imprime o relatório final.
- `Verifier` — um agent read-only que refaz buscas de fontes e sinaliza inconsistências. Roda o mesmo cenário com o verificador presente.
...
- Deploy pelo menos um agent verificador read-only com acesso independente a fontes.
...
2. Adicione uma segunda alucinação: o agent B inventa um tamanho de dataset. O verificador deveria pegar as duas sem ser ajustado manualmente pra nenhuma.
...
|-------|-------------------|--------------------------|
| Message pool | "Histórico de chat compartilhado" | Log append-only que cada agent lê. Transparência total, escalabilidade ruim. |
...
| Envenenamento de memória | "Alucinações se espalhando" | O erro de um agent entra no estado compartilhado, agents downstream adotam como fato. |
...
| Projeção | "Visão escopada" | Visão por agent computada do estado global. Os reducers do LangGraph são o caso canônico. |
...
- [LangGraph state and reducers](https://docs.langchain.com/oss/python/langgraph/workflows-agents) — o padrão de projeção por agent em produção
- [Anthropic — How we built our multi-agent research system](https://www.anthropic.com/engineering/multi-agent-research-system) — notas sobre proveniência e verificação de um deploy em produção
