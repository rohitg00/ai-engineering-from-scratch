...
> Dinamik bir komisyoncu her isteği değerlendirir (görev türü, token uzunluk, embedding benzerlik, güven) ve basit sorguları ucuz bir modele göndererek karmaşık olanları bir sınır modeline yükseltir. Ayrıca model basamaklı olarak da adlandırılır. Üretim örnek olay incelemeleri, ABD/İngiltere/AB deployment'lerde izo kalitesinde %20-60 maliyet düşüşü olduğunu gösteriyor; Yüksek hacimli SaaS'ta %30'luk yönlendirme verimliliği artışı, altı haneli yıllık tasarruflara dönüşüyor. 2026 bağlamı, LLM inference fiyatlarının yılda yaklaşık 10 kat düştüğü yönünde — GPT-4 sınıfı token, 2022 sonlarından 2026'ya kadar $20/M to ~$0,40/M'den düştü. Düşüşün çoğu, donanımdan değil, daha iyi hizmet veren yığınlardan (Aşama 17 · 04-09) kaynaklanıyor. Yönlendirme, ürün gerilemesi olmadan bu fiyat düşüşünü marja nasıl dönüştürdüğünüzdür. Başarısızlık modu ucuz model sürüklenmesidir: rota %40'ı daha zayıf bir modele iter, muhakeme görevlerinde kalite %3-5 düşer, çeyrek boyunca kimse fark etmez. Rotaları yalnızca çevrimdışı değerlendirme kümelerine göre değil, çevrimiçi kalite ölçümlerine göre de değerlendirin.

**Tür:** Öğren
**Diller:** Python (stdlib, oyuncak basamaklı yönlendirici simülatörü)
**Önkoşullar:** Aşama 17 · 01 (Yönetilen Yüksek Lisans Platformları), Aşama 17 · 19 (AI Ağ Geçitleri)
**Süre:** ~60 dakika

## Öğrenme Hedefleri

- Model basamaklandırmasını açıklayın: Önce ucuza güven kontrolü yapın, düşük güveni artırın.
- Dört yönlendirme sinyalini numaralandırın (görev sınıflandırması, prompt uzunluğu, bilinen sabit kümeye embedding benzerlik, ilk geçişten itibaren özgüven).
...
1. **Görev sınıflandırması**: basit/karmaşık/kod oluşturma/matematik/sohbet. Kurallara dayalı bir sınıflandırıcı, küçük bir Yüksek Lisans (0,25 ABD doları/milyon değerinde Haiku sınıfı) veya etiketli paketlere embedding benzerlik olabilir. Çıktı: rota = ucuz / dengeli / sınır.

2. **Prompt uzunluk**: prompts >4K tokens genellikle tutarlılık için sınıra ihtiyaç duyar. Prompts <500 tokens genellikle bunu yapmaz.

3. Bilinen sabit kümeye **Embedding benzerlik**: sorgu bilinen sabit kümeye yakınsa (kosinüs > 0,88), doğrudan sınıra iletin.
...
|-------------|-----------|------|--------|
| GPT-4 düzeyinde kalite | ~$20/M | ~$0,40/A | 50 kat daha ucuz |
...
`code/main.py` , karma bir iş yükünde ön rotayı, basamaklamayı ve birleştirmeyi simüle eder. Maliyeti, kalite kaybını ve artış oranını karma olarak raporlar.

## Ship It — Kullanıma Sun

Bu ders `outputs/skill-router-plan.md` üretir. İş yükü ve kalite bütçesi göz önüne alındığında, bir yönlendirme modeli ve sinyaller seçer.

## Egzersizler

1. `code/main.py`'yı çalıştırın. Basamak hangi doğruluk seviyesinde ön rotayı geçiyor?
...
- [RouteLLM ödevi / kodu](https://github.com/lm-sys/RouteLLM)
