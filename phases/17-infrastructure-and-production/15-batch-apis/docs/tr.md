...
Parti size %50 indirim sağlar. Ayrıca prompt sisteminde prompt önbelleğe almayı etkinleştirirsiniz (50 bin çağrının tamamında paylaşılır). Üst üste, fatura gecelik 180$'a düşüyor; yani taban değerin ~%9'u. Aynı işlem hattı, üç yapılandırma değişikliği.
...
**OpenAI Batch API**: İsteklerin listesini içeren JSONL dosyası yükleme. 24 saatlik geri dönüş sözü verildi (pratikte genellikle ~2-8 saat). Giriş ve çıkış token'lerde %50 indirim. `/v1/batches` uç noktası. Önbelleğe uygun girişler ayrıca önbelleğe alınmış giriş fiyatlandırmasına da sahiptir.
...
- Senkronize önbelleğe alınmamış: 50000 × ($input × 4000 + $output × 200) tam hızlarda.
- Eşzamanlı önbelleğe alınmış: prompt sistemi ilk yazmadan sonra önbelleğe alınmıştır; kalan 49999 ise 10 kat daha ucuz girdi alıyor.
- Toplu önbelleğe alma: Yukarıdakilerin hepsine ek olarak hem okuma hem de yazmada %50 indirim.

Yığın: toplu iş + önbellek = önbelleğe alınmamış senkronizasyon faturasının ~%10'u. Gece boyunca çalışan ve prompt paylaşılan sistemine sahip tüm iş yükleri bunu kullanmalıdır.
...
Bazı özellikler etkileşimli görünüyor ancak 5-10 dakikayı tolere ediyor. Örnek: "yenile" butonunun yer aldığı gecelik müşteri sağlık raporu. Kullanıcı yenilemeyi tıklar; 10 dakika bekle tamamdır. Ekip bunu senkronize olarak gönderiyor. 50 eşzamanlı yenilemenin maliyeti, toplu olarak e-posta yoluyla teslim edilen maliyetin 10 katıdır.

Sorulması gereken soru: "Bu kullanıcı için 24 saat ne anlama geliyor?" Cevap "fark etmezler" ise toplu işleyin.
...
Bu ders `outputs/skill-batch-triager.md`'yi üretir. İş yükü özellikleri göz önüne alındığında, etkileşimli/yarı/toplu olarak önceliklendirme yapılır ve tasarruflar tahmin edilir.

## Egzersizler

1. `code/main.py`'yi çalıştırın. 3K-token sistemi prompt ve 500-token çıkışına sahip 100k belgeli bir ardışık düzen için, senkronizasyon temeline kıyasla tam yığın (toplu + önbellek) tasarruflarını hesaplayın.
...
- [Finout — OpenAI ve Antropik API Fiyatlandırması 2026](https://www.finout.io/blog/openai-vs-anthropic-api-pricing-comparison)
