...
> Donanıma özel inference derlemesi, taşınabilirliği işlem hacmi için değiştirir ve TensorRT-LLM (yalnızca NVIDIA, Blackwell için ayarlanmıştır) bu ticaretin karşılığını almanın en açık örneğidir. Dinamo düzenlemeli GB200 NVL72'de SemiAnalytic InferenceX, H100 + vLLM'de $0.012 per million tokens on a 120B model in Q1-Q2 2026, against $0,09/M ölçtü - 7 kat ekonomik boşluk. Yığın, birleştirilmiş üç kayan nokta rejiminden oluşur: FP8, ihtiyaç duydukları dinamik aralığa sahip olduğundan KV önbelleği ve dikkat çekirdekleri için kritik olmaya devam eder; NVFP4 (4 bit mikro ölçeklendirme), ağırlıkları ve etkinleştirmeleri yönetir; çoklu-token tahmini (MTP) ve ayrıştırılmış önceden doldurma/kod çözme, üstüne 2-3 kat daha ekler. Day-0 modeli, FP4 ağırlıklarını eğitim sonrası dönüştürmeye gerek kalmadan doğrudan yükler. 2026 mühendislik ekipleri için önemli nokta: TRT-LLM açık kaynaktır ancak NVIDIA'ya özeldir (CUDA ve Blackwell'de uzmanlaşmıştır), bu nedenle onu benimsemek, verim için taşınabilirliği değiştirir. Taahhüt etmeden önce model ve donanım karışımınızda matematiği çalıştırın.
...
2026'da inference ekonomisinin sınırı "dolar başına kaç token" olacaktır. Cevap dört üst üste seçeneğe bağlıdır: donanım üretimi (Hopper H100/H200 ile Blackwell B200/GB200), hassasiyet (BF16 → FP8 → NVFP4), hizmet motoru (vLLM, SGLang, TRT-LLM) ve orkestrasyon (düz, ayrıştırılmış ve Dinamo).

vLLM'li Hopper'da, 120 milyar MoE ~$0.09 per million tokens. On Blackwell with TRT-LLM + Dynamo, the same model runs at ~$0,012 — 7 kat daha ucuza çalışır. Bu boşluğun bir kısmı donanımdan kaynaklanıyor (Blackwell, Hopper'a kıyasla GPU başına LLM verimi 11-15x). Bunlardan bazıları yığındır: FP4 ağırlıkları, MTP taslağı, ayrıştırılmış ön doldurma/kod çözme ve MoE uzman iletişimi için hepsi bir arada NVLink 5.
...
- **Çoklu-token tahmini (MTP)**: EAGLE (Aşama 17 · 05) ile aynı fikir ancak TRT-LLM yapısına entegre edilmiştir.
...
- TRT-LLM aracılığıyla GPT-OSS-120B'de HGX B200 $0,02/M tokensn.
- Dinamo aracılığıyla GB200 NVL72 $0,012/M tokens (TRT-LLM'yi yönetiyor).
- Karşılaştırılabilir iş yükünde H100 + vLLM ≈ $0,09/M tokens.
...
100 milyon ABD dolarının üzerinde yıllık inference tutarındaki fatura için Hopper + vLLM'de çalışmak, masada 7-10 kat bırakır. Maliyet ağırlıklı iş yüklerini Blackwell + TRT-LLM + Dynamo'ya taşıyın. Model yineleme hızı için deneme katmanını H100 + vLLM'de tutun. NVFP4'e dönüştürülen her modelin kalitesini üretimden önce doğrulayın.
...
`code/main.py` , üç yığındaki bir model için HBM ayak izini, kod çözme verimini (belleğe bağlı rejim) ve $/M-token'lari hesaplar: H100 + BF16 + vLLM, H100 + FP8 + vLLM, B200 + NVFP4/FP8 + TRT-LLM. Bileşik etkiyi ve her değişikliğin katkıda bulunduğu boşluğun payını görmek için bunu çalıştırın.

## Ship It — Kullanıma Sun

Bu ders `outputs/skill-trtllm-blackwell-advisor.md` üretir. İş yükü, model boyutu ve yıllık token hacmi göz önüne alındığında, Blackwell + TRT-LLM yığınının NVIDIA kilidine değip değmeyeceğine karar verir.

## Egzersizler

1. `code/main.py`'yı çalıştırın. %30 aktif parametrelere sahip bir 120B MoE'de, H100 BF16, H100 FP8 ve B200 NVFP4/FP8'deki bellek bant genişliği sınırlı kod çözme verimini hesaplayın. En büyük sıçrama nereden geliyor?
...
| 0. Gün FP4 | "FP4 ağırlıklarını gönderin" | Model sağlayıcılar ağırlıkları halihazırda FP4'te yayınlıyor; tren sonrası dönüşüm adımı yok |
| MTP | "çoklu-token tahmin" | TRT-LLM'nin entegre spekülatif kod çözme taslağı (Aşama 17 · 05) |
...
| Hepsinden hepsine | "MEB uzman iletişimi" | token'ları uzman GPU'lara yönlendiren iletişim modeli; NVLink 5, 3 kez keser |
| InferenceX | "YarıAnaliz inference tezgahı" | 2026'da sektör tarafından kabul edilen maliyet başına maliyettoken benchmark |
...
- [NVIDIA — MoE Inference on Blackwell](https://developer.nvidia.com/blog/delivering-massive-performance-leaps-for-mixture-of-experts-inference-on-nvidia-blackwell/) — NVLink 5 hepsi bir arada ve MoE çekirdekleri.
- [TensorRT-LLM'ye Genel Bakış](https://nvidia.github.io/TensorRT-LLM/overview.html) — resmi motor belgeleri.
- [NVIDIA — Dinamoyla Tanışıyoruz](https://developer.nvidia.com/blog/introducing-nvidia-dynamo-a-low-latency-distributed-inference-framework-for-scaling-reasoning-ai-models/) — TRT-LLM'nin üzerinde ayrıştırılmış orkestrasyon.
