...
> Spekülatif kod çözme, hızlı taslak modeli hedef modelle eşleştirir. Taslak K token'ları öneriyor; hedef tek bir ileri yönde doğrulanır; kabul edilen token'lar ücretsizdir. 2026'da, EAGLE-3 üretim sınıfı varyanttır; taslak kafayı ham token'lar yerine hedef modelin gizli durumları üzerinde eğitir ve genel sohbette kabul oranı alfa'yı 0,6-0,8 bandına iter. Doğru soru "taslak ne kadar hızlı" değil, "trafiğimde alfa nedir?" Alfa ~0,55'in altına düşerse spekülatif kod çözme yüksek eşzamanlılıkta net olumsuzdur çünkü reddedilen her taslak ikinci bir hedef ileri geçişe mal olur. Bu ders size önce alfayı ölçmeyi, sonra bayrağı ters çevirmeyi öğretir.

**Tür:** Öğren
**Diller:** Python (stdlib, oyuncak kabul oranı simülatörü)
**Önkoşullar:** Aşama 17 · 04 (Motorun Dahili Bileşenlerine Hizmet Verme), Aşama 10 · 18 (Çoklu-Token Tahmin)
**Süre:** ~60 dakika
...
- Bir ölçüm planı yazın: hangi benchmark, hangi prompt dağılımı, hangi eşzamanlılık noktası, hangi metriğe geçilecek.

## Sorun

Kod çözme belleğe bağlıdır. Llama 3.3 70B FP8 çalıştıran bir H100'de, kodu çözülen her token, ~140 GB/sn ağırlıkları okur ve bir token yayar. GPU hesaplaması kod çözme sırasında neredeyse boştadır; darboğaz, matmul verimi değil, HBM bant genişliğidir.

Spekülatif kod çözme, boşluktan yararlanır. Ucuz bir taslak modelle K aday token'lar oluşturun, ardından hedef modelden tek bir ileri geçişte tüm K'yı doğrulamasını isteyin. Doğrulanan her token fiilen ücretsizdir (her halükarda hedefin yapmak zorunda kalacağı bir K partisi halinde amortismana tabi tutulur).
...
Önemli nokta: EAGLE-3, vLLM 2026'da isteğe bağlıdır. `speculative_config` açıkça ayarlanmalıdır. Bayrak yok, hızlanma yok. Gerçek trafiklerinde alfa değerini ölçmeden bu özelliği açan ekipler genellikle kuyruk gecikmesinin iyileşmek yerine daha da kötüleştiğini görür.

## Konsept

### Spekülatif kod çözme aslında neyi satın alır?

Özel kod çözme olmadan, token başına maliyet bir hedef ileridir. K taslak uzunluğundaki spesifikasyon kod çözme ve kabul alfa ile, hedef başına beklenen tokens ileri: `1 + K * alpha`. Hızlanma `(1 + K * alpha) / (1 + epsilon)` olup epsilon taslak artı doğrulama ek yüküdür. K=5 için alfa=0,7: `(1 + 5*0.7) / (1 + 0.1) = 4.5 / 1.1 = 4.1x`. Gerçek dünyadaki sayılar 2-3 kat civarında kümeleniyor çünkü alfa üretim trafiğinde nadiren bu kadar yüksek oluyor ve epsilon yüksek parti boyutunda büyüyor.

### Neden önemli olan tek ölçüm alfadır?

Reddedilen token'lar kaybolmaz; ilk reddedilen token için ikinci bir hedefi ileri doğru zorlarlar. Alfanın 0,4'e düştüğü bir iş yükünde, taslak genel gider artı doğrulama artı yeniden kayıt ücreti ödersiniz. Yüksek eşzamanlılıkta (örneğin 256 eşzamanlı), kod çözme grubu zaten "yalnız hedef" ile "doğrulamalı hedef" arasındaki bellek bant genişliği boşluğunu daraltacak kadar büyüktür. Çoğu 2026 donanımında alfa 0,55'in altında, spesifikasyon kod çözme net negatiftir.
...
2. vLLM `speculative_config` aracılığıyla EAGLE-3 taslağını etkinleştirin. benchmark'ı yeniden çalıştırın.
...
- Çok kısa çıkışlar (50 tokens'nin altında). Genel masrafların taslağını çıkarın ve maliyetin hakim olduğunu doğrulayın.
...
Bu ders `outputs/skill-eagle3-rollout.md` üretir. Bir hedef modeli, trafik dağıtım açıklaması ve eşzamanlılık hedefi göz önüne alındığında, aşamalı bir EAGLE-3 kullanıma sunma planı üretir - benchmark temel, yapılandırmayı etkinleştir, alfayı ölç, alfa üzerinde kapı >= 0,55, P99 ITL'yi izle.

## Egzersizler

1. `code/main.py`'yı çalıştırın. K=5'te 2 kat hızlanma için hangi alfaya ihtiyacınız var? 3 kat hızlanma için mi? Verify_overhead için bu ne kadar hassas?
...
|------|----------------|------------------------|
| Spekülatif kod çözme | "taslak artı doğrulama" | Ucuz bir modelle K token'ları önerin, tüm K'yi tek bir hedefte doğrulayın |
| Kabul oranı alfa | "özellik kabul oranı" | Hedef tarafından kabul edilen token taslaklarının kesri; önemli olan tek ölçüm |
...
| N-gram spesifikasyon kodunun çözülmesi | "N-gram taslak" | prompt'da N gram aramalarını kullanan GPU tarafı taslağı; parçalanmış önceden doldurma uyumlu |
...
- [vLLM — Spekülatif Kod Çözme belgeleri](https://docs.vllm.ai/en/latest/features/spec_decode/) — `speculative_config` konusunda yetkili kaynak ve V1'de parçalanmış önceden doldurma uyumluluğu.
- [vLLM Spekülatif Yapılandırma API'si](https://docs.vllm.ai/en/latest/api/vllm/config/speculative/) — tam alan kümesi.
- [EAGLE makalesi (arXiv:2401.15077)](https://arxiv.org/abs/2401.15077) — orijinal EAGLE taslak kafası formülasyonu.
- [EAGLE-2 makalesi (arXiv:2406.16858)](https://arxiv.org/abs/2406.16858) — uyarlanabilir taslaklar ve ağaçlar.
- [UC Berkeley EECS-2025-224](https://www2.eecs.berkeley.edu/Pubs/TechRpts/2025/EECS-2025-224.html) — spekülatif kod çözme özelliğine sahip verimli Yüksek Lisans sistemi.
