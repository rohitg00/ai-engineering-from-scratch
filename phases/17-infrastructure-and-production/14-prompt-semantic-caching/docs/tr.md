...
> Önbelleğe alma iki katmanda gerçekleşir. L2 (sağlayıcı düzeyi) prompt/prefix önbelleğe alma, tekrarlanan önekler için dikkat KV'sini yeniden kullanır — Anthropic'in prompt-önbellekleme belgeleri, uzun prompt'larda %90'a varan maliyet düşüşü ve %85 gecikme azalmasının reklamını yapar; Claude 3.5 için Sonnet önbellek okumaları, 5 dakikalık TTL ve 1 saatlik TTL seçeneği için 2 kat yazma primiyle $0.30/M vs $3,00/M tazedir (docs.anthropic.com, 2026-04). OpenAI prompt önbelleğe alma, prompts ≥1024 tokens için otomatik olarak uygulanır ve önbelleğe alınan girişi yeniye kıyasla yaklaşık %90 indirimle fiyatlandırır (platform.openai.com, 2026-04); Model başına önbelleğe alınan kesin ücret, canlı ücret listesine bağlıdır. L1 (uygulama düzeyinde) anlamsal önbelleğe alma, embedding benzerlik isabetinde LLM'yi tamamen atlar. Satıcı "%95 doğruluk" isabet oranını değil eşleşme doğruluğunu ifade eder; rapor edilen üretim isabet oranları %10'dan (açık uçlu sohbet) %70'e (yapılandırılmış SSS) kadar değişir; hiçbir sağlayıcı resmi bir temel yayınlamadığından bunları garanti yerine topluluk telemetrisi olarak değerlendirin. Üretim tuzakları: paralelleştirme, önbelleğe almayı ortadan kaldırır (ilk önbelleğe yazma işleminden önce gönderilen N paralel istek, harcamayı birkaç kat artırabilir) ve önek içindeki dinamik içerik, önbellek isabetlerini tamamen önler. ProjectDiscovery, dinamik metni önbelleğe alınabilir önekten kaldırarak isabet oranının %7'den %74'e (2025-11) çıktığını bildirdi.
...
- L2 prompt/ön ek önbelleğe almayı (sağlayıcıda KV'nin yeniden kullanımı) L1 anlamsal önbelleğe almadan (benzer prompt'larde LLM atlaması) ayırın.
- Anthropic'in açık `cache_control` işaretlemesini ve iki TTL seçeneğini (5 dakika ve 1 saat) fiyat çarpanlarıyla birlikte açıklayın.
- İsabet oranı, prompt/yanıt karışımı ve token fiyatlarına göre beklenen aylık tasarrufları hesaplayın.
...
RAG hizmetinize prompt önbelleğe almayı eklersiniz. Tasarı sabit kalıyor. İsabet oranını ölçersiniz; %7'dir. prompt'lariniz statik görünüyor ama değiller — prompt sistemi, dakikaya göre biçimlendirilmiş güncel tarihi, bir istek kimliğini ve çeşitlilik için rastgele bir örnek yeniden sıralamayı içerir. Her istek yeni bir önbellek girişi yazar, sıfır okur.

Ayrı olarak, agent cihazınız kullanıcı sorusu başına on paralel araç çağrısı çalıştırır. On tanesinin tümü, ilk önbellek yazımı tamamlanmadan önce sağlayıcıya ulaşır. On yazıyor, sıfır okuyor. Faturanız, "önbelleklemeyle" maliyetinin 5-10 katıdır.

Önbelleğe alma bir bayrak değil, bir protokoldür. İki katman, iki farklı arıza modu.

## Konsept

### L2 — sağlayıcı prompt/prefix önbelleğe alma

Sağlayıcı, önbelleğe alınabilir bir önek için dikkat KV'sini saklar ve önekle eşleşen bir sonraki istekte onu yeniden kullanır. Bir kez yazma ücreti ödersiniz, okuma neredeyse bedavadır.

**Antropik (Claude 3.5 / 3.7 / 4 serisi)**: istekte açık `cache_control` işaretçisi. Hangi blokların önbelleğe alınabileceğini etiketlersiniz. TTL: 5 dakika (yazma maliyeti taban 1,25x) veya 1 saat (yazma maliyeti taban taban 2 kat). Önbellek şunu okur: $0.30/M on Claude 3.5 Sonnet vs $3,00/M taze — 10 kat daha ucuz (docs.anthropic.com, 2026-04 itibarıyla). Fiyatlar modele göre farklılık gösterir (Opus/Haiku ayrı olarak yayınlanır); her zaman canlı fiyatlandırma sayfasını çapraz kontrol edin.

**OpenAI**: prompts ≥1024 tokens (platform.openai.com, 2026-04) için otomatik önbelleğe alma. Açık bir bayrak yok. Önbelleğe alınmış giriş, mevcut gpt-4o/gpt-5 ücret listelerinde taze girişten yaklaşık 10 kat daha ucuzdur. Ne belgeler ne de sürüm notları resmi bir isabet oranı temeli yayınlamaz; topluluk raporları dikkatli bir prompt tasarımıyla %30-60 civarında kümelenir. Kendinizinkini ölçmek için `usage.cached_tokens` 'yi izleyin.
...
LLM'yi çağırmadan önce, prompt'u hashleyin, gömün ve önbelleğe alınmış benzer bir isteği arayın (eşiğin üzerindeki kosinüs benzerliği, genellikle 0,95+). İsabet halinde önbelleğe alınan yanıtı döndürün. Kaçırıldığında LLM'yi arayın ve sonucu önbelleğe alın.
...
- Ses agent tekrarlayan prompts: %50-80 (ses normalleştirmesi sabit ayarlı).

### Paralelleştirme anti-örüntüsü

agent cihazınız paralel olarak 10 takım çağrısı yapar. 10 tanesinin tümü aynı 4K-token sistemine prompt sahiptir. Antropik önbellek yazma işlemleri istek başına yapılır; İlk önbellek yazma işlemi, sağlayıcı prompt'u gördükten yaklaşık 300 ms sonra tamamlanır. 2-10 arasındaki istekler aynı milisaniyelik pencerede gelir ve her biri önbellek eksikliğini görür. 10 yazma primi, 0 okuma indirimi ödersiniz.
...
prompt sisteminiz şuna benziyor:
...
ProjectDiscovery önbellek isabet oranını bu şekilde %7'den %74'e çıkardı ve anatomiyi yayınladı.
...
- Antropik önbelleğe alınmış okuma: Claude 3.5 Sonnet'te 0,30 ABD doları/milyon, yeni girdiden (docs.anthropic.com) yaklaşık 10 kat daha ucuz.
- Antropik önbellek yazma premiumu: 1,25x (5 dakikalık TTL) veya 2x (1 saatlik TTL).
- OpenAI otomatik önbellek: prompts ≥1024 tokens için geçerlidir; önbelleğe alınmış giriş, mevcut ücret listelerindeki (platform.openai.com) yeni girişin kabaca %10'u kadar fiyatlandırılır.
...
Bu ders `outputs/skill-cache-auditor.md` üretir. prompt şablonu ve trafiği göz önüne alındığında, önbelleğe alınabilirliği denetler ve yeniden yapılandırma önerir.

## Egzersizler

1. `code/main.py`'yı çalıştırın. Paralelleştirme bayrağını açın/kapatın. Tasarı ne kadar değişiyor?
...
4. Anlamsal önbellek 0,95 eşiğinde %20'ye ulaşır. 0,85'te %50'ye ulaşır ancak önbelleğe alınmış yanlış yanıtları görürsünüz. Doğru eşiği seçin ve gerekçelendirin.
...
| Dinamik içerik tuzağı | "zaman-prompt tuzağı" | Önek öldürme isabet oranındaki dinamik baytlar |
...
- [ProjectDiscovery — Prompt Önbelleğe Alma](https://projectdiscovery.io/blog/how-we-cut-llm-cost-with-prompt-caching) ile Yüksek Lisans Maliyetlerini %59 Azaltın
