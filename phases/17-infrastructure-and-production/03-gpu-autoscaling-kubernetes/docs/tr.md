...
- Çalışan GPU işlerini sonlandıran Karpenter birleştirme politikasını (`WhenEmptyOrUnderutilized`) adlandırın ve 2026 güvenli alternatifini belirtin.
...
Ayrı olarak, düğümler için Küme Otomatik Ölçekleyiciyi kullanırsınız. 1M-token prompt, 2 a.m'ye varıyor.; küme bir düğümün hazırlığını yapmak için 3 dakika harcar ve istek zaman aşımına uğrar.

Ayrı olarak, yine 2 düğümde 8 GPU gerektiren bir 70B modelini dağıtırsınız. Kümede 7 ücretsiz GPU bulunur ve 1 tanesi 3 düğüme dağılmıştır. Küme Otomatik Ölçekleyici, eksik 1 GPU için bir düğüm hazırlar. Kubernetes son GPU'yu çalıştırırken yedi düğüm 4 dakika bekleyerek para yakıyor.
...
Karpenter, bekleyen bölmeleri izler ve düğümleri yaklaşık 45-60 saniye içinde hazırlar (Küme Otomatik Ölçekleyici, GPU düğümleri için genellikle 90-120 saniye sürer). Örnek türlerini `NodePool` kısıtlamasına göre dinamik olarak seçer; kapsülünüzün 8 H100'e ihtiyacı varsa ve kümede eşleşen düğüm yoksa Karpenter, mevcut bir grubu ölçeklendirmek yerine doğrudan bir tane tedarik eder.

**Konsolidasyon tuzağı**: Karpenter'in varsayılan `consolidationPolicy: WhenEmptyOrUnderutilized` değeri GPU havuzları için tehlikelidir. Pod'ları daha ucuz, doğru boyutlu bir örneğe taşımak için çalışan bir GPU düğümünü sonlandıracaktır. inference iş yükü için bu, çalışan isteklerin çıkarılması ve 70B modelinin yeni düğüme yeniden yüklenmesi anlamına gelir. Kayıp, dakikalarca kapasite artı istek hatalarıdır.
...
**Grup planlaması** — ya hep ya hiç planlayın. 8 GPU gerektiren dağıtılmış bir inference bölmesi ya 8'inin tamamı birlikte başlar ya da hiçbiri başlamaz. Bu olmadan kısmi tahsis tuzağına düşersiniz: 8 bölmeden 7'si başlar, süresiz olarak bekler, para yakar.
...
Daha da kötüsü, vLLM ve benzeri motorlar KV önbelleğini önceden tahsis eder (en fazla `--gpu-memory-utilization`). Bellek kullanımı tek istekte bile %90'a yakın kalıyor. Bellek tabanlı HPA'nın ölçeği asla küçülmez.
...
Soğuk başlatma azaltma (Aşama 17 · 10), düğüm sağlama süresinin kullanıcı tarafından görülebildiği yerdir. Karpenter'in 45-60 saniyelik ısınması artı 20 GB model yükü artı motorun başlatılması, sıfırdan gelen bir isteğin 2-5 dakika süreceği anlamına gelir. SLO açısından kritik yollar için sıcak bir havuz (`min_workers=1`) tutun veya uygulama katmanında Modal tarzı kontrol noktası kullanın.
...
Bu ders `outputs/skill-gpu-autoscaler-plan.md` üretir. Küme topolojisi, iş yükü şekli ve SLO göz önüne alındığında, üç katmanlı bir otomatik ölçeklendirme planı tasarlar.

## Egzersizler

1. `code/main.py`'yı çalıştırın. Yoğun bir iş yükü altında, sıra derinliğindeki HPA'nın yakaladığı saf görev döngüsü HPA'sı kaç isteği düşürür? Fark nereden geliyor?
...
3. Ekibiniz "GPU'lar mevcut ancak kapsül zamanlanmıyor" nedeniyle deployment'ların Beklemede kaldığını bildirdi. Teşhis — bu Karpenter mi, kube-scheduler mı, yoksa KAI Scheduler mı? Hangi ölçümler doğruluyor?
...
5. P99 TTFT > 10 sn'de günde ortalama 60 istek bırakma olayı gerçekleştiren 7x24 üretim hizmetindeki `WhenEmptyOrUnderutilized` konsolidasyon tuzağının maliyetini hesaplayın.
...
- [Karpenter Kesinti Kontrolleri](https://karpenter.sh/docs/concepts/disruption/) — birleştirme politikası semantiği ve GPU güvenli varsayılanlar.
- [NVIDIA — Kubernetes'te Ayrıştırılmış LLM Inference](https://developer.nvidia.com/blog/deploying-disaggregated-llm-inference-workloads-on-kubernetes/) — Dynamo Planner ölçeklendirme sinyalleri.
- [Ray docs — RayClusters için KAI Scheduler](https://docs.ray.io/en/latest/cluster/kubernetes/k8s-ecosystem/kai-scheduler.html) — Ray entegrasyon modeli.
- [AWS EKS Bilgi İşlem ve Otomatik Ölçeklendirmeyle İlgili En İyi Uygulamalar](https://docs.aws.amazon.com/eks/latest/best-practices/aiml-compute.html) — yönetilen Kubernetes'e özel rehberlik.
