# Inference Platform Ekonomisi — Fireworks, Birlikte, Baseten, Modal, Replika, Her Ölçekte

> 2026 inference pazarı artık GPU zaman kiralaması değil. Özel silikon (Groq, Cerebras, SambaNova), GPU platformları (Baseten, Together, Fireworks, Modal) ve API öncelikli pazarlara (Replicate, DeepInfra) ayrılır. Fireworks, günde 10T+ tokensn için fiyatı $1/hr per GPU on May 1, 2026, and $4 milyar artırdı, hacim odaklı modelin işe yaradığını gösteriyor. Baseten Ocak 2026'da $300M Series E at $5B'yi kapattı. Rekabetçi konumlandırma kuralı basittir: Fireworks gecikmeyi optimize eder, Together katalog genişliğini optimize eder, Baseten kurumsal cilayı optimize eder, Modal Python'a özgü DX'i optimize eder, Replicate çok modlu erişimi optimize eder, Anyscale dağıtılmış Python'u optimize eder. Bu ders size bir kurucuya verebileceğiniz bir matris verir.
...
- "token başına" API fiyatlandırma modelinin neden donanımın değil de hizmet veren motorun maliyet eğrisine doğru sıkıştığını açıklayın.
...
- Belirli bir iş yükü için hangi platformun doğru varsayılan olduğunu belirleyin (sunucusuz patlamalı, istikrarlı yüksek verimli, ince ayarlı değişkenler, çok modlu).

## Sorun

Yönetilen hiper ölçekleyici platformlarını değerlendirdiniz. Daha dar kapsamlı, daha hızlı bir sağlayıcıya ihtiyacınız olduğuna karar verdiniz: Gecikme için Fireworks, genişlik için Together, ince ayarlı özel bir model için Baseten. Artık altı gerçek seçeneğiniz var ve fiyatlandırma sayfaları sıralanmıyor. Havai fişek gösterileri $/M tokens; Baseten shows $/dakika; Modal, $/second; Replicate shows $/tahmini gösterir. İş yükünü modellemeden bunları bire bir karşılaştıramazsınız.

Daha da kötüsü, her fiyatlandırma sayfasının arkasındaki iş modeli farklıdır. Fireworks, paylaşılan GPU'larda kendi özel motorunu (FireAttention) çalıştırır; başına-token oranı kullanım eğrisini yansıtır. Baseten size Truss + özel GPU'lar sunar; Dakika başına düşen oran ayrıcalığı yansıtır. Modal gerçek Python sunucusuzdur; bir saniyenin altında soğuk başlatmayla saniye başına faturalandırma. Aynı çıktı (bir LLM yanıtı), üç farklı maliyet fonksiyonu.
...
**Özel silikon** — Groq (LPU), Cerebras (WSE), SambaNova (RDU). Genellikle aynı modeldeki GPU tabanlı kümeye göre 5-10 kat daha hızlı kod çözme. token başına daha yüksek fiyat (Groq, 2025 sonlarında Llama-70B'de ~0,99$/milyon idi) ancak gecikmeye duyarlı kullanım durumları için rakipsizdir. Groq, sesli agent'lar ve gerçek zamanlı çeviri için prodüksiyon seçimidir.
...
- 2026 ortası: 1 Mayıs 2026'dan itibaren isteğe bağlı GPU kiralama ücreti saat başına 1 ABD dolarına yükseltildi. Toplu fiyatlandırmada belirli ölçekte pazarlık yapılabilir.
- Mali sinyal: 4 milyar dolarlık değerleme, 10T+ tokens/gün halledildi.
...
- Tek bir API'de Inference + fine-tuning + eğitim.

### Baseten — kurumsal kullanım için optimize edilmiş

- Kafes framework: bağımlılıkları, sırları, hizmet yapılandırmasını tek bir bildirimde içeren model paketleme.
...
- $5B valuation, January 2026 Series E ($CapitalG, IVP, NVIDIA'dan 300 milyon milyon).

### Modal — Python'da yerel olarak optimize edilmiş

- Saf Python'da kod olarak altyapı. Bir işlevi `@modal.function(gpu="A100")` ile süsleyin ve tek komutla konuşlandırın.
- Saniye başına faturalandırma. Soğuk, ön ısıtmayla 2-4 saniye başlar; Küçük modeller için <1s.
- $87M Series B at $1,1 milyar değerleme (2025). Bağımsız anketlerde en güçlü geliştirici deneyimi puanı.
...
- LLM başına-token oranlarında daha az rekabetçi, ancak çok modlu çeşitlilikte kazanıyor.
...
### Dakika başına -token başına - her biri kazandığında

Per-token, iş yükünün gecikmeye duyarsız ve yoğun olduğu durumlarda mantıklıdır; yalnızca kullandığınız kadar ödersiniz. Kullanımın yüksek ve öngörülebilir olduğu durumlarda dakika başına değer mantıklıdır — GPU'yu doyurduğunuzda-token başına geçersiniz.

Kaba kural: Özel bir GPU'nun sürekli kullanımının ~%30'un üzerindeki iş yükleri için, dakika başına (Baseten, Modal), -token (Fireworks, Together) başına geçmeye başlar. Bunun altında, boşta kalmak için ödeme yapmaktan kaçındığınız için per-token kazanır.

### Özel motor asıl hendektir

vLLM ve SGLang'ın üzerindeki her platform özel bir motora sahiptir. FireAttention, RayTurbo, Baseten'in inference yığını. Özel motor, pazarlamayı gölgede bırakıyor — dürüst çerçeve, vLLM + SGLang'ın açık kaynak inference üretiminin yaklaşık %80'ini temsil ettiği ve platform katmanındaki farklılaştırıcıların DX, ilişkilendirme ve SLA'lar olduğu yönündedir.
...
- Temel değerleme: $5B (Series E, Jan 2026, $300 milyon tur).
- Modal değerleme: 1,1 Milyar Dolar (Seri B, 2025).
- Dakika başına vuruş başına-token ~%30'un üzerinde sürekli kullanım.
...
`code/main.py` , altı tedarikçiyi sentetik bir iş yükünde farklı fiyatlandırma modellerinde karşılaştırıyor. $/day and effective $/M tokens'yi raporlar. -token ve dakika başına arasındaki başabaş noktasını bulmak için bunu çalıştırın.

## Ship It — Kullanıma Sun

Bu ders `outputs/skill-inference-platform-picker.md` üretir. İş yükü profili, SLA ve bütçe göz önüne alındığında, birincil inference platformunu seçer ve ikinciyi belirler.

## Egzersizler

1. `code/main.py`'yı çalıştırın. Baseten (dakika başına) hangi sürekli kullanımda bir H100'de 70B modeli için Fireworks'ü (token başına) yener? Çapraz geçişi kendiniz türetin ve temel kuralla karşılaştırın.
2. Ürününüz görüntü oluşturmanın yanı sıra sohbetin yanı sıra konuşmayı metne dönüştürme olanağı da sunar. Her modalite için platformlar seçin ve bunları birleştiren ağ geçidi modelini adlandırın.
...
| Başına-token | "API fiyatlandırması" | Tüketilen token sn kadar şarj; boşta kalmamak için ödeme yapın |
...
- [Baseten Fiyatlandırma](https://www.baseten.co/pricing/) — dakika başına ücretler, taahhüt edilen kapasite, kurumsal katmanlar.
- [Modal Fiyatlandırma](https://modal.com/pricing) — saniye başına GPU oranları ve ücretsiz katman.
- [Birlikte AI Fiyatlandırması](https://www.together.ai/pricing) — model kataloğu ve başına-token oranları.
