...
def _assert_safe_torch_for_weights_only() -> None:
    """Ensure torch version is >= 2.6.1 for secure weights_only loading.

    Earlier versions (including 2.6.0) have known vulnerabilities (GHSA-53q9-r3pm-6pq6, etc.).
    """
    ...
    if (major, minor, patch) < (2, 6, 1):
        raise RuntimeError(
            f"torch>=2.6.1 is required for secure weights_only deserialization (found {torch.__version__}). "
